...
$exc = new exchange($yke->table(1,'region'), $db, 'region_id', 'region_name');
...
if ($_REQUEST['act'] == 'list')
{
    ...
    if ($region_id > 0)
    {
        ...
        $action_link = array('text' => $_LANG['back_page'], 'href' => 'area_manage.php?sysessid='.$GLOBALS['sysessid'].'&act=list&&pid=' . $parent_id);
    }
    else
    {
        $action_link = '';
    ...
    $smarty->assign('sysessid',   $GLOBALS['sysessid']);
    assign_query_info();
    $smarty->assign('sysessid',   $GLOBALS['sysessid']);  $smarty->display('area_list.htm');
}
...
elseif ($_REQUEST['act'] == 'add_area')
{
    ...
    $sql = "INSERT INTO " . $yke->table(1,'region') . " (sysid,parent_id, region_name, region_type) ".
           "VALUES ("."'".$GLOBALS['sysid']."',"."'$parent_id', '$region_name', '$region_type')";
...
elseif ($_REQUEST['act'] == 'drop_area')
{
    ...
    $sql = "SELECT * FROM " . $yke->table(1,'region') . " WHERE region_id = '$id'"." and sysid ='".$GLOBALS['sysid']."' ";
    $region = $db->getRow($sql);

//    /* 如果底下有下级区域,不能删除 */
//    $sql = "SELECT COUNT(*) FROM " . $yke->table(1,'region') . " WHERE parent_id = '$id'";
//    if ($db->getOne($sql) > 0)
//    {
...
//    }
    ...
    $sql="DELETE FROM ". $yke->table(1,"region")."WHERE region_id".db_create_in($delete_region)." and sysid ='".$GLOBALS['sysid']."' ";
     $db->query($sql);
    if ($exc->drop($id))
    {
...
function new_region_id($region_id)
{
    ...
    $sql="SELECT region_id FROM ".$GLOBALS['yke']->table(1,"region")."WHERE parent_id ".db_create_in($region_id)." and sysid ='".$GLOBALS['sysid']."' ";
