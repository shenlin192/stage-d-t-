...
class cls_session
{
    ...
    function __construct(&$db, $session_table, $session_data_table, $session_name = 'yke_ID', $session_id = '')
    {
        $this->cls_session($db, $session_table, $session_data_table, $session_name, $session_id);
    }

    function cls_session(&$db, $session_table, $session_data_table, $session_name = 'yke_ID', $session_id = '')
    {
        //echo 'session  $session_id'. $session_id;
   ...
   //     include(ROOTWEB_PATH.'modules/public/includes/cls_getmac.php');//获取mac地址
   //     $maccode=new cls_getmac();
        $this->_mac = $GLOBALS['maccode']->mac_addr;
        ...
        {
            $this->session_id = $_COOKIE[$this->session_name];//借助cookie，如果不能存取cookie，？所以最好能够传递参数
        }
        else
        {
        ...
        //echo '$this->session_id 15 '. $this->session_id;
        ...
        //           echo '4$this->session_id = '.$this->session_id."\r\n";
                   //echo '5$tmp_session_id = '.$tmp_session_id."\r\n";
                   //echo '6 $this->_ip = '. $this->_ip."\r\n";
                   //echo '60 $this->gen_session_key($tmp_session_id) = '. $this->gen_session_key($tmp_session_id)."\r\n";
                   //echo '61 $this->session_id = '.$this->session_id."\r\n";
                   ...
                   //echo '63 $this->session_id = '.$this->session_id."\r\n";
            */
               }
               else
               {
       ...
       //            echo 'session  $this->session_id 13'. $this->session_id;
                   ...
                   //$this->session_id . $this->gen_session_key($this->session_id);保持保存到数据库和保存在cookie中的session一致
               }
           ...
           function gen_session_id()
           {
               ...
               $this->session_id = $this->session_id . $this->gen_session_key($this->session_id);//增加本地属性
//               echo 'session  $this->session_id 12'. $this->session_id;
           ...
           function gen_session_key($session_id)
           {
               ...
               //echo    crc32(ROOT_PATH . $ip . $session_id);
               //echo sprintf('%08x', crc32(ROOT_PATH . $ip . $session_id));

               return sprintf('%08x', crc32(ROOT_PATH . $ip . $this->_mac . $session_id));
           }

           function insert_session()
           {
//               echo 'session  $this->session_id'. $this->session_id;

               return $this->db->query('INSERT INTO ' . $this->session_table . " (sessname,sesskey, expiry,mac,ip, data) VALUES ('" . $this->session_name . "','" . $this->session_id . "', '". $this->_time ."', '". $this->_mac ."', '". $this->_ip ."', 'a:0:{}')");
           }

           function load_session()
           {
               $session = $this->db->getRow('SELECT sesskey,sessname,userid, adminid, user_name, user_rank, discount, email, data, expiry FROM ' . $this->session_table . " WHERE sesskey = '" . $this->session_id . "'" );
               if (empty($session))
               {
                   $this->insert_session();
               ...
               else
               {
                   if (!empty($session['data']) && $this->_time - $session['expiry'] <= $this->max_life_time)
                   {
                       ...
                       $GLOBALS['_SESSION']['sesskey'] =$session['sesskey'];
                       $GLOBALS['_SESSION']['sessname'] =$session['sessname'];
    ...
    function destroy_session()
    {
        ...
        /*
        if (!empty($GLOBALS['yke']))
        {
            $this->db->query('DELETE FROM ' . $GLOBALS['yke']->table(1,'cart') . " WHERE session_id = '$this->session_id'");
        }
        */
