...
function get_categories_tree($cat_id = 0)
{
    if ($cat_id > 0)
    {
        $sql = 'SELECT parent_id FROM ' . $GLOBALS['yke']->table(1,'category') . " WHERE cat_id = '$cat_id'"." and sysid ='".$GLOBALS['sysid']."' ";
    ...
    /*
     ...
     如果不是取当前分类及其下的子分类
    */
    $sql = 'SELECT count(*) FROM ' . $GLOBALS['yke']->table(1,'category') . " WHERE parent_id = '$parent_id' AND is_show = 1 "." and sysid ='".$GLOBALS['sysid']."' ";
    if ($GLOBALS['db']->getOne($sql) || $parent_id == 0)
    {
        /* 获取当前分类及其子分类 */
        $sql = 'SELECT cat_id,cat_name ,parent_id,is_show,type_img ' .
                'FROM ' . $GLOBALS['yke']->table(1,'category') .
                "WHERE parent_id = '$parent_id' AND is_show = 1 AND is_virtual=0 "." and sysid ='".$GLOBALS['sysid']."' "."ORDER BY sort_order ASC, cat_id ASC";
...
{
    $three_arr = array();
    $sql = 'SELECT count(*) FROM ' . $GLOBALS['yke']->table(1,'category') . " WHERE parent_id = '$tree_id' AND is_show = 1 "." and sysid ='".$GLOBALS['sysid']."' ";
    if ($GLOBALS['db']->getOne($sql) || $tree_id == 0)
    {
        $child_sql = 'SELECT cat_id, cat_name, parent_id, is_show,type_img ' .
                'FROM ' . $GLOBALS['yke']->table(1,'category') .
                "WHERE parent_id = '$tree_id' AND is_show = 1 "." and sysid ='".$GLOBALS['sysid']."' "." ORDER BY sort_order ASC, cat_id ASC";
...
function get_top10($cats = '')
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.shop_price, g.goods_thumb, SUM(og.goods_number) as goods_number ' .
           ...
           "WHERE g.sysid=o.sysid and g.sysid=og.sysid and g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 $where $top10_time " ." and g.sysid ='".$GLOBALS['sysid']."' ";
    //判断是否启用库存，库存数量是否大于0
    ...
    if (empty($type_goods[$type]))
    {
        ...
        if ($data === false)
        {
            $sql = 'SELECT g.goods_id, g.is_best, g.is_new, g.is_hot, g.is_promote, b.brand_name,g.sort_order ' .
               ' FROM ' . $GLOBALS['yke']->table(1,'goods') . ' AS g ' .
               ' LEFT JOIN ' . $GLOBALS['yke']->table(1,'brand') . ' AS b ON b.brand_id = g.brand_id and b.sysid=g.sysid ' .
               ' WHERE g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 AND (g.is_best = 1 OR g.is_new =1 OR g.is_hot = 1)'." and g.sysid ='".$GLOBALS['sysid']."' ".
        ...
        $sql .= ' WHERE g.goods_id ' . db_create_in($type_merge)." and g.sysid ='".$GLOBALS['sysid']."' ";
...
function get_promote_goods($cats = '')
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
            ...
            'LEFT JOIN ' . $GLOBALS['yke']->table(1,'brand') . ' AS b ON b.brand_id = g.brand_id and b.sysid=g.sysid ' .
            "LEFT JOIN " . $GLOBALS['yke']->table(1,'member_price') . " AS mp ".
                "ON mp.sysid=g.sysid and mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
            'WHERE g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 ' ." and g.sysid ='".$GLOBALS['sysid']."' ".
...
function get_category_recommend_goods($type = '', $cats = '', $brand = 0, $min =0,  $max = 0, $ext='')
{
    ...
    $sql =  'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
                ...
                'promote_start_date, promote_end_date, g.goods_brief, g.goods_thumb, goods_img, b.brand_name ' .
            'FROM ' . $GLOBALS['yke']->table(1,'goods') . ' AS g ' .
            'LEFT JOIN ' . $GLOBALS['yke']->table(1,'brand') . ' AS b ON b.brand_id = g.brand_id and g.sysid=b.sysid ' .
            "LEFT JOIN " . $GLOBALS['yke']->table(1,'member_price') . " AS mp ".
                    "ON mp.sysid=g.sysid and mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' "." and g.sysid ='".$GLOBALS['sysid']."' ".
...
function get_goods_info($goods_id)
{
    ...
    $sql = 'SELECT g.*, c.measure_unit, b.brand_id, b.brand_name AS goods_brand, m.type_money AS bonus_money, ' .
            ...
            'FROM ' . $GLOBALS['yke']->table(1,'goods') . ' AS g ' .
            'LEFT JOIN ' . $GLOBALS['yke']->table(1,'category') . ' AS c ON g.cat_id = c.cat_id and g.sysid=c.sysid ' .
            'LEFT JOIN ' . $GLOBALS['yke']->table(1,'brand') . ' AS b ON g.brand_id = b.brand_id  and g.sysid=b.sysid ' .
            'LEFT JOIN ' . $GLOBALS['yke']->table(1,'comment') . ' AS r '.
                'ON r.id_value = g.goods_id  and g.sysid=r.sysid AND comment_type = 0 AND r.parent_id = 0 AND r.status = 1 ' .
            'LEFT JOIN ' . $GLOBALS['yke']->table(1,'bonus_type') . ' AS m ' .
                "ON g.bonus_type_id = m.type_id  and g.sysid=m.sysid AND m.send_start_date <= '$time' AND m.send_end_date >= '$time'" .
            " LEFT JOIN " . $GLOBALS['yke']->table(1,'member_price') . " AS mp ".
                    "ON mp.goods_id = g.goods_id  and g.sysid=mp.sysid AND mp.user_rank = '$_SESSION[user_rank]' ".
            "WHERE g.goods_id = '$goods_id' AND g.is_delete = 0 " ." and g.sysid ='".$GLOBALS['sysid']."' ".
            "GROUP BY g.goods_id";
...
function get_goods_properties($goods_id)
{
    ...
    $sql = "SELECT attr_group ".
            "FROM " . $GLOBALS['yke']->table(1,'goods_type') . " AS gt, " . $GLOBALS['yke']->table(1,'goods') . " AS g ".
            "WHERE g.goods_id='$goods_id' AND gt.cat_id=g.goods_type and gt.sysid=g.sysid "." and gt.sysid ='".$GLOBALS['sysid']."' ";
    ...
    $sql = "SELECT a.attr_id, a.attr_name, a.attr_group, a.is_linked, a.attr_type, ".
            ...
            'FROM ' . $GLOBALS['yke']->table(1,'goods_attr') . ' AS g ' .
            'LEFT JOIN ' . $GLOBALS['yke']->table(1,'attribute') . ' AS a ON a.attr_id = g.attr_id and g.sysid=a.sysid ' .
    ...
    if (!empty($attr))
    {
        foreach ($attr['lnk'] AS $key => $val)
        {
            ...
            $sql = 'SELECT g.goods_id, g.goods_name, g.goods_thumb, g.goods_img, g.shop_price AS org_price, ' .
                    ...
                    'FROM ' . $GLOBALS['yke']->table(1,'goods') . ' AS g ' .
                    'LEFT JOIN ' . $GLOBALS['yke']->table(1,'goods_attr') . ' as a ON g.goods_id = a.goods_id and g.sysid = a.sysid ' .
                    "LEFT JOIN " . $GLOBALS['yke']->table(1,'member_price') . " AS mp ".
                        "ON mp.goods_id = g.goods_id  and g.sysid = mp.sysid AND mp.user_rank = '$_SESSION[user_rank]' ".
                    "WHERE a.attr_id = '$key' AND g.is_on_sale=1 AND a.attr_value = '$val[value]' AND g.goods_id <> '$_REQUEST[id]' " ." and g.sysid ='".$GLOBALS['sysid']."' ".
...
function get_goods_gallery($goods_id)
{
    ...
    $sql = 'SELECT img_id, img_url, thumb_url, img_desc' .
        ' FROM ' . $GLOBALS['yke']->table(1,'goods_gallery') .
        " WHERE goods_id = '$goods_id' "." and sysid ='".$GLOBALS['sysid']."' "."LIMIT " . $GLOBALS['_CFG']['goods_gallery_number'];
...
function assign_cat_goods($cat_id, $num = 0, $from = 'web', $order_rule = '')
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_name,g.click_count,g.market_price, g.shop_price AS org_price, ' .
            ...
            'WHERE g.is_on_sale = 1 AND g.is_alone_sale = 1 AND '.
                'g.is_delete = 0 AND (' . $children . 'OR ' . get_extension_goods($children) . ') '." and g.sysid ='".$GLOBALS['sysid']."' ";
    ...
    /* 分类信息 */
    $sql = 'SELECT cat_name FROM ' . $GLOBALS['yke']->table(1,'category') . " WHERE cat_id = '$cat_id'"." and sysid ='".$GLOBALS['sysid']."' ";
...
function assign_brand_goods($brand_id, $num = 0, $cat_id = 0,$order_rule = '')
{
    $sql =  'SELECT g.goods_id, g.goods_name, g.market_price, g.shop_price AS org_price, ' .
            ...
            "LEFT JOIN " . $GLOBALS['yke']->table(1,'member_price') . " AS mp ".
                    "ON mp.goods_id = g.goods_id and mp.sysid=g.sysid AND mp.user_rank = '$_SESSION[user_rank]' ".
            "WHERE g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 AND g.brand_id = '$brand_id'"." and g.sysid ='".$GLOBALS['sysid']."' ";
    ...
    /* 分类信息 */
    $sql = 'SELECT brand_name FROM ' . $GLOBALS['yke']->table(1,'brand') . " WHERE brand_id = '$brand_id'"." and sysid ='".$GLOBALS['sysid']."' ";
...
function get_extension_goods($cats)
{
    ...
    $sql = 'SELECT goods_id FROM ' . $GLOBALS['yke']->table(1,'goods_cat') . " AS g WHERE $cats"." and sysid ='".$GLOBALS['sysid']."' ";
...
function spec_price($spec)
{
    if (!empty($spec))
    {
        ...
        $sql = 'SELECT SUM(attr_price) AS attr_price FROM ' . $GLOBALS['yke']->table(1,'goods_attr') . " WHERE $where"." and sysid ='".$GLOBALS['sysid']."' ";
...
function group_buy_info($group_buy_id, $current_num = 0)
{
    ...
    $sql = "SELECT *, act_id AS group_buy_id, act_desc AS group_buy_desc, start_time AS start_date, end_time AS end_date " .
            ...
            "AND act_type = '" . GAT_GROUP_BUY . "'"." and sysid ='".$GLOBALS['sysid']."' ";
...
function group_buy_stat($group_buy_id, $deposit)
{
    ...
    $sql = "SELECT COUNT(*) AS total_order, SUM(g.goods_number) AS total_goods " .
            "FROM " . $GLOBALS['yke']->table(1,'order_info') . " AS o, " .
                $GLOBALS['yke']->table(1,'order_goods') . " AS g " .
            " WHERE o.order_id = g.order_id and o.sysid=g.sysid " .
            "AND o.extension_code = 'group_buy' " .
            "AND o.extension_id = '$group_buy_id' " .
            "AND g.goods_id = '$group_buy_goods_id' " .
            "AND (order_status = '" . OS_CONFIRMED . "' OR order_status = '" . OS_UNCONFIRMED . "')"." and o.sysid ='".$GLOBALS['sysid']."' ";
...
{
    $sql = "SELECT * FROM " . $GLOBALS['yke']->table(1,'goods_activity') . " WHERE act_id = '$act_id'"." and sysid ='".$GLOBALS['sysid']."' ";
    ...
    /* 查询出价用户数和最后出价 */
    $sql = "SELECT COUNT(DISTINCT bid_user) FROM " . $GLOBALS['yke']->table(1,'auction_log') .
            " WHERE act_id = '$act_id'"." and sysid ='".$GLOBALS['sysid']."' ";
    $auction['bid_user_count'] = $GLOBALS['db']->getOne($sql);
    if ($auction['bid_user_count'] > 0)
    {
        $sql = "SELECT a.*, u.user_name " .
                "FROM " . $GLOBALS['yke']->table(1,'auction_log') . " AS a, " .
                        $GLOBALS['yke']->table(1,'users') . " AS u " .
                "WHERE a.bid_user = u.user_id and a.sysid=u.sysid " .
                "AND act_id = '$act_id' "." and a.sysid ='".$GLOBALS['sysid']."' " .
...
{
    $sql = "SELECT * FROM " . $GLOBALS['yke']->table(1,'favourable_activity') .
            " WHERE act_id = '$act_id'"." and sysid ='".$GLOBALS['sysid']."' ";
    $row = $GLOBALS['db']->getRow($sql);
    if (!empty($row))
    {
...
{
    $sql = "SELECT * FROM " . $GLOBALS['yke']->table(1,'wholesale') .
            " WHERE act_id = '$act_id'"." and sysid ='".$GLOBALS['sysid']."' ";
    $row = $GLOBALS['db']->getRow($sql);
    if (!empty($row))
    {
...
function get_goods_attr($goods_id)
{
    ...
    $sql = "SELECT a.attr_id, a.attr_name " .
            "FROM " . $GLOBALS['yke']->table(1,'goods') . " AS g, " . $GLOBALS['yke']->table(1,'attribute') . " AS a " .
            "WHERE g.goods_id = '$goods_id' " .
            "AND g.goods_type = a.cat_id and g.sysid=a.sysid " .
            "AND a.attr_type = 1"." andg.sysid ='".$GLOBALS['sysid']."' ";
    $attr_id_list = $GLOBALS['db']->getCol($sql);
    $res = $GLOBALS['db']->query($sql);
    while ($attr = $GLOBALS['db']->fetchRow($res))
    {
        if (defined('yke_ADMIN'))
        {
    ...
    $sql = "SELECT attr_id, goods_attr_id, attr_value " .
            "FROM " . $GLOBALS['yke']->table(1,'goods_attr') .
            " WHERE goods_id = '$goods_id' " .
            "AND attr_id " . db_create_in($attr_id_list)." and sysid ='".$GLOBALS['sysid']."' ";
    $res = $GLOBALS['db']->query($sql);
...
function get_goods_fittings($goods_list = array())
{
    ...
    $sql = 'SELECT gg.parent_id, ggg.goods_name AS parent_name, gg.goods_id, gg.goods_price, g.goods_name, g.goods_thumb, g.goods_img, g.shop_price AS org_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price ".
            'FROM ' . $GLOBALS['yke']->table(1,'group_goods') . ' AS gg ' .
            'LEFT JOIN ' . $GLOBALS['yke']->table(1,'goods') . 'AS g ON g.goods_id = gg.goods_id and gg.sysid=g.sysid ' .
            "LEFT JOIN " . $GLOBALS['yke']->table(1,'member_price') . " AS mp ".
                    "ON mp.goods_id = gg.goods_id and mp.sysid=gg.sysid AND mp.user_rank = '$_SESSION[user_rank]' ".
            "LEFT JOIN " . $GLOBALS['yke']->table(1,'goods') . " AS ggg ON ggg.goods_id = gg.parent_id and ggg.sysid=gg.sysid ".
            "WHERE gg.parent_id " . db_create_in($goods_list) . " AND g.is_delete = 0 AND g.is_on_sale = 1 "." and gg.sysid ='".$GLOBALS['sysid']."' ".
...
function get_products_info($goods_id, $spec_goods_attr_id)
{
    ...
    if(isset($goods_attr_array['sort']))
    {
        ...
        $sql = "SELECT * FROM " .$GLOBALS['yke']->table(1,'products'). " WHERE goods_id = '$goods_id' AND goods_attr = '$goods_attr' "." and sysid ='".$GLOBALS['sysid']."' "." LIMIT 0, 1";
...
'FROM ' . $GLOBALS['yke']->table(1,'category') .
"WHERE parent_id = '$parent_id' AND is_show = 1 "." and sysid ='".$GLOBALS['sysid']."' "." ORDER BY sort_order ASC, cat_id ASC ";
$res = $GLOBALS['db']->getAll($child_sql);
foreach ($res AS $row)
{
if ($row['is_show'])
...
{
     $sql= "select sum(goods_number) as count from ".$GLOBALS['yke']->table(1,'order_goods')."where goods_id ='".$goods_id."'"." and sysid ='".$GLOBALS['sysid']."' ";
...
function get_pc_url(){
   ...
   $sql = "select value from ".$yke->table(1,'ykesmart_shop_config')." where code = 'pc_url' "." and sysid ='".$GLOBALS['sysid']."' ";

   return $db->getOne($sql);
}
function get_evaluation_sum($goods_id)
{
$sql = "SELECT count(*) FROM " . $GLOBALS['yke']->table(1,'comment') . " WHERE status=1 and  comment_type =0 and id_value =".$goods_id ." and sysid ='".$GLOBALS['sysid']."' ";//status=1表示通过了的评论才算  comment_type =0表示针对商品的评价
...
'FROM ' . $GLOBALS ['YKE']->table ( 'goods' ) . ' AS g ' . 
'LEFT JOIN ' . $GLOBALS ['YKE']->table ( 'category' ) . ' AS c ON c.cat_id = g.cat_id and c.sysid=g.sysid ' .
"LEFT JOIN " . $GLOBALS ['YKE']->table ( 'member_price' ) . " AS mp " . 
"ON mp.goods_id = g.goods_id and mp.sysid = g.sysid AND mp.user_rank = '$_SESSION[user_rank]' " .
"Where g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 " ." and g.sysid ='".$GLOBALS['sysid']."' ".
...
function pre_sale_info($pre_sale_id, $current_num = 0)
{
    ...
    $sql = "SELECT b.*,g.*, b.act_id AS pre_sale_id, b.act_desc AS pre_sale_desc, b.start_time, b.end_time " .
        "FROM " . $GLOBALS['yke']->table(1,'goods_activity') . " AS b " .
        "LEFT JOIN " . $GLOBALS['yke']->table(1,'goods') . " AS g ON b.goods_id = g.goods_id and b.sysid=g.sysid " .
        "WHERE act_id = '$pre_sale_id' " .
        "AND act_type = '" . GAT_PRE_SALE . "'"." and b.sysid ='".$GLOBALS['sysid']."' ";
    ...
    // 获取商品描述
    $pre_sale['goods_desc'] = $GLOBALS['db']->getOne("select goods_desc from ".$GLOBALS['yke']->table(1,'goods')." where goods_id=".$pre_sale['goods_id']." and sysid ='".$GLOBALS['sysid']."' ");
...
function pre_sale_stat($pre_sale_id, $deposit)
{
    ...
    $sql = "SELECT goods_id " .
        "FROM " . $GLOBALS['yke']->table(1,'goods_activity') .
        "WHERE act_id = '$pre_sale_id' " .
        "AND act_type = '" . GAT_PRE_SALE . "'"." and sysid ='".$GLOBALS['sysid']."' ";
...
{
    $sql = "select count(*) from " . $GLOBALS['yke']->table(1,'comment') . " where id_value = '$goods_id' and status = 0 and parent_id = 0"." and sysid ='".$GLOBALS['sysid']."' ";
...
function goods_sale_count($goods_id)
{
    ...
    $sql = 'SELECT IFNULL(SUM(g.goods_number), 0) ' .
        ...
        " AND g.goods_id = '$goods_id'"." and sysid ='".$GLOBALS['sysid']."' ";
