...
function get_parent_cats($cat)
{
    ...
    $arr = $GLOBALS['db']->GetAll('SELECT cat_id, cat_name, parent_id FROM ' . $GLOBALS['yke']->table(1,'category'));
...
{
    $sql = 'SELECT cat_name FROM ' . $GLOBALS['yke']->table(1,'ykesmart_article_cat') . " WHERE cat_id = '" . $id ."'";
...
function recalculate_price()
{
    ...
    $sql = 'SELECT c.rec_id, c.goods_id, c.goods_attr_id, g.promote_price, g.promote_start_date, c.goods_number,'.
            ...
            'LEFT JOIN ' . $GLOBALS['yke']->table(1,'goods') . ' AS g ON g.goods_id = c.goods_id '.
   ...
   while ($row = $GLOBALS['db']->fetchRow($res))
    {
		...
		if($rec_id)
		{
			...
			$sql="delete from ".$GLOBALS['yke']->table(1,'cart')." where rec_id='".$row['rec_id']."'";
			$GLOBALS['db']->query($sql);
		}
		else
		{
...
function assign_comment($id, $type, $page = 1,$comment_level=0)
{
	...
	if($row['user_id'])
	{
		$arr[$row['comment_id']]['headimg'] = $GLOBALS['db']->getOne('select headimg from '. $GLOBALS['yke']->table(1,'users') .' where user_id=\''. $row['user_id'] .'\'');
	}

	/* 会员级别 */
	$sql_ur = "select user_rank  from " . $GLOBALS['yke']->table(1,'users') . " where user_id = '$row[user_id]' ";
...
function article_categories_tree($cat_id = 0)
{
    if ($cat_id > 0)
    {
        $sql = 'SELECT parent_id FROM ' . $GLOBALS['yke']->table(1,'ykesmart_article_cat') . " WHERE cat_id = '$cat_id'";
    ...
    /*
     ...
     如果不是取当前分类及其下的子分类
    */
    $sql = 'SELECT count(*) FROM ' . $GLOBALS['yke']->table(1,'ykesmart_article_cat') . " WHERE parent_id = '$parent_id'";
    if ($GLOBALS['db']->getOne($sql))
    {
        ...
        $sql = 'SELECT a.cat_id, a.cat_name, a.sort_order AS parent_order, a.cat_id, ' .
                ...
                'LEFT JOIN ' . $GLOBALS['yke']->table(1,'ykesmart_article_cat') . ' AS b ON b.parent_id = a.cat_id ' .
...
function get_article_parent_cats($cat)
{
    ...
    $arr = $GLOBALS['db']->GetAll('SELECT cat_id, cat_name, parent_id FROM ' . $GLOBALS['yke']->table(1,'ykesmart_article_cat'));
...
function license_info()
{
    if($GLOBALS['_CFG']['licensed'] > 0)
    {
        ...
        $license = '<a href="http://www.ykshop.com/license.php?product=ykshop_b2c&url=' . urlencode($host) . '" target="_blank"
>&nbsp;&nbsp;Licensed</a>';
        return $license;
    }
    else
