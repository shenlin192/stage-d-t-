                ...
                switch ($s[0])
                {
                    case 'escape':
                        ...
                        elseif ($s[1] == 'u8_url')
                        {
                            if (EC_CHARSET != 'utf-8')
                            {
                                $p = 'urlencode(yk_iconv("' . EC_CHARSET . '", "utf-8",' . $p . '))';
                            }
                            else
                            {
                                $p = 'urlencode(' . $p . ')';
...
//        echo $content;
        ...
        eval('?' . '>' . trim($content));//eval — 把字符串作为PHP代码执行
        $content = ob_get_contents();//此函数返回输出缓冲区的内容，或者如果输出缓冲区无效将返回 FALSE 。
        ob_end_clean();
        return $content;
    }
      ...
      {
          weixindebug($url,'$url',0,'','','','mobile/includes/cls_template.php:1424',1424);
      }
