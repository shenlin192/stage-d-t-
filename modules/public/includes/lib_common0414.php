...
{
    $sql = 'SELECT region_id, region_name FROM ' . $GLOBALS['yke']->table(1,'region') .
            " WHERE region_type = '$type' AND parent_id = '$parent'"." and sysid ='".$GLOBALS['sysid']."' ";
...
{
    /* 获得配置信息 */
    $sql = 'SELECT configure FROM ' . $GLOBALS['yke']->table(1,'shipping_area') . " WHERE shipping_area_id = '$area_id'"." and sysid ='".$GLOBALS['sysid']."' ";
...
//    echo $GLOBALS['_CFG']['integrate_code'];
    ...
    if ($res === NULL)
    {
        ...
        if ($data === false)
        {
            $sql = "SELECT c.cat_id, c.cat_name, c.measure_unit, c.parent_id, c.is_show, c.show_in_nav, c.grade, c.sort_order, COUNT(s.cat_id) AS has_children ".
                'FROM ' . $GLOBALS['yke']->table(1,'category') . " AS c ".
                "LEFT JOIN " . $GLOBALS['yke']->table(1,'category') . " AS s ON s.parent_id=c.cat_id and c.sysid=s.sysid ".
                 "WHERE c.is_virtual= '0' "." and c.sysid ='".$GLOBALS['sysid']."' ".
                "GROUP BY c.cat_id ".
            ...
            $sql = "SELECT cat_id, COUNT(*) AS goods_num " .
                    ...
                    " WHERE is_delete = 0 AND is_on_sale = 1 " ." and sysid ='".$GLOBALS['sysid']."' ".
                    " GROUP BY cat_id";
            $res2 = $GLOBALS['db']->getAll($sql);

            $sql = "SELECT gc.cat_id, COUNT(*) AS goods_num " .
                    " FROM " . $GLOBALS['yke']->table(1,'goods_cat') . " AS gc , " . $GLOBALS['yke']->table(1,'goods') . " AS g " .
                    " WHERE g.goods_id = gc.goods_id and gc.sysid=g.sysid AND g.is_delete = 0 AND g.is_on_sale = 1 " ." and gc.sysid ='".$GLOBALS['sysid']."' ".
                    " GROUP BY gc.cat_id";
...
function load_config()
{
    ...
    {
       // echo 'a2';
        $sql = 'SELECT code, value FROM ' . $GLOBALS['yke']->table(1,'ykesmart_shop_config') . ' WHERE parent_id > 0'." and sysid ='".$GLOBALS['sysid']."' ";
        ...
        if (!isset($GLOBALS['_CFG']['yke_version']))
        {
            /* 如果没有版本号则默认为2.0.5 */
            $GLOBALS['_CFG']['yke_version'] = 'v2.0.5';
        }
        ...
        {
            $arr['integrate_code'] = 'ykshop'; // 默认的会员整合插件为 ykshop
        }
...
{
    $sql = 'SELECT brand_id, brand_name FROM ' . $GLOBALS['yke']->table(1,'brand') ." where sysid ='".$GLOBALS['sysid']."' ". ' ORDER BY sort_order';
    $res = $GLOBALS['db']->getAll($sql);
...
function get_brands($cat = 0, $app = 'brand')
{
    ...
    $sql = "SELECT b.brand_id, b.brand_name, b.brand_logo, b.brand_img,b.brand_desc,b.wap_brand_desc, COUNT(*) AS goods_num, IF(b.brand_logo > '', '1', '0') AS tag ".
            "FROM " . $GLOBALS['yke']->table(1,'brand') . "AS b, ".
                $GLOBALS['yke']->table(1,'goods') . " AS g ".
            "WHERE b.sysid=g.sysid and g.brand_id = b.brand_id $children AND is_show = 1 " .
            " AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 "." and b.sysid ='".$GLOBALS['sysid']."' ".
...
function get_promotion_info($goods_id = '',$suppid=-1)
{
    ...
    $sql = 'SELECT act_id, act_name, act_type, start_time, end_time FROM ' . $GLOBALS['yke']->table(1,'goods_activity') . " WHERE is_finished=0 AND start_time <= '$gmtime' AND end_time >= '$gmtime'"." and sysid ='".$GLOBALS['sysid']."' ";
    if(!empty($goods_id))
    {
    ...
    $favourable = array();
    $sql = 'SELECT act_id, act_range, act_range_ext, act_name, start_time, end_time FROM ' . $GLOBALS['yke']->table(1,'favourable_activity') . " WHERE start_time <= '$gmtime' AND end_time >= '$gmtime' $where_suppid"." and sysid ='".$GLOBALS['sysid']."' ";
    if(!empty($goods_id))
    {
    ...
    {
        $sql = "SELECT cat_id, brand_id FROM " . $GLOBALS['yke']->table(1,'goods') .
...
{
    $sql = 'SELECT template_subject, is_html, template_content FROM ' . $GLOBALS['yke']->table(1,'mail_templates') . " WHERE template_code = '$tpl_name'"." and sysid ='".$GLOBALS['sysid']."' ";
...
function order_action($order_sn, $order_status, $shipping_status, $pay_status, $note = '', $username = null, $place = 0)
{
    ...
    $sql = 'INSERT INTO ' . $GLOBALS['yke']->table(1,'order_action') .
                ' (sysid,order_id, action_user, order_status, shipping_status, pay_status, action_place, action_note, log_time) ' .
            'SELECT ' .
                "sysid,order_id, '$username', '$order_status', '$shipping_status', '$pay_status', '$place', '$note', '" .gmtime() . "' " .
            'FROM ' . $GLOBALS['yke']->table(1,'order_info') . " WHERE order_sn = '$order_sn'"." and sysid ='".$GLOBALS['sysid']."' ";
...
function price_format($price, $change_price = true)
{
    ...
    if ($change_price && defined('yke_ADMIN') === false)
    {
...
function get_virtual_goods($order_id, $shipping = false)
{
    if ($shipping)
    {
        $sql = 'SELECT goods_id, goods_name, send_number AS num, extension_code FROM '.
           $GLOBALS['yke']->table(1,'order_goods') .
           " WHERE order_id = '$order_id' AND extension_code > ''"." and sysid ='".$GLOBALS['sysid']."' ";
    }
    else
    {
        $sql = 'SELECT goods_id, goods_name, (goods_number - send_number) AS num, extension_code FROM '.
           $GLOBALS['yke']->table(1,'order_goods') .
           " WHERE order_id = '$order_id' AND is_real = 0 AND (goods_number - send_number) > 0 AND extension_code > '' "." and sysid ='".$GLOBALS['sysid']."' ";
    }
    $res = $GLOBALS['db']->getAll($sql);
...
function virtual_card_shipping ($goods, $order_sn, &$msg, $process = 'other')
{
    ...
    /* 检查有没有缺货 */
    $sql = "SELECT COUNT(*) FROM ".$GLOBALS['yke']->table(1,'virtual_card')." WHERE goods_id = '$goods[goods_id]' AND is_saled = 0 "." and sysid ='".$GLOBALS['sysid']."' ";
    ...
    if ($num < $goods['num'])
    {
     ...
     /* 取出卡片信息 */
     $sql = "SELECT card_id, card_sn, card_password, end_date, crc32 FROM ".$GLOBALS['yke']->table(1,'virtual_card')." WHERE goods_id = '$goods[goods_id]' AND is_saled = 0 "." and sysid ='".$GLOBALS['sysid']."' "." LIMIT " . $goods['num'];
     ...
     /* 标记已经取出的卡片 */
    $sql = "UPDATE ".$GLOBALS['yke']->table(1,'virtual_card')." SET ".
           "is_saled = 1 ,".
           "order_sn = '$order_sn' ".
           "WHERE " . db_create_in($card_ids, 'card_id')." and sysid ='".$GLOBALS['sysid']."' ";
    ...
    /* 更新库存 */
    $sql = "UPDATE ".$GLOBALS['yke']->table(1,'goods'). " SET goods_number = goods_number - '$goods[num]' WHERE goods_id = '$goods[goods_id]'"." and sysid ='".$GLOBALS['sysid']."' ";
    $GLOBALS['db']->query($sql);

    if (true)
    {
        /* 获取订单信息 */
        $sql = "SELECT order_id, order_sn, consignee, email FROM ".$GLOBALS['yke']->table(1,'order_info'). " WHERE order_sn = '$order_sn'"." and sysid ='".$GLOBALS['sysid']."' ";
        ...
        if ($process == 'split')
        {
            $sql = "UPDATE ".$GLOBALS['yke']->table(1,'order_goods'). "
                    ...
                    AND goods_id = '" . $goods['goods_id'] . "' "." and sysid ='".$GLOBALS['sysid']."' ";
        }
        else
        {
            $sql = "UPDATE ".$GLOBALS['yke']->table(1,'order_goods'). "
...
function virtual_card_result($order_sn, $goods)
{
    ...
    /* 获取已经发送的卡片数据 */
    $sql = "SELECT card_sn, card_password, end_date, crc32 FROM ".$GLOBALS['yke']->table(1,'virtual_card')." WHERE goods_id= '$goods[goods_id]' AND order_sn = '$order_sn' "." and sysid ='".$GLOBALS['sysid']."' ";
...
function get_snatch_result($id)
{
    $sql = 'SELECT u.user_id, u.user_name, u.email, lg.bid_price, lg.bid_time, count(*) as num' .
            ' FROM ' . $GLOBALS['yke']->table(1,'snatch_log') . ' AS lg '.
            ' LEFT JOIN ' . $GLOBALS['yke']->table(1,'users') . ' AS u ON lg.user_id = u.user_id and lg.sysid = u.sysid '.
            " WHERE lg.snatch_id = '$id'"." and lg.sysid ='".$GLOBALS['sysid']."' ".
    ...
    if ($rec)
    {
        ...
        /* 活动信息 */
        $sql = 'SELECT ext_info " .
               " FROM ' . $GLOBALS['yke']->table(1,'goods_activity') .
               " WHERE act_id= '$id' AND act_type=" . GAT_SNATCH." and sysid ='".$GLOBALS['sysid']."' ".
               " LIMIT 1";
        ...
        $sql = "SELECT COUNT(*)" .
                ...
                " AND extension_id = '$id'" .
                " AND order_status " . db_create_in(array(OS_CONFIRMED, OS_UNCONFIRMED))." and sysid ='".$GLOBALS['sysid']."' ";
...
function build_uri($app, $params, $append = '', $page = 0, $keywords = '', $size = 0)
{
    ...
    switch ($app)
    {
    	case 'supplier':
            ...
            if ($go == 'category' || $go == 'index')
            {
                ...
                {
                    $uri = $app.'.php?sysessid='.$GLOBALS['sysessid'].'&go='.$go.'&amp;suppId='.$suppid.'&amp;id=' . $cid;
                    if (!empty($bid))
                    {
                        $uri .= '&amp;brand=' . $bid;
                    }
            ...
            {
                $uri = $rewrite ? $app.'-article-'.$suppid.'-' . $aid : $app.'.php?sysessid='.$GLOBALS['sysessid'].'&go=article&suppId='.$suppid.'&id=' . $aid;
            }elseif($go == 'search')
            {
            	if ($rewrite)
                {
                ...
                {
                    $uri = $app.'.php?sysessid='.$GLOBALS['sysessid'].'&go='.$go.'&amp;suppId='.$suppid;
                	if (!empty($cid))
                    {
                        $uri .= '&amp;cid=' . $cid;
                    }
        ...
        case 'stores':
            ...
            else
            {
                ...
                {
                    $uri = 'stores.php?sysessid='.$GLOBALS['sysessid'].'&id=' . $cid;
                    if (!empty($page))
                    {
                        $uri .= '&amp;page=' . $page;
                    }
        ...
        case 'category':
            ...
            else
            {
                ...
                {
                    $uri = 'category.php?sysessid='.$GLOBALS['sysessid'].'&id=' . $cid;
                    if (!empty($bid))
                    {
                        $uri .= '&amp;brand=' . $bid;
                    }
        ...
        case 'goods':
            ...
            else
            {
				...
				if(!empty($userid)){
					$uri = $rewrite ? 'goods-' . $gid : 'goods.php?sysessid='.$GLOBALS['sysessid'].'&id=' . $gid."&u=".$userid;
				}else{
					
					$uri = $rewrite ? 'goods-' . $gid : 'goods.php?sysessid='.$GLOBALS['sysessid'].'&id=' . $gid;
				}
				/*杭州一开修改*/
            }
        ...
        case 'brand':
            ...
            else
            {
                ...
                {
                    $uri = 'brand.php?sysessid='.$GLOBALS['sysessid'].'&id=' . $bid;
                    if (!empty($cid))
                    {
                        $uri .= '&amp;cat=' . $cid;
                    }
        ...
        case 'article_cat':
            ...
            else
            {
                ...
                {
                    $uri = 'article_cat.php?sysessid='.$GLOBALS['sysessid'].'&id=' . $acid;
                    if (!empty($page))
                    {
                        $uri .= '&amp;page=' . $page;
                    }
        ...
        case 'article':
            ...
            else
            {
				...
				if(!empty($userid)){
					...
					$uri = $rewrite ? 'article-' . $aid : 'article.php?sysessid='.$GLOBALS['sysessid'].'&id=' . $aid."&u=".$userid;
				}else{
					
					$uri = $rewrite ? 'article-' . $aid : 'article.php?sysessid='.$GLOBALS['sysessid'].'&id=' . $aid;
				}
				/*杭州一开修改*/
            }
        ...
        case 'group_buy':
            ...
            {
                $uri = $rewrite ? 'group_buy-' . $gbid : 'group_buy.php?sysessid='.$GLOBALS['sysessid'].'&act=view&amp;id=' . $gbid;
            }
        ...
        case 'auction':
            ...
            {
                $uri = $rewrite ? 'auction-' . $auid : 'auction.php?sysessid='.$GLOBALS['sysessid'].'&act=view&amp;id=' . $auid;
            }
        ...
        case 'snatch':
            ...
            {
                $uri = $rewrite ? 'snatch-' . $sid : 'snatch.php?sysessid='.$GLOBALS['sysessid'].'&id=' . $sid;
            }
	...
	case 'pro_search':
        ...
        case 'exchange':
            ...
            {
                $uri = 'exchange.php?sysessid='.$GLOBALS['sysessid'].'&cat_id=' . $cid;
                if (isset($price_min))
                {
        ...
        case 'exchange_goods':
            ...
            else
            {
				...
				if(!empty($userid)){
					...
					$uri = $rewrite ? 'exchange-id' . $gid : 'exchange.php?sysessid='.$GLOBALS['sysessid'].'&id=' . $gid ."&u=".$userid. '&amp;act=view';
				}else{
					
					$uri = $rewrite ? 'exchange-id' . $gid : 'exchange.php?sysessid='.$GLOBALS['sysessid'].'&id=' . $gid . '&amp;act=view';
				}
				/*杭州一开修改*/
            }
            break;
...
function log_account_change($user_id, $user_money = 0, $frozen_money = 0, $rank_points = 0, $pay_points = 0, $change_desc = '', $change_type = ACT_OTHER)
{
    ...
    $GLOBALS['db']->autoExecute($GLOBALS['yke']->table(1,'account_log'), $account_log, 'INSERT');
	$log_id=$GLOBALS['db']->insert_id();	
    /* 更新用户信息 */
    $sql = "UPDATE " . $GLOBALS['yke']->table(1,'users') .
            " SET user_money = user_money + ('$user_money')," .
            " frozen_money = frozen_money + ('$frozen_money')," .
            " rank_points = rank_points + ('$rank_points')," .
            " pay_points = pay_points + ('$pay_points')" .
            " WHERE user_id = '$user_id'"." and sysid ='".$GLOBALS['sysid']."' "." LIMIT 1";
    $GLOBALS['db']->query($sql);
	return $log_id;
}
    ...
    if ($res === NULL)
    {
        ...
        if ($data === false)
        {
            $sql = "SELECT c.*, COUNT(s.cat_id) AS has_children, COUNT(a.article_id) AS aricle_num ".
               ' FROM ' . $GLOBALS['yke']->table(1,'ykesmart_article_cat') . " AS c".
               " LEFT JOIN " . $GLOBALS['yke']->table(1,'ykesmart_article_cat') . " AS s ON s.parent_id=c.cat_id and c.sysid=s.sysid ".
               " LEFT JOIN " . $GLOBALS['yke']->table(1,'ykesmart_article') . " AS a ON a.cat_id=c.cat_id and a.sysid=c.sysid "." and c.sysid ='".$GLOBALS['sysid']."' ".
               " GROUP BY c.cat_id ".
...
function get_volume_price_list($goods_id, $price_type = '1')
{
    ...
    $sql = "SELECT `volume_number` , `volume_price`".
           " FROM " .$GLOBALS['yke']->table(1,'volume_price'). "".
           " WHERE `goods_id` = '" . $goods_id . "' AND `price_type` = '" . $price_type . "'"." and sysid ='".$GLOBALS['sysid']."' ".
...
function sort_goods_attr_id_array($goods_attr_id_array, $sort = 'asc')
{
    ...
    $sql = "SELECT a.attr_type, v.attr_value, v.goods_attr_id
            FROM " .$GLOBALS['yke']->table(1,'attribute'). " AS a
            LEFT JOIN " .$GLOBALS['yke']->table(1,'goods_attr'). " AS v
                ON v.attr_id = a.attr_id and a.sysid=v.sysid
                AND a.attr_type = 1
            WHERE v.goods_attr_id " . db_create_in($goods_attr_id_array) ." and a.sysid ='".$GLOBALS['sysid']."' ". "
...
function get_package_info($id)
{
    ...
    $sql = "SELECT act_id AS id,  act_name AS package_name, goods_id , goods_name, start_time, end_time, act_desc, ext_info".
           " FROM " . $GLOBALS['yke']->table(1,'goods_activity') .
           " WHERE act_id='$id' AND act_type = " . GAT_PACKAGE." and sysid ='".$GLOBALS['sysid']."' ";
    ...
    $sql = "SELECT pg.package_id, pg.goods_id, pg.goods_number, pg.admin_id, ".
           ...
           " FROM " . $GLOBALS['yke']->table(1,'package_goods') . " AS pg ".
           "   LEFT JOIN ". $GLOBALS['yke']->table(1,'goods') . " AS g ".
           "   ON g.goods_id = pg.goods_id and pg.sysid=g.sysid ".
           " LEFT JOIN " . $GLOBALS['yke']->table(1,'member_price') . " AS mp ".
                "ON mp.goods_id = g.goods_id and mp.sysid=g.sysid AND mp.user_rank = '$_SESSION[user_rank]' ".
           " WHERE pg.package_id = " . $id. " "." and pg.sysid ='".$GLOBALS['sysid']."' ".
...
function get_package_goods($package_id)
{
    $sql = "SELECT pg.goods_id, g.goods_name, pg.goods_number,g.goods_thumb, p.goods_attr, p.product_number, p.product_id
            FROM " . $GLOBALS['yke']->table(1,'package_goods') . " AS pg
                LEFT JOIN " .$GLOBALS['yke']->table(1,'goods') . " AS g ON pg.goods_id = g.goods_id and pg.sysid=g.sysid
                LEFT JOIN " . $GLOBALS['yke']->table(1,'products') . " AS p ON pg.product_id = p.product_id and pg.sysid=p.sysid
            WHERE pg.package_id = '$package_id'"." and pg.sysid ='".$GLOBALS['sysid']."' ";
    if ($package_id == 0)
    {
    ...
    if ($good_product_str != '')
    {
        $sql = "SELECT goods_attr_id, attr_value FROM " .$GLOBALS['yke']->table(1,'goods_attr'). " WHERE goods_id IN ($good_product_str)"." and sysid ='".$GLOBALS['sysid']."' ";
...
function get_good_products($goods_id, $conditions = '')
{
    ...
    /* 取货品 */
    $sql = "SELECT * FROM " .$GLOBALS['yke']->table(1,'products'). " WHERE $_goods_id $conditions"." and sysid ='".$GLOBALS['sysid']."' ";
    $result_products = $GLOBALS['db']->getAll($sql);

    /* 取商品属性 */
    $sql = "SELECT goods_attr_id, attr_value FROM " .$GLOBALS['yke']->table(1,'goods_attr'). " WHERE $_goods_id"." and sysid ='".$GLOBALS['sysid']."' ";
    $result_goods_attr = $GLOBALS['db']->getAll($sql);
...
function get_specifications_list($goods_id, $conditions = '')
{
    ...
    $sql = "SELECT ga.goods_attr_id, ga.attr_id, ga.attr_value, a.attr_name
            FROM " .$GLOBALS['yke']->table(1,'goods_attr'). " AS ga, " .$GLOBALS['yke']->table(1,'attribute'). " AS a
            WHERE ga.attr_id = a.attr_id and ga.sysid=a.sysid
            AND ga.goods_id = '$goods_id'
            $conditions"." and ga.sysid ='".$GLOBALS['sysid']."' ";
...
function get_province_city($provinceid = 0, $cityid = 0)
{
    $sql = 'SELECT region_name FROM ' . $GLOBALS['yke']->table(1,'region') .
            " WHERE region_id in (".$provinceid.",".$cityid.")"." and sysid ='".$GLOBALS['sysid']."' "." order by region_id";
...
function get_supplier_info($suppid){
	$sql = "select code,value,supplier_id from ". $GLOBALS['yke']->table(1,'supplier_shop_config'). " where supplier_id=".$suppid." and parent_id=8"." and sysid ='".$GLOBALS['sysid']."' ";
 	$result = $GLOBALS['db']->getAll($sql);
...
function pushUserMsg($ecuid,$msg=array(),$type=1){
	$weixinconfig = $GLOBALS['db']->getRow ( "SELECT * FROM " .$GLOBALS['yke']->table(1,'weixin_config'). "WHERE `sysid` = '".$GLOBALS['sysid']."'" );
	...
	$user = $GLOBALS['db']->getRow("select * from " .$GLOBALS['yke']->table(1,'weixin_user'). " where ecuid='{$ecuid}'"." and sysid ='".$GLOBALS['sysid']."' ");
	if($user && $user['fake_id']){
		...
		$createtime = time();
		$sql = "insert into " .$GLOBALS['yke']->table(1,'weixin_corn')." (`sysid`,`ecuid`,`content`,`createtime`,`sendtime`,`issend`,`sendtype`)
			value ("."'".$GLOBALS['sysid']."',"."{$ecuid},'{$content}','{$createtime}','{$sendtime}','0','{$type}')";
		$GLOBALS['db']->query($sql);
		return true;
	}else{
		return false;
...
/* 代码增加_start   By www.yikaiqiche.cn */
function get_city_info($province, $city, $district)
{
	$sql = 'select region_id from ' . $GLOBALS['yke']->table(1,'region') . " where region_name='$province'"." and sysid ='".$GLOBALS['sysid']."' ";
	$province_id = $GLOBALS['db']->getOne($sql);
	
	if($province_id > 0)
	{
		$sql = 'select region_id from ' . $GLOBALS['yke']->table(1,'region') .
				"where parent_id=$province_id and region_name='$city'"." and sysid ='".$GLOBALS['sysid']."' ";
		$city_id = $GLOBALS['db']->getOne($sql);
		if($city_id > 0)
		{
			$sql = 'select region_id from ' . $GLOBALS['yke']->table(1,'region') .
					"where parent_id=$city_id and region_name='$district'"." and sysid ='".$GLOBALS['sysid']."' ";
