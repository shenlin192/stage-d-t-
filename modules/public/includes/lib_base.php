...
{
   // echo $GLOBALS['_CFG']['mail_charset'];
   // echo EC_CHARSET;
    /* 如果邮件编码不是EC_CHARSET，创建字符集转换对象，转换编码 */
    if ($GLOBALS['_CFG']['mail_charset'] != EC_CHARSET)
    {
        $name      = yk_iconv(EC_CHARSET, $GLOBALS['_CFG']['mail_charset'], $name);
        $subject   = yk_iconv(EC_CHARSET, $GLOBALS['_CFG']['mail_charset'], $subject);
        $content   = yk_iconv(EC_CHARSET, $GLOBALS['_CFG']['mail_charset'], $content);
        $shop_name = yk_iconv(EC_CHARSET, $GLOBALS['_CFG']['mail_charset'], $GLOBALS['_CFG']['shop_name']);
    }
    if (empty($shop_name) || !$shop_name)
    {
        $shop_name='cheyikai';
    }
 ...
 * @return  void
 **/
function yk_header($string, $replace = true, $http_response_code = 0)
{
    //对$string进行处理，如果其中不包含?sysessid='.$GLOBALS['sysessid'].'\n则加上，如果存在两个？
...
function yk_iconv($source_lang, $target_lang, $source_string = '')
{
    static $chs = NULL;
...
function json_str_iconv($str)
{
    if (EC_CHARSET != 'utf-8')
    {
        if (is_string($str))
        {
            return addslashes(stripslashes(yk_iconv('utf-8', EC_CHARSET, $str)));
        }
        elseif (is_array($str))
        {
...
function to_utf8_iconv($str)
{
    if (EC_CHARSET != 'utf-8')
    {
        if (is_string($str))
        {
            return yk_iconv(EC_CHARSET, 'utf-8', $str);
        }
        elseif (is_array($str))
        {
            foreach ($str as $key => $value)
