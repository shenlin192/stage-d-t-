...
function insert_query_info()
{
    ...
    $online_count = $GLOBALS['db']->getOne("SELECT COUNT(*) FROM " . $GLOBALS['yke']->table(1,'sessions')." where sysid ='".$GLOBALS['sysid']."' ");

    /* 加入触发cron代码 */
    $cron_method = empty($GLOBALS['_CFG']['cron_method']) ? '<img src="api/cron.php?sysessid='.$GLOBALS['sysessid'].'&t=' . gmtime() . '" alt="" style="width:0px;height:0px;" />' : '';
...
function insert_history()
{
    ...
    if (!empty($_COOKIE['YKE']['history']))
    {
        ...
        $sql   = 'SELECT goods_id, goods_name, goods_thumb, shop_price FROM ' . $GLOBALS['yke']->table(1,'goods') .
                " WHERE $where AND is_on_sale = 1 AND is_alone_sale = 1 AND is_delete = 0"." and sysid ='".$GLOBALS['sysid']."' ";
...
{
$where = db_create_in($_COOKIE['YKE']['history'], 'goods_id');
        $sql   = 'SELECT cat_id FROM ' . $GLOBALS['yke']->table(1,'goods') .
                " WHERE $where AND is_on_sale = 1 AND is_alone_sale = 1 AND is_delete = 0 "." and sysid ='".$GLOBALS['sysid']."' "." limit 1";
...
$sql = "select * from ".$GLOBALS['yke']->table(1,'goods')." where cat_id = ".$catid." and is_best = 1 "." and sysid ='".$GLOBALS['sysid']."' ";
...
{
$sql = "SELECT count(*) FROM " . $GLOBALS['yke']->table(1,'comment') . " WHERE status=1 and  comment_type =0 and id_value =".$goods_id ." and sysid ='".$GLOBALS['sysid']."' ";//status=1表示通过了的评论才算  comment_type =0表示针对商品的评价 感谢zhangyh的提醒
...
function insert_history_list()
{
    ...
    {
        $where = db_create_in($_COOKIE['YKE']['history'], 'goods_id');
        $sql   = 'SELECT goods_id, goods_name, goods_thumb, shop_price, goods_type FROM ' . $GLOBALS['yke']->table(1,'goods') .
                " WHERE $where AND is_on_sale = 1 AND is_alone_sale = 1 AND is_delete = 0"." and sysid ='".$GLOBALS['sysid']."' ";
        $query = $GLOBALS['db']->query($sql);
        $res = array();
		$str .= '<ul>';
...
function insert_cart_info()
{
    ...
    $sql = 'SELECT c.*,g.goods_name,g.goods_thumb,g.goods_id,c.goods_number,c.goods_price' .
           ...
           " WHERE $sql_where AND rec_type = '" . CART_GENERAL_GOODS . "'"." and c.sysid ='".$GLOBALS['sysid']."' ";
    ...
    $sql = 'SELECT SUM(goods_number) AS number, SUM(goods_price * goods_number) AS amount' .
           ' FROM ' . $GLOBALS['yke']->table(1,'cart') ." AS c ".
           " WHERE $sql_where AND rec_type = '" . CART_GENERAL_GOODS . "'"." and sysid ='".$GLOBALS['sysid']."' ";
 ...
 * @return type
 */
function insert_ykesmart_tel(){
    $sql = "select value from ".$GLOBALS['yke']->table(1,'ykesmart_shop_config')." where code = 'service_phone'"." and sysid ='".$GLOBALS['sysid']."' ";
    $tel = $GLOBALS['db']->getOne($sql);
    return $tel;
}
/*杭州一开 新增实时获取客户分享ID功能*/
function insert_fenxiangid(){
...
function insert_ads($arr)
{
    ...
    if (!empty($arr['num']) && $arr['num'] != 1)
    {
        $sql  = 'SELECT a.ad_id, a.position_id, a.media_type, a.ad_link, a.ad_code, a.ad_name, p.ad_width, ' .
                ...
                'FROM ' . $GLOBALS['yke']->table(1,'ykesmart_ad') . ' AS a '.
                'LEFT JOIN ' . $GLOBALS['yke']->table(1,'ykesmart_ad_position') . ' AS p ON a.position_id = p.position_id and a.sysid=p.sysid ' .
                "WHERE enabled = 1 AND start_time <= '" . $time . "' AND end_time >= '" . $time . "' ".
                    "AND a.position_id = '" . $arr['id'] . "' "." and a.sysid ='".$GLOBALS['sysid']."' " .
        ...
        if ($static_res[$arr['id']] === NULL)
        {
            $sql  = 'SELECT a.ad_id, a.position_id, a.media_type, a.ad_link, a.ad_code, a.ad_name, p.ad_width, '.
                    ...
                    "WHERE enabled = 1 AND a.position_id = '" . $arr['id'] .
                        "' AND start_time <= '" . $time . "' AND end_time >= '" . $time . "' " ." and a.sysid ='".$GLOBALS['sysid']."' ".
    ...
    foreach ($res AS $row)
    {
        ...
        switch ($row['media_type'])
        {
            case 0: // 图片广告
                ...
                $href='affiche.php?sysessid='.$GLOBALS['sysessid'].'&ad_id=$row[ad_id]&amp;uri=" .urlencode($row["ad_link"])';
                $ads[] = "<a href='".$href. "'
...
function insert_member_info()
{
    ...
    {
        if (!empty($_COOKIE['YKE']['username']))
        {
            $GLOBALS['smarty']->assign('yke_username', stripslashes($_COOKIE['YKE']['username']));
        }
...
function insert_bought_notes($arr)
{
    ...
    $sql = 'SELECT u.user_name, og.goods_number, oi.add_time, IF(oi.order_status IN (2, 3, 4), 0, 1) AS order_status ' .
           'FROM ' . $GLOBALS['yke']->table(1,'order_info') . ' AS oi LEFT JOIN ' . $GLOBALS['yke']->table(1,'users') . ' AS u ON oi.user_id = u.user_id and oi.sysid=u.sysid, ' . $GLOBALS['yke']->table(1,'order_goods') . ' AS og ' .
           'WHERE oi.order_id = og.order_id and oi.sysid = og.sysid AND ' . time() . ' - oi.add_time < 2592000 AND og.goods_id = ' . $arr['id'] ." and oi.sysid ='".$GLOBALS['sysid']."' ". ' ORDER BY oi.add_time DESC LIMIT 5';
    ...
    $sql = 'SELECT count(*) ' .
           'FROM ' . $GLOBALS['yke']->table(1,'order_info') . ' AS oi LEFT JOIN ' . $GLOBALS['yke']->table(1,'users') . ' AS u ON oi.user_id = u.user_id and oi.sysid = u.sysid, ' . $GLOBALS['yke']->table(1,'order_goods') . ' AS og ' .
           'WHERE oi.order_id = og.order_id and oi.sysid = og.sysid AND ' . time() . ' - oi.add_time < 2592000 AND og.goods_id = ' . $arr['id']." and oi.sysid ='".$GLOBALS['sysid']."' ";
...
function insert_apply_supplier($user){
    ...
    $sql = "select * from ".$GLOBALS['yke']->table(1,'supplier')." where user_id = ".$user['id']." and sysid ='".$GLOBALS['sysid']."' ".' limit 1';
    ...
    if($shownum == 1){
    	...
    	$sql="select rank_id,rank_name from ". $GLOBALS['yke']->table(1,'supplier_rank') ." where sysid ='".$GLOBALS['sysid']."' "." order by sort_order";
		$supplier_rank=$GLOBALS['db']->getAll($sql);
		$GLOBALS['smarty']->assign('supplier_rank', $supplier_rank);
		$sql="select str_id,str_name from ". $GLOBALS['yke']->table(1,'street_category') ." where is_show=1"." and sysid ='".$GLOBALS['sysid']."' "." order by sort_order";
		...
		$GLOBALS['smarty']->assign('supplier_country', $supplier_country);
    	/* 供货商等级 */
		$sql="select rank_name from ". $GLOBALS['yke']->table(1,'supplier_rank') ." where rank_id = ".$supplier['rank_id']." and sysid ='".$GLOBALS['sysid']."' ";
		$rank_name=$GLOBALS['db']->getOne($sql);
		$supplier['rank_name'] = $rank_name;
		/* 店铺类型 */
		 $sql="select str_name from ". $GLOBALS['yke']->table(1,'street_category') ." where str_id = ".$supplier['type_id']." and sysid ='".$GLOBALS['sysid']."' ";
		$type_name=$GLOBALS['db']->getOne($sql);
		$supplier['type_name'] = $type_name;
		
		$GLOBALS['smarty']->assign('mydomain', $GLOBALS['yke']->url());
    }
...
function insert_goods_sells($arr)
{
    $sql = 'SELECT SUM(goods_number) AS number ' .
           ' FROM ' . $GLOBALS['yke']->table(1,'order_goods') ." AS og , " . $GLOBALS['yke']->table(1,'order_info') ." AS  o ".
           " WHERE og.order_id = o.order_id and og.sysid = o.sysid and og.goods_id=".$arr['goods_id']." and og.sysid ='".$GLOBALS['sysid']."' ";
    $row = $GLOBALS['db']->GetRow($sql);
    if ($row)
    {
        $number = intval($row['number']);
...
function insert_supplier_list(){
	...
	$sql = "SELECT ssc.value,ssc.supplier_id,ssc.code FROM ".$GLOBALS['yke']->table(1,'supplier')." as s,".$GLOBALS['yke']->table(1,'supplier_shop_config')." as ssc WHERE s.sysid=ssc.sysid and s.`status` = 1 AND ssc.supplier_id = s.supplier_id AND code in('shop_name','shop_logo')"." and s.sysid ='".$GLOBALS['sysid']."' "." limit 36";
    $query = $GLOBALS['db']->query($sql);
    $ret = array();
    while ($row = $GLOBALS['db']->fetch_array($query)){
    	$row['value'] = empty($row['value']) ? '/data/supplier/dianpu.jpg' : $row['value'];
    	$ret[$row['supplier_id']][$row['code']] = $row['value'];
    	$ret[$row['supplier_id']]['shop_url'] = 'supplier.php?sysessid='.$GLOBALS['sysessid'].'&suppId='.$row['supplier_id'];
    }
    $num = 24 - count($ret);
    if($num > 0){
    	for($i=0;$i<$num;$i++){
...
function insert_get_shop_shipping($arr){
	...
	if(count($shipping_list)>0){
		...
		$sql = 'SELECT count(*) FROM ' . $yke->table(1,'cart') . " WHERE $sql_where AND `extension_code` != 'package_buy' AND `is_shipping` = 0 AND rec_id in (".$_SESSION['sel_cartgoods'].")"." and sysid ='".$GLOBALS['sysid']."' "; //jx
...
function get_children_tree($cat_id)
{
     if ($cat_id >0 )
    {
        $sql = 'SELECT count(*) FROM ' . $GLOBALS['yke']->table(1,'category') . " WHERE parent_id = '$cat_id'"." and sysid ='".$GLOBALS['sysid']."' ";
        ...
        if ($GLOBALS['db']->getOne($sql))
        {
            ...
            $sql = 'SELECT a.cat_id, a.cat_name, a.sort_order AS parent_order, a.cat_id, ' .
                ...
                'FROM ' . $GLOBALS['yke']->table(1,'category') . ' AS a ' .
                'LEFT JOIN ' . $GLOBALS['yke']->table(1,'category') . ' AS b ON b.parent_id = a.cat_id and a.sysid=b.sysid ' .
                "WHERE a.cat_id = '$cat_id'"." and a.sysid ='".$GLOBALS['sysid']."' "." ORDER BY parent_order ASC, a.cat_id ASC, child_order ASC";
        }        
        else
        {
            ...
            if ($parent_id > 0)
            {
                ...
                $sql = 'SELECT a.cat_id, a.cat_name, b.cat_id AS child_id, b.cat_name AS child_name, b.sort_order ' .
                    'FROM ' . $GLOBALS['yke']->table(1,'category') . ' AS a ' .
                    'LEFT JOIN ' . $GLOBALS['yke']->table(1,'category') . ' AS b ON b.parent_id = a.cat_id and b.sysid = a.sysid ' .
                    "WHERE b.parent_id = '$parent_id' "." and sysid ='".$GLOBALS['sysid']."' "."ORDER BY sort_order ASC";
            }
            else
            {
                //获取当前分类
                $sql = 'SELECT a.cat_id, a.cat_name FROM '
                        . $GLOBALS['yke']->table(1,'category') . ' AS a ' .
                        "WHERE a.cat_id = '$cat_id'"." and sysid ='".$GLOBALS['sysid']."' ";
            }
        }
