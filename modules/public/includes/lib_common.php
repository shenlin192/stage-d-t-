...
{
    /* 获得配置信息 */
    $sql = 'SELECT configure FROM ' . $GLOBALS['yke']->table(1,'shipping_area') . " WHERE shipping_area_id = '$area_id'";
    ...
    if ($res === NULL)
    {
        ...
        if ($data === false)
        {
            $sql = "SELECT c.cat_id, c.cat_name, c.measure_unit, c.parent_id, c.is_show, c.show_in_nav, c.grade, c.sort_order, COUNT(s.cat_id) AS has_children ".
                ...
                "LEFT JOIN " . $GLOBALS['yke']->table(1,'category') . " AS s ON s.parent_id=c.cat_id ".
...
function load_config()
{
    ...
    if ($data === false)
    {
        $sql = 'SELECT code, value FROM ' . $GLOBALS['yke']->table(1,'ykesmart_shop_config') . ' WHERE parent_id > 0';
		//echo $sql; 
        $res = $GLOBALS['db']->getAll($sql);
...
{
    $sql = 'SELECT brand_id, brand_name FROM ' . $GLOBALS['yke']->table(1,'brand') . ' ORDER BY sort_order';
    $res = $GLOBALS['db']->getAll($sql);
...
function get_promotion_info($goods_id = '',$suppid=-1)
{
    ...
    $sql = 'SELECT act_id, act_name, act_type, start_time, end_time FROM ' . $GLOBALS['yke']->table(1,'goods_activity') . " WHERE is_finished=0 AND start_time <= '$gmtime' AND end_time >= '$gmtime'";
    if(!empty($goods_id))
    {
    ...
    $favourable = array();
    $sql = 'SELECT act_id, act_range, act_range_ext, act_name, start_time, end_time FROM ' . $GLOBALS['yke']->table(1,'favourable_activity') . " WHERE start_time <= '$gmtime' AND end_time >= '$gmtime' $where_suppid";
    if(!empty($goods_id))
    {
...
{
    $sql = 'SELECT template_subject, is_html, template_content FROM ' . $GLOBALS['yke']->table(1,'mail_templates') . " WHERE template_code = '$tpl_name'";
...
function order_action($order_sn, $order_status, $shipping_status, $pay_status, $note = '', $username = null, $place = 0)
{
    ...
    $sql = 'INSERT INTO ' . $GLOBALS['yke']->table(1,'order_action') .
            ...
            'FROM ' . $GLOBALS['yke']->table(1,'order_info') . " WHERE order_sn = '$order_sn'";
...
function virtual_card_shipping ($goods, $order_sn, &$msg, $process = 'other')
{
    ...
    /* 检查有没有缺货 */
    $sql = "SELECT COUNT(*) FROM ".$GLOBALS['yke']->table(1,'virtual_card')." WHERE goods_id = '$goods[goods_id]' AND is_saled = 0 ";
    ...
    if ($num < $goods['num'])
    {
     ...
     /* 取出卡片信息 */
     $sql = "SELECT card_id, card_sn, card_password, end_date, crc32 FROM ".$GLOBALS['yke']->table(1,'virtual_card')." WHERE goods_id = '$goods[goods_id]' AND is_saled = 0  LIMIT " . $goods['num'];
    ...
    /* 更新库存 */
    $sql = "UPDATE ".$GLOBALS['yke']->table(1,'goods'). " SET goods_number = goods_number - '$goods[num]' WHERE goods_id = '$goods[goods_id]'";
    $GLOBALS['db']->query($sql);

    if (true)
    {
        /* 获取订单信息 */
        $sql = "SELECT order_id, order_sn, consignee, email FROM ".$GLOBALS['yke']->table(1,'order_info'). " WHERE order_sn = '$order_sn'";
...
function virtual_card_result($order_sn, $goods)
{
    ...
    /* 获取已经发送的卡片数据 */
    $sql = "SELECT card_sn, card_password, end_date, crc32 FROM ".$GLOBALS['yke']->table(1,'virtual_card')." WHERE goods_id= '$goods[goods_id]' AND order_sn = '$order_sn' ";
    ...
    if ($res === NULL)
    {
        ...
        if ($data === false)
        {
            $sql = "SELECT c.*, COUNT(s.cat_id) AS has_children, COUNT(a.article_id) AS aricle_num ".
               ...
               " LEFT JOIN " . $GLOBALS['yke']->table(1,'ykesmart_article_cat') . " AS s ON s.parent_id=c.cat_id".
               " LEFT JOIN " . $GLOBALS['yke']->table(1,'ykesmart_article') . " AS a ON a.cat_id=c.cat_id".
               " GROUP BY c.cat_id ".
...
function get_package_goods($package_id)
{
    $sql = "SELECT pg.goods_id, g.goods_name, pg.goods_number,g.goods_thumb, p.goods_attr, p.product_number, p.product_id
            FROM " . $GLOBALS['yke']->table(1,'package_goods') . " AS pg
                LEFT JOIN " .$GLOBALS['yke']->table(1,'goods') . " AS g ON pg.goods_id = g.goods_id
                LEFT JOIN " . $GLOBALS['yke']->table(1,'products') . " AS p ON pg.product_id = p.product_id
    ...
    if ($good_product_str != '')
    {
        $sql = "SELECT goods_attr_id, attr_value FROM " .$GLOBALS['yke']->table(1,'goods_attr'). " WHERE goods_id IN ($good_product_str)";
...
function get_good_products($goods_id, $conditions = '')
{
    ...
    /* 取货品 */
    $sql = "SELECT * FROM " .$GLOBALS['yke']->table(1,'products'). " WHERE $_goods_id $conditions";
    $result_products = $GLOBALS['db']->getAll($sql);

    /* 取商品属性 */
    $sql = "SELECT goods_attr_id, attr_value FROM " .$GLOBALS['yke']->table(1,'goods_attr'). " WHERE $_goods_id";
    $result_goods_attr = $GLOBALS['db']->getAll($sql);
...
function get_supplier_info($suppid){
	$sql = "select code,value,supplier_id from ". $GLOBALS['yke']->table(1,'supplier_shop_config'). " where supplier_id=".$suppid." and parent_id=8";
 	$result = $GLOBALS['db']->getAll($sql);
...
function pushUserMsg($ecuid,$msg=array(),$type=1){
	...
	$user = $GLOBALS['db']->getRow("select * from " .$GLOBALS['yke']->table(1,'weixin_user'). " where ecuid='{$ecuid}'");
	if($user && $user['fake_id']){
		...
		$createtime = time();
		$sql = "insert into " .$GLOBALS['yke']->table(1,'weixin_corn')." (`ecuid`,`content`,`createtime`,`sendtime`,`issend`,`sendtype`) 
...
{
	$sql = 'select region_id from ' . $GLOBALS['yke']->table(1,'region') . " where region_name='$province'";
