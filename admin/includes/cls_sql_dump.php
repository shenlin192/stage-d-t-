...
class cls_sql_dump
{
    ...
    function get_table_data($table, $pos)
    {
        ...
        /* 获取数据表记录总数 */
        $total = $this->db->getOne("SELECT COUNT(*) FROM $table"." where sysid ='".$GLOBALS['sysid']."' ");
        ...
        {
            /* 获取数据库数据 */
            $data = $this->db->getAll("SELECT * FROM $table "." and sysid ='".$GLOBALS['sysid']."' "." LIMIT " . ($this->offset * $i + $pos) . ', ' . $this->offset);
    ...
    function dump_table($path, $vol)
    {
        ...
        $this->dump_sql .= '-- END ykshop v2.x SQL Dump Program ';
    ...
    {
        /* 获取sql文件头部信息 */
        $sql_info = array('date'=>'', 'mysql_ver'=> '', 'php_ver'=>0, 'yke_ver'=>'', 'vol'=>0);
        $fp = fopen($path,'rb');
        $str = fread($fp, 250);
        fclose($fp);
        $arr = explode("\n", $str);
        ...
        foreach ($arr AS $val)
        {
            ...
            if ($pos > 0)
            {
                ...
                elseif ($type == 'ykshop VERSION')
                {
                    $sql_info['yke_ver'] = $value;
                }
                elseif ($type == 'Vol')
                {
                    $sql_info['vol'] = $value;
