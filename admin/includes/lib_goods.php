...
{
    $sql = "SELECT * FROM " . $GLOBALS['yke']->table(1,'user_rank') ." where sysid ='".$GLOBALS['sysid']."' ".
        " ORDER BY min_points";
...
function get_member_price_list($goods_id)
{
    ...
    $sql = "SELECT user_rank, user_price FROM " .
           $GLOBALS['yke']->table(1,'member_price') .
...
function handle_goods_attr($goods_id, $id_list, $is_spec_list, $value_price_list)
{
    ...
    foreach ($id_list AS $key => $id)
    {
        ...
        // 插入或更新记录
        $sql = "SELECT goods_attr_id FROM " . $GLOBALS['yke']->table(1,'goods_attr') . " WHERE goods_id = '$goods_id' and sysid ='".$GLOBALS['sysid']."' ". "   AND attr_id = '$id' AND attr_value = '$value' LIMIT 0, 1";
        $result_id = $GLOBALS['db']->getOne($sql);
        if (!empty($result_id))
        {
            $sql = "UPDATE " . $GLOBALS['yke']->table(1,'goods_attr') . "
                    SET attr_value = '$value'
                    WHERE goods_id = '$goods_id'
                    AND attr_id = '$id'
                    AND goods_attr_id = '$result_id' and sysid ='".$GLOBALS['sysid']."' "    ;

            $goods_attr_id[$id] = $result_id;
        }
        else
        {
            $sql = "INSERT INTO " . $GLOBALS['yke']->table(1,'goods_attr') . " (sysid,goods_id, attr_id, attr_value, attr_price) " .
                    "VALUES ('".$GLOBALS['sysid']."',"."'$goods_id', '$id', '$value', '$price')";
        }
...
function handle_member_price($goods_id, $rank_list, $price_list)
{
    ...
    foreach ($rank_list AS $key => $rank)
    {
        ...
        // 插入或更新记录
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['yke']->table(1,'member_price') .
               " WHERE goods_id = '$goods_id' AND user_rank = '$rank' and sysid ='".$GLOBALS['sysid']."' "
;
        if ($GLOBALS['db']->getOne($sql) > 0)
        {
            ...
            if ($price < 0)
            {
                $sql = "DELETE FROM " . $GLOBALS['yke']->table(1,'member_price') .
                       " WHERE goods_id = '$goods_id' and sysid ='".$GLOBALS['sysid']."'  AND user_rank = '$rank' LIMIT 1";
            }
            else
            {
                $sql = "UPDATE " . $GLOBALS['yke']->table(1,'member_price') .
        ...
        else
        {
            ...
            {
                $sql = "INSERT INTO " . $GLOBALS['yke']->table(1,'member_price') . " (sysid,goods_id, user_rank, user_price) " .
                       "VALUES ('".$GLOBALS['sysid']."',"."'$goods_id', '$rank', '$price')";
            }
        }
...
{
    /* 查询现有的扩展分类 */
    $sql = "SELECT cat_id FROM " . $GLOBALS['yke']->table(1,'goods_cat') .
            " WHERE goods_id = '$goods_id'    and sysid ='".$GLOBALS['sysid']."' "    ;
    ...
    if ($delete_list)
    {
        $sql = "DELETE FROM " . $GLOBALS['yke']->table(1,'goods_cat') .
                " WHERE goods_id = '$goods_id' " .
                "AND cat_id " . db_create_in($delete_list)  ." and sysid ='".$GLOBALS['sysid']."' ";
        $GLOBALS['db']->query($sql);
    }
    ...
    {
        // 插入记录
        $sql = "INSERT INTO " . $GLOBALS['yke']->table(1,'goods_cat') .
                " (sysid,goods_id, cat_id) " .
                "VALUES ('".$GLOBALS['sysid']."',"."'$goods_id', '$cat_id')";
        $GLOBALS['db']->query($sql);
    }
}
...
{
    $sql = "UPDATE " . $GLOBALS['yke']->table(1,'link_goods') . " SET " .
            " goods_id = '$goods_id' " .
            " WHERE goods_id = '0'" .
            " AND admin_id = '$_SESSION[admin_id]' and sysid ='".$GLOBALS['sysid']."' "    ;
    $GLOBALS['db']->query($sql);
...
{
    $sql = "UPDATE " . $GLOBALS['yke']->table(1,'group_goods') . " SET " .
            " parent_id = '$goods_id' " .
            " WHERE parent_id = '0'" .
            " AND admin_id = '$_SESSION[admin_id]'  and sysid ='".$GLOBALS['sysid']."' "  ;
    $GLOBALS['db']->query($sql);
...
{
    $sql = "UPDATE " . $GLOBALS['yke']->table(1,'goods_article') . " SET " .
            " goods_id = '$goods_id' " .
            " WHERE goods_id = '0'" .
            " AND admin_id = '$_SESSION[admin_id]'  and sysid ='".$GLOBALS['sysid']."' "
;
...
function handle_gallery_image($goods_id, $image_files, $image_descs, $image_urls)
{
    ...
    foreach ($image_descs AS $key => $img_desc)
    {
        ...
        if ($flag)
        {
            ...
            $sql = "INSERT INTO " . $GLOBALS['yke']->table(1,'goods_gallery') . " (sysid,goods_id, img_url, img_desc, thumb_url, img_original) " .
                    "VALUES ('".$GLOBALS['sysid']."',"."'$goods_id', '$img_url', '$img_desc', '$thumb_url', '$img_original')";
            $GLOBALS['db']->query($sql);
            ...
            {
                $GLOBALS['db']->query("UPDATE " . $GLOBALS['yke']->table(1,'goods_gallery') . " SET img_original='' WHERE `goods_id`='{$goods_id}'  and sysid ='".$GLOBALS['sysid']."' "    );
                @unlink('../' . $img_original);
            }
        }
...
function update_goods($goods_id, $field, $value)
{
    if ($goods_id)
    {
        ...
        $sql = "UPDATE " . $GLOBALS['yke']->table(1,'goods') .
                ...
                "WHERE goods_id " . db_create_in($goods_id)    ."  and sysid ='".$GLOBALS['sysid']."' " ;
...
function delete_goods($goods_id)
{
    ...
    /* 取得有效商品id */
    $sql = "SELECT DISTINCT goods_id FROM " . $GLOBALS['yke']->table(1,'goods') .
            " WHERE goods_id " . db_create_in($goods_id) . " AND is_delete = 1     and sysid ='".$GLOBALS['sysid']."' "
;
...
;
    ...
    /* 删除商品 */
    $sql = "DELETE FROM " . $GLOBALS['yke']->table(1,'goods') .
            " WHERE goods_id " . db_create_in($goods_id) ." and sysid ='".$GLOBALS['sysid']."' "
;
    $GLOBALS['db']->query($sql);

    /* 删除商品的货品记录 */
    $sql = "DELETE FROM " . $GLOBALS['yke']->table(1,'products') .
            " WHERE goods_id " . db_create_in($goods_id)   ." and sysid ='".$GLOBALS['sysid']."' "
;
    $GLOBALS['db']->query($sql);
...
;
    ...
    /* 删除商品相册 */
    $sql = "DELETE FROM " . $GLOBALS['yke']->table(1,'goods_gallery') . " WHERE goods_id " . db_create_in($goods_id)    ." and sysid ='".$GLOBALS['sysid']."' "
;
    $GLOBALS['db']->query($sql);

    /* 删除相关表记录 */
    $sql = "DELETE FROM " . $GLOBALS['yke']->table(1,'collect_goods') . " WHERE goods_id " . db_create_in($goods_id)    ." and sysid ='".$GLOBALS['sysid']."' "    ;
    $GLOBALS['db']->query($sql);
    $sql = "DELETE FROM " . $GLOBALS['yke']->table(1,'goods_article') . " WHERE goods_id " . db_create_in($goods_id)   ." and sysid ='".$GLOBALS['sysid']."' "   ;
    $GLOBALS['db']->query($sql);
    $sql = "DELETE FROM " . $GLOBALS['yke']->table(1,'goods_attr') . " WHERE goods_id " . db_create_in($goods_id)   ." and sysid ='".$GLOBALS['sysid']."' "   ;
    $GLOBALS['db']->query($sql);
    $sql = "DELETE FROM " . $GLOBALS['yke']->table(1,'goods_cat') . " WHERE goods_id " . db_create_in($goods_id)   ." and sysid ='".$GLOBALS['sysid']."' "   ;
    $GLOBALS['db']->query($sql);
    $sql = "DELETE FROM " . $GLOBALS['yke']->table(1,'member_price') . " WHERE goods_id " . db_create_in($goods_id)   ." and sysid ='".$GLOBALS['sysid']."' "   ;
    $GLOBALS['db']->query($sql);
    $sql = "DELETE FROM " . $GLOBALS['yke']->table(1,'group_goods') . " WHERE parent_id " . db_create_in($goods_id)   ." and sysid ='".$GLOBALS['sysid']."' "   ;
    $GLOBALS['db']->query($sql);
    $sql = "DELETE FROM " . $GLOBALS['yke']->table(1,'group_goods') . " WHERE goods_id " . db_create_in($goods_id)   ." and sysid ='".$GLOBALS['sysid']."' "   ;
    $GLOBALS['db']->query($sql);
    $sql = "DELETE FROM " . $GLOBALS['yke']->table(1,'link_goods') . " WHERE goods_id " . db_create_in($goods_id)   ." and sysid ='".$GLOBALS['sysid']."' "   ;
    $GLOBALS['db']->query($sql);
    $sql = "DELETE FROM " . $GLOBALS['yke']->table(1,'link_goods') . " WHERE link_goods_id " . db_create_in($goods_id)   ." and sysid ='".$GLOBALS['sysid']."' "   ;
    $GLOBALS['db']->query($sql);
    $sql = "DELETE FROM " . $GLOBALS['yke']->table(1,'tag') . " WHERE goods_id " . db_create_in($goods_id)." and sysid ='".$GLOBALS['sysid']."' ";
    $GLOBALS['db']->query($sql);
    $sql = "DELETE FROM " . $GLOBALS['yke']->table(1,'comment') . " WHERE comment_type = 0 AND id_value " . db_create_in($goods_id)   ." and sysid ='".$GLOBALS['sysid']."' "   ;
    $GLOBALS['db']->query($sql);

    /* 删除相应虚拟商品记录 */
    $sql = "DELETE FROM " . $GLOBALS['yke']->table(1,'virtual_card') . " WHERE goods_id " . db_create_in($goods_id)   ." and sysid ='".$GLOBALS['sysid']."' "   ;
...
function generate_goods_sn($goods_id)
{
    ...
    $sql = "SELECT goods_sn FROM " . $GLOBALS['yke']->table(1,'goods') .
            " WHERE goods_sn LIKE '" . mysql_like_quote($goods_sn) . "%' AND goods_id <> '$goods_id' "    ." and sysid ='".$GLOBALS['sysid']."' "   .
...
function check_goods_sn_exist($goods_sn, $goods_id = 0)
{
    ...
    if (empty($goods_id))
    {
        $sql = "SELECT goods_id FROM " . $GLOBALS['yke']->table(1,'goods') ."
                WHERE goods_sn = '$goods_sn'"   ." and sysid ='".$GLOBALS['sysid']."' "   ;
    }
    else
    {
        $sql = "SELECT goods_id FROM " . $GLOBALS['yke']->table(1,'goods') ."
                WHERE goods_sn = '$goods_sn'
                AND goods_id <> '$goods_id'"   ." and sysid ='".$GLOBALS['sysid']."' "  ;
    }
...
function get_attr_list($cat_id, $goods_id = 0)
{
    ...
    $sql = "SELECT a.attr_id, a.attr_name, a.attr_input_type, a.attr_type, a.attr_values, v.attr_value, v.attr_price ".
            ...
            "ON v.attr_id = a.attr_id AND  v.sysid = a.sysid AND v.goods_id = '$goods_id' ".
            "WHERE a.cat_id = " . intval($cat_id) ." OR a.cat_id = 0 "   ." and a.sysid ='".$GLOBALS['sysid']."' "   .
...
function get_goods_type_specifications()
{
    ...
    $sql = "SELECT DISTINCT cat_id
            FROM " .$GLOBALS['yke']->table(1,'attribute'). "
            WHERE attr_type = 1"." and sysid ='".$GLOBALS['sysid']."' " ;
    $row = $GLOBALS['db']->GetAll($sql);
...
function get_linked_goods($goods_id)
{
    $sql = "SELECT lg.link_goods_id AS goods_id, g.goods_name, lg.is_double " .
            "FROM " . $GLOBALS['yke']->table(1,'link_goods') . " AS lg, " .
                $GLOBALS['yke']->table(1,'goods') . " AS g " .
            "WHERE lg.goods_id = '$goods_id' " .
            "AND lg.link_goods_id = g.goods_id AND lg.sysid = g.sysid "." and lg.sysid ='".$GLOBALS['sysid']."' " ;
    if ($goods_id == 0)
    {
...
function get_group_goods($goods_id)
{
    $sql = "SELECT gg.goods_id, CONCAT(g.goods_name, ' -- [', gg.goods_price, ']') AS goods_name " .
            ...
            "AND gg.goods_id = g.goods_id AND gg.sysid = g.sysid "." and gg.sysid ='".$GLOBALS['sysid']."' ";
    if ($goods_id == 0)
    {
...
function get_goods_articles($goods_id)
{
    $sql = "SELECT g.article_id, a.title " .
            "FROM " .$GLOBALS['yke']->table(1,'goods_article') . " AS g, " .
                $GLOBALS['yke']->table(1,'article') . " AS a " .
            "WHERE g.goods_id = '$goods_id' " .
            "AND g.article_id = a.article_id AND g.sysid = a.sysid " ." and g.sysid ='".$GLOBALS['sysid']."' ";
    if ($goods_id == 0)
    {
...
function goods_list($is_delete, $real_goods=1, $conditions = '')
{
    ...
    if ($result === false)
    {
        ...
        /* 记录总数 */
        $sql = "SELECT COUNT(*) FROM " .$GLOBALS['yke']->table(1,'goods'). " AS g WHERE is_delete='$is_delete' $where"  ." and g.sysid ='".$GLOBALS['sysid']."' ";
        ...
        $sql = "SELECT goods_id, goods_name, goods_type, goods_sn, shop_price, is_on_sale, is_best, is_new, is_hot, sort_order, goods_number, integral, " .
                    ...
                    " FROM " . $GLOBALS['yke']->table(1,'goods') . " AS g WHERE is_delete='$is_delete' $where" ." and g.sysid ='".$GLOBALS['sysid']."' " .
...
function check_goods_product_exist($goods_id, $conditions = '')
{
    ...
    $sql = "SELECT goods_id
            FROM " . $GLOBALS['yke']->table(1,'products') . "
            WHERE goods_id = '$goods_id'" ." and sysid ='".$GLOBALS['sysid']."' ".
            "" . $conditions . "
            LIMIT 0, 1";
...
function product_number_count($goods_id, $conditions = '')
{
    ...
    $sql = "SELECT SUM(product_number)
            FROM " . $GLOBALS['yke']->table(1,'products') . "
            WHERE goods_id = '$goods_id'
            " . $conditions." and sysid ='".$GLOBALS['sysid']."' ";
...
function product_goods_attr_list($goods_id)
{
    ...
    $sql = "SELECT goods_attr_id, attr_value FROM " . $GLOBALS['yke']->table(1,'goods_attr') . " WHERE goods_id = '$goods_id'"." and sysid ='".$GLOBALS['sysid']."' ";
...
function get_goods_specifications_list($goods_id)
{
    ...
    $sql = "SELECT g.goods_attr_id, g.attr_value, g.attr_id, a.attr_name
            FROM " . $GLOBALS['yke']->table(1,'goods_attr') . " AS g
                LEFT JOIN " . $GLOBALS['yke']->table(1,'attribute') . " AS a
                    ON a.attr_id = g.attr_id and a.sysid = g.sysid
            WHERE goods_id = '$goods_id'
            AND a.attr_type = 1 "." and g.sysid ='".$GLOBALS['sysid']."' "."
...
function product_list($goods_id, $conditions = '')
{
    ...
    if ($result === false)
    {
        ...
        /* 记录总数 */
        $sql = "SELECT COUNT(*) FROM " .$GLOBALS['yke']->table(1,'products'). " AS p WHERE goods_id = $goods_id $where"." and sysid ='".$GLOBALS['sysid']."' ";
        $filter['record_count'] = $GLOBALS['db']->getOne($sql);

        $sql = "SELECT product_id, goods_id, goods_attr, product_sn, product_number
                FROM " . $GLOBALS['yke']->table(1,'products') . " AS g
                WHERE goods_id = $goods_id $where "." and g.sysid ='".$GLOBALS['sysid']."' "."
                ORDER BY $filter[sort_by] $filter[sort_order]";
...
function get_product_info($product_id, $filed = '')
{
    ...
    $sql = "SELECT $filed FROM  " . $GLOBALS['yke']->table(1,'products') . " WHERE product_id = '$product_id'"." and sysid ='".$GLOBALS['sysid']."' ";
...
function check_goods_specifications_exist($goods_id)
{
    ...
    $sql = "SELECT COUNT(a.attr_id)
            FROM " .$GLOBALS['yke']->table(1,'attribute'). " AS a, " .$GLOBALS['yke']->table(1,'goods'). " AS g
...
function check_goods_attr_exist($goods_attr, $goods_id, $product_id = 0)
{
    ...
    if (empty($product_id))
    {
        $sql = "SELECT product_id FROM " . $GLOBALS['yke']->table(1,'products') ."
...
function check_product_sn_exist($product_sn, $product_id = 0)
{
    ...
    $sql="SELECT goods_id FROM ". $GLOBALS['yke']->table(1,'goods')."WHERE goods_sn='$product_sn'"." and sysid ='".$GLOBALS['sysid']."' ";
    ...
    {
        $sql = "SELECT product_id FROM " . $GLOBALS['yke']->table(1,'products') ."
                WHERE product_sn = '$product_sn'"." and sysid ='".$GLOBALS['sysid']."' ";
    }
    else
    {
        $sql = "SELECT product_id FROM " . $GLOBALS['yke']->table(1,'products') ."
