 ...
 * ykshop 购物流程函数库
 * ============================================================================
 * 版权所有 2015-2016 杭州一开网络科技有限公司，并保留所有权利。
...
function available_shipping_list($region_id_list)
{
    $sql = 'SELECT s.shipping_id, s.shipping_code, s.shipping_name, ' .
            ...
            ' AND r.shipping_area_id = a.shipping_area_id  AND a.sysid = s.sysid  AND r.sysid = a.sysid AND a.shipping_id = s.shipping_id AND s.enabled = 1'." and s.sysid ='".$GLOBALS['sysid']."' ";
...
function shipping_area_info($shipping_id, $region_id_list)
{
    $sql = 'SELECT s.shipping_code, s.shipping_name, ' .
            ...
            ' AND r.shipping_area_id = a.shipping_area_id AND a.shipping_id = s.shipping_id AND a.sysid = s.sysid AND r.sysid = a.sysid AND s.enabled = 1'." and s.sysid ='".$GLOBALS['sysid']."' ";
...
function payment_list()
{
    $sql = 'SELECT pay_id, pay_name ' .
            'FROM ' . $GLOBALS['yke']->table(1,'payment') .
            ' WHERE enabled = 1'." and sysid ='".$GLOBALS['sysid']."' ";
...
{
    $sql = 'SELECT * FROM ' . $GLOBALS['yke']->table(1,'payment') .
...
{
    $sql = "SELECT * FROM " . $GLOBALS['yke']->table(1,'pack') ." WHERE pack_id = '$pack_id'"." and sysid ='".$GLOBALS['sysid']."' ";
...
{
    $sql = "SELECT * FROM " . $GLOBALS['yke']->table(1,'card') . " WHERE card_id = '$card_id'"." and sysid ='".$GLOBALS['sysid']."' ";
...
function order_goods($order_id)
{
    $sql = "SELECT rec_id, goods_id, goods_name, goods_sn, market_price, goods_number, " .
            ...
            " WHERE order_id = '$order_id'"." and sysid ='".$GLOBALS['sysid']."' ";  // 修改 by www.ykshop120.com 增加 package_attr_id 字段
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        {
            $row['package_goods_list'] = get_package_goods($row['goods_id'], $row['package_attr_id']); // 修改 by www.ykshop120.com
        }
        $goods_list[] = $row;
    }
...
function order_weight_price($order_id)
{
    $sql = "SELECT SUM(g.goods_weight * o.goods_number) AS weight, " .
            ...
            "AND o.goods_id = g.goods_id AND o.sysid = g.sysid "." and o.sysid ='".$GLOBALS['sysid']."' ";
...
function order_fee($order, $goods, $consignee)
{
    ...
    if ($order['shipping_id'] > 0 && $total['real_goods_count'] > 0)
    {
        ...
        if (!empty($shipping_info))
        {
            ...
            $sql = 'SELECT count(*) FROM ' . $GLOBALS['yke']->table(1,'cart') . " WHERE  `session_id` = '" . SESS_ID. "' AND `extension_code` != 'package_buy' AND `is_shipping` = 0"." and sysid ='".$GLOBALS['sysid']."' ";
    ...
    if ($order['extension_code'] == 'exchange_goods')
    {
        $sql = 'SELECT SUM(eg.exchange_integral) '.
               ...
               "WHERE c.goods_id = eg.goods_id AND c.sysid = eg.sysid AND c.session_id= '" . SESS_ID . "' " .
...
function update_order($order_id, $order)
{
    return $GLOBALS['db']->autoExecute($GLOBALS['yke']->table(1,'order_info'),
        $order, 'UPDATE', "order_id = '$order_id'"." and sysid ='".$GLOBALS['sysid']."' "." and sysid ='".$GLOBALS['sysid']."' ");
}
...
function cart_goods($type = CART_GENERAL_GOODS)
{
    $sql = "SELECT rec_id, user_id, goods_id, goods_name, goods_sn, goods_number, " .
            ...
            "AND rec_type = '$type'"." and sysid ='".$GLOBALS['sysid']."' "; //这句SQL有修改 by www.ykshop120.com 增加一个 package_attr_id
    ...
    foreach ($arr as $key => $value)
    {
        ...
        {
            $arr[$key]['package_goods_list'] = get_package_goods($value['goods_id'], $value['package_attr_id']); //修改 by www.ykshop120.com
        }
    }
    ...
    if ($row)
    {
        ...
        foreach ($row as $val)
        {
            ...
            $sql = 'SELECT count(*) FROM ' .
                    ...
                    $GLOBALS['yke']->table(1,'goods') . ' AS g ' .
                    "WHERE g.goods_id = pg.goods_id AND  g.sysid = pg.sysid AND g.is_shipping = 0 AND pg.package_id = '"  . $val['goods_id'] . "'"." and pg.sysid ='".$GLOBALS['sysid']."' ";
            $shipping_count = $GLOBALS['db']->getOne($sql);
...
function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0)
{
    ...
    $sql = "SELECT g.goods_name, g.goods_sn, g.is_on_sale, g.is_real, ".
            ...
            " AND g.is_delete = 0"." and g.sysid ='".$GLOBALS['sysid']."' ";
    ...
    $sql = "SELECT * FROM " .$GLOBALS['yke']->table(1,'products'). " WHERE goods_id = '$goods_id' LIMIT 0, 1"." and sysid ='".$GLOBALS['sysid']."' ";
...
function clear_cart($type = CART_GENERAL_GOODS)
{
    $sql = "DELETE FROM " . $GLOBALS['yke']->table(1,'cart') .
            " WHERE session_id = '" . SESS_ID . "' AND rec_type = '$type'"." and sysid ='".$GLOBALS['sysid']."' ";
    $GLOBALS['db']->query($sql);
}
...
function get_goods_attr_info($arr, $type = 'pice')
{
    ...
    if (!empty($arr))
    {
        ...
        $sql = "SELECT a.attr_name, ga.attr_value, ga.attr_price ".
                ...
                "WHERE " .db_create_in($arr, 'ga.goods_attr_id')." AND a.attr_id = ga.attr_id AND a.sysid = ga.sysid "." and ga.sysid ='".$GLOBALS['sysid']."' ";
        $res = $GLOBALS['db']->query($sql);
...
function get_cart_goods()
{
    ...
    $sql = "SELECT *, IF(parent_id, parent_id, goods_id) AS pid " .
            " FROM " . $GLOBALS['yke']->table(1,'cart') . " " .
            " WHERE session_id = '" . SESS_ID . "' AND rec_type = '" . CART_GENERAL_GOODS . "'" ." and sysid ='".$GLOBALS['sysid']."' ".
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        {
            $sql = "SELECT attr_value FROM " . $GLOBALS['yke']->table(1,'goods_attr') . " WHERE goods_attr_id " .
            db_create_in($row['goods_attr'])." and sysid ='".$GLOBALS['sysid']."' ";
        ...
        if ($row['extension_code'] == 'package_buy')
        {
            $row['package_goods_list'] = get_package_goods($row['goods_id'], $row['package_attr_id'] ); //修改 by www.ykshop120.com 增加一个变量
        }
        $goods_list[] = $row;
    }
    ...
    switch ($storage)
    {
        case 0 :
            $sql = "SELECT goods_id, SUM(send_number) AS num, MAX(extension_code) AS extension_code, product_id FROM " . $GLOBALS['yke']->table(1,'order_goods') .
                    " WHERE order_id = '$order_id' AND is_real = 1 "." and sysid ='".$GLOBALS['sysid']."' "." GROUP BY goods_id, product_id";
        break;
...
{
    $sql = "SELECT pay_id FROM " . $GLOBALS['yke']->table(1,'payment');
    if ($is_cod)
    {
...
function send_order_bonus($order_id)
{
    ...
    if ($bonus_list)
    {
        ...
        if ($count > 0)
        {
            ...
            $GLOBALS['smarty']->assign('sysid', $GLOBALS['sysid']);
...
function compute_discount_amount()
{
    ...
    $sql = "SELECT c.goods_id, c.goods_price * c.goods_number AS subtotal, g.cat_id, g.brand_id " .
            ...
            "AND c.parent_id = 0 " .
            "AND c.is_gift = 0 " .
            "AND rec_type = '" . CART_GENERAL_GOODS . "'"." and g.sysid ='".$GLOBALS['sysid']."' ";
    $goods_list = $GLOBALS['db']->getAll($sql);
    if (!$goods_list)
    {
        return 0;
...
//    }
    ...
    $parent = array(
        ...
        'market_price'  => $package_pricea[0] ? $package_pricea[0] :  $package['market_package'], //修改 by www.ykshop120.com
        'goods_price'   => $package_pricea[1] ? $package_pricea[1] :  $package['package_price'], //修改 by www.ykshop120.com
		'package_attr_id' =>$package_attr_id, //增加 by www.ykshop120.com
        'goods_number'  => $num,
        'goods_attr'    => '',
        'goods_attr_id' => '',
    ...
    if ($num > 0)
    {
        ...
        $sql = "SELECT sysid,goods_number FROM " .$GLOBALS['yke']->table(1,'cart').
                ...
                " AND package_attr_id = '$package_attr_id'  AND rec_type = '" . CART_GENERAL_GOODS . "'"." and sysid ='".$GLOBALS['sysid']."' ";  //修改 by www.ykshop120.com 增加一条件
...
function judge_package_stock($package_id, $package_num = 1)
{
    ...
    if ($goods['product_ids'] != '')
    {
        $sql = "SELECT p.product_id
                ...
                WHERE pg.product_id = p.product_id and pg.sysid = p.sysid
    ...
    if ($goods['goods_ids'] != '')
    {
        $sql = "SELECT g.goods_id
                ...
                WHERE pg.goods_id = g.goods_id and pg.sysid = g.sysid
