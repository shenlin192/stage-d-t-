    ...
    if ($res === NULL)
    {
        ...
        if ($data === false)
        {
            $sql = "SELECT c.cat_id, c.cat_name, c.measure_unit, c.parent_id, c.is_show, c.show_in_nav, c.grade, c.sort_order, COUNT(s.cat_id) AS has_children ".
                ...
                "LEFT JOIN " . $GLOBALS['yke']->table(1,'category') . " AS s ON s.parent_id=c.cat_id and s.sysid = c.sysid ".
                "where c.sysid ='".$GLOBALS['sysid']."' ".
                "GROUP BY c.cat_id ".
...
function load_config()
{
    ...
    if ($data === false)
    {
        $sql = 'SELECT code, value FROM ' . $GLOBALS['yke']->table(1,'shop_config') . ' WHERE parent_id > 0' ." and sysid ='".$GLOBALS['sysid']."' "  ;
        $res = $GLOBALS['db']->getAll($sql);
...
function get_brands($cat = 0, $app = 'brand')
{
    ...
    $sql = "SELECT b.brand_id, b.brand_name, b.brand_logo, b.brand_desc, COUNT(*) AS goods_num, IF(b.brand_logo > '', '1', '0') AS tag ".
            ...
            "WHERE g.brand_id = b.brand_id and g.sysid = b.sysid $children AND is_show = 1 " .
            " AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 and g.sysid ='".$GLOBALS['sysid']."' ".
...
function get_promotion_info($goods_id = '')
{
    ...
    $favourable = array();
    $sql = 'SELECT act_id, act_range, act_range_ext, act_name, start_time, end_time FROM ' . $GLOBALS['yke']->table(1,'favourable_activity') . " WHERE start_time <= '$gmtime' AND end_time >= '$gmtime' and sysid ='".$GLOBALS['sysid']."' "     ;
    if(!empty($goods_id))
    {
...
function order_action($order_sn, $order_status, $shipping_status, $pay_status, $note = '', $username = null, $place = 0)
{
    ...
    $sql = 'INSERT INTO ' . $GLOBALS['yke']->table(1,'order_action') .
        ...
        'SELECT ' . "'" . $GLOBALS['sysid'] . "',order_id, '$username', '$order_status', '$shipping_status', '$pay_status', '$place', '$note', '" . gmtime() . "' " .
...
;
    ...
    if ($num < $goods['num'])
    {
     ...
     /* 取出卡片信息 */
     $sql = "SELECT card_id, card_sn, card_password, end_date, crc32 FROM ".$GLOBALS['yke']->table(1,'virtual_card')." WHERE goods_id = '$goods[goods_id]' AND is_saled = 0  LIMIT " . $goods['num'] ." and sysid ='".$GLOBALS['sysid']."' ";
...
function get_snatch_result($id)
{
    $sql = 'SELECT u.user_id, u.user_name, u.email, lg.bid_price, lg.bid_time, count(*) as num' .
            ...
            ' LEFT JOIN ' . $GLOBALS['yke']->table(1,'users') . ' AS u ON lg.user_id = u.user_id'.
            " WHERE lg.snatch_id = '$id'" ." and sysid ='".$GLOBALS['sysid']."' ".
    ...
    if ($rec)
    {
        ...
        $sql = "SELECT COUNT(*)" .
                ...
                " AND extension_id = '$id'" ." and sysid ='".$GLOBALS['sysid']."' ".
    ...
    if ($res === NULL)
    {
        ...
        if ($data === false)
        {
            $sql = "SELECT c.*, COUNT(s.cat_id) AS has_children, COUNT(a.article_id) AS aricle_num ".
               ' FROM ' . $GLOBALS['yke']->table(1,'article_cat') . " AS c".
               " LEFT JOIN " . $GLOBALS['yke']->table(1,'article_cat') . " AS s ON s.parent_id=c.cat_id and s.sysid=c.sysid ".
               " LEFT JOIN " . $GLOBALS['yke']->table(1,'article') . " AS a ON a.cat_id=c.cat_id and a.sysid=c.sysid  and c.sysid ='".$GLOBALS['sysid']."' "    .
               " GROUP BY c.cat_id ".
...
function sort_goods_attr_id_array($goods_attr_id_array, $sort = 'asc')
{
    ...
    $sql = "SELECT a.attr_type, v.attr_value, v.goods_attr_id
            ...
            LEFT JOIN " .$GLOBALS['yke']->table(1,'goods_attr'). " AS v
                ON v.attr_id = a.attr_id and v.sysid = a.sysid
                AND a.attr_type = 1
...
function get_package_info($id)
{
    ...
    $sql = "SELECT pg.package_id, pg.goods_id, pg.goods_number, pg.admin_id, ".
           ...
           "   ON g.goods_id = pg.goods_id and g.sysid = pg.sysid ".
           " LEFT JOIN " . $GLOBALS['yke']->table(1,'member_price') . " AS mp ".
                "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
           " WHERE pg.package_id = " . $id. " "." and g.sysid ='".$GLOBALS['sysid']."' ".
...
/* 修改 by www.ykshop120.com 增加一个参数 */
function get_package_goods($package_id, $package_attr_id='')
{
    ...
    $sql = "SELECT pg.goods_id, g.goods_name, pg.goods_number, p.goods_attr, p.product_number, p.product_id
            ...
            WHERE pg.package_id = '$package_id' $sql_package_attr_id " ." and pg.sysid ='".$GLOBALS['sysid']."' "   ; //有修改 by www.ykshop120.com 注意最后那个 $sql_package_attr_id
    if ($package_id == 0)
    {
...
function get_specifications_list($goods_id, $conditions = '')
{
    ...
    $sql = "SELECT ga.goods_attr_id, ga.attr_id, ga.attr_value, a.attr_name
            ...
            AND ga.goods_id = '$goods_id' and ga.sysid ='".$GLOBALS['sysid']."' " .
            $conditions;
