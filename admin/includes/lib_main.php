...
{
    $rank_list = array();
    $sql = 'SELECT rank_id, rank_name, min_points FROM ' . $GLOBALS['yke']->table(1,'user_rank');
    $sql .= ' WHERE '." sysid ='".$GLOBALS['sysid']."'";
    if ($is_special)
    {
...
{
    $user_list = array();
    $sql = 'SELECT user_id, user_name FROM ' . $GLOBALS['yke']->table(1,'users') . $where." and sysid ='".$GLOBALS['sysid']."'".
...
{
    $sql = 'TRUNCATE TABLE ' .$GLOBALS['yke']->table($table_name);
