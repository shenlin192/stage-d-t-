...
function add_feed($id, $feed_type)
{
    ...
    if ($feed_type == BUY_GOODS)
    {
        ...
        $id = intval($id);
        $order_res = $GLOBALS['db']->getAll("SELECT g.goods_id, g.goods_name, g.goods_sn, g.goods_desc, g.goods_thumb, o.goods_price FROM " . $GLOBALS['yke']->table(1,'order_goods') . " AS o, " . $GLOBALS['yke']->table(1,'goods') . " AS g WHERE o.order_id='{$id}' AND o.goods_id=g.goods_id and o.sysid-g.sysid "." and o.sysid ='".$GLOBALS['sysid']."' ");
        foreach($order_res as $goods_data)
        {
            if(!empty($goods_data['goods_thumb']))
            {
                $url = $GLOBALS['yke']->url() . $goods_data['goods_thumb'];
            }
            else
            {
                $url = $GLOBALS['yke']->url() . $GLOBALS['_CFG']['no_picture'];
            }
            $link = $GLOBALS['yke']->url() .'goods.php?sysessid='.$GLOBALS['sysessid'].'&id=' . $goods_data["goods_id"];
