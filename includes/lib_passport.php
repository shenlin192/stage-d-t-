...
function _register ($username, $password, $email_or_mobile, $other = array(), $register_type = 'email')
{
	...
	else
	{
		...
		if(isset($affiliate['on']) && $affiliate['on'] == 1)
		{
			...
			if($up_uid)
			{
				if(! empty($affiliate['config']['level_register_all']))
				{
					...
					{
						$rank_points = $GLOBALS['db']->getOne("SELECT rank_points FROM " . $GLOBALS['yke']->table(1,'users') . " WHERE user_id = '$up_uid'"." and sysid ='".$GLOBALS['sysid']."' ");
				...
				// 设置推荐人
				$sql = 'UPDATE ' . $GLOBALS['yke']->table(1,'users') . ' SET parent_id = ' . $up_uid . ' WHERE user_id = ' . $_SESSION['user_id']." and sysid ='".$GLOBALS['sysid']."' ";
		...
		$GLOBALS['db']->autoExecute($GLOBALS['yke']->table(1,'users'), $update_data, 'UPDATE', 'user_id = ' . $_SESSION['user_id']." and sysid ='".$GLOBALS['sysid']."' ");
...
function check_userinfo($user_name, $email)
{
    ...
    {
        yk_header('Location: user.php?sysessid='.$GLOBALS['sysessid'].'&act=get_password');//.'\n'
...
function send_pwd_email($uid, $user_name, $email, $code)
{
    ...
    $reset_email = $GLOBALS['yke']->url() . 'user.php?sysessid='.$GLOBALS['sysessid'].'&act=get_password&uid=' . $uid . '&code=' . $code;
...
function send_regiter_hash ($user_id)
{
    ...
    $validate_email = $GLOBALS['yke']->url() . 'user.php?sysessid='.$GLOBALS['sysessid'].'&act=validate_email&hash=' . $hash;

    $sql = "SELECT user_name, email FROM " . $GLOBALS['yke']->table(1,'users') . " WHERE user_id = '$user_id'"." and sysid ='".$GLOBALS['sysid']."' ";
...
{
    $res = $GLOBALS['db']->getOne("SELECT COUNT(*) FROM " . $GLOBALS['yke']->table(1,'admin_user') .
                                  " WHERE user_name = '$adminname'"." and sysid ='".$GLOBALS['sysid']."' ");
    return $res;
}
...
function check_username_exist ($username)
{
	...
	$sql = "select user_name from " . $GLOBALS['yke']->table(1,'users') . " where user_id = '" . $_SESSION['user_id'] . "'"." and sysid ='".$GLOBALS['sysid']."' ";
	...
	{
		$sql = "select count(*) from " . $GLOBALS['yke']->table(1,'users') . " where user_name = '$username'"." and sysid ='".$GLOBALS['sysid']."' ";
