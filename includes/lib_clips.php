...
function get_collection_goods($user_id, $num = 10, $start = 0)
{
    $sql = 'SELECT g.goods_id, g.goods_name,g.goods_thumb, g.market_price, g.shop_price AS org_price, '.
            ...
            ' FROM ' . $GLOBALS['yke']->table(1,'collect_goods') . ' AS c' .
            " LEFT JOIN " . $GLOBALS['yke']->table(1,'goods') . " AS g ".
                "ON g.goods_id = c.goods_id and g.sysid = c.sysid ".
            ...
            " WHERE c.user_id = '$user_id' "." and c.sysid ='".$GLOBALS['sysid']."' "." ORDER BY c.rec_id DESC";
...
function get_follow_shops($user_id, $num = 10, $start = 0)
{
    $sql = 'SELECT sg.id,sg.supplierid,s.supplier_name,s.tel,s.company_name,s.rank_id ' .
            ' FROM ' . $GLOBALS['yke']->table(1,'supplier_guanzhu') . ' AS sg' .
            " LEFT JOIN " . $GLOBALS['yke']->table(1,'supplier') . " AS s ".
                "ON sg.supplierid = s.supplier_id and sg.sysid = s.sysid ".
            " WHERE sg.userid = '$user_id' "." and sg.sysid ='".$GLOBALS['sysid']."' "." ORDER BY sg.addtime DESC";
    $res = $GLOBALS['db'] -> selectLimit($sql, $num, $start);
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $suppinfo = $GLOBALS['db'] -> query("select value,code from " . $GLOBALS['yke']->table(1,'supplier_shop_config') ." where supplier_id = ".$row['supplierid']." AND code in('shop_name','shop_logo','qq','ww')");
...
{
    $sql = 'SELECT COUNT(*) '.
           'FROM ' .$GLOBALS['yke']->table(1,'booking_goods').
           "WHERE user_id = '$user_id' AND goods_id = '$goods_id' AND is_dispose = 0"." and sysid ='".$GLOBALS['sysid']."' ";
...
function get_message_list($user_id, $user_name, $num, $start, $order_id = 0)
{
    /* 获取留言数据 */
    $msg = array();
    $sql = "SELECT * FROM " .$GLOBALS['yke']->table(1,'feedback');
    if ($order_id)
    {
        $sql .= " WHERE parent_id = 0 AND order_id = '$order_id' AND user_id = '$user_id'"." and sysid ='".$GLOBALS['sysid']."' "." ORDER BY msg_time DESC";
    }
    else
    {
        $sql .= " WHERE parent_id = 0 AND user_id = '$user_id' AND user_name = '" . $_SESSION['user_name'] . "' AND order_id=0 "." and sysid ='".$GLOBALS['sysid']."' "." ORDER BY msg_time DESC";
    }
    ...
    while ($rows = $GLOBALS['db']->fetchRow($res))
    {
        ...
        //{
            ...
            $sql   = "SELECT user_name, user_email, msg_time, msg_content".
                     " FROM " .$GLOBALS['yke']->table(1,'feedback') .
                     " WHERE parent_id = '" . $rows['msg_id'] . "'"." and sysid ='".$GLOBALS['sysid']."' ";
...
function add_message($message)
{
    ...
    $sql = "INSERT INTO " . $GLOBALS['yke']->table(1,'feedback') .
            " (sysid,msg_id, parent_id, user_id, user_name, user_email, msg_title, msg_type, msg_status,  msg_content, msg_time, message_img, order_id, msg_area)".
            " VALUES ("."'".$GLOBALS['sysid']."',"."NULL, 0, '$message[user_id]', '$message[user_name]', '$message[user_email]', ".
...
{
     $sql = "DELETE FROM ".$GLOBALS['yke']->table(1,'tag').
            " WHERE tag_words = '$tag_words' AND user_id = '$user_id'"." and sysid ='".$GLOBALS['sysid']."' ";
...
function get_booking_list($user_id, $num, $start)
{
    ...
    $sql = "SELECT bg.rec_id, bg.goods_id, bg.goods_number, bg.booking_time, bg.dispose_note, g.goods_name,g.goods_thumb ".   
           "FROM " .$GLOBALS['yke']->table(1,'booking_goods')." AS bg , " .$GLOBALS['yke']->table(1,'goods')." AS g". " WHERE bg.goods_id = g.goods_id and bg.sysid=g.sysid AND bg.user_id = '$user_id'"." and bg.sysid ='".$GLOBALS['sysid']."' "." ORDER BY bg.booking_time DESC";
    $res = $GLOBALS['db']->SelectLimit($sql, $num, $start);
...
{
    $info = array();
    $sql  = "SELECT goods_name FROM " .$GLOBALS['yke']->table(1,'goods'). " WHERE goods_id = '$goods_id'"." and sysid ='".$GLOBALS['sysid']."' ";
    ...
    if (!empty($_SESSION['user_id']))
    {
        ...
        $sql = "SELECT ua.consignee, ua.email, ua.tel, ua.mobile ".
               ...
               " WHERE u.address_id = ua.address_id and ua.sysid=u.sysid AND u.user_id = '$_SESSION[user_id]'"." andua. sysid ='".$GLOBALS['sysid']."' ";
...
{
    $sql = 'DELETE FROM ' .$GLOBALS['yke']->table(1,'booking_goods').
           " WHERE rec_id = '$booking_id' AND user_id = '$user_id'"." and sysid ='".$GLOBALS['sysid']."' ";
...
{
    $sql = "INSERT INTO " .$GLOBALS['yke']->table(1,'booking_goods').
            " VALUES ("."'".$GLOBALS['sysid']."',"."'', '$_SESSION[user_id]', '$booking[email]', '$booking[linkman]', ".
...
{
    $sql = 'INSERT INTO ' .$GLOBALS['yke']->table(1,'user_account').
           ' (sysid,user_id, admin_user, amount, add_time, paid_time, admin_note, user_note, process_type, payment, is_paid)'.
            " VALUES ("."'".$GLOBALS['sysid']."',"."'$surplus[user_id]', '', '$amount', '".gmtime()."', 0, '', '$surplus[user_note]', '$surplus[process_type]', '$surplus[payment]', 0)";
    $GLOBALS['db']->query($sql);
...
{
    $sql = 'UPDATE ' .$GLOBALS['yke']->table(1,'user_account'). ' SET '.
           "amount     = '$surplus[amount]', ".
           "user_note  = '$surplus[user_note]', ".
           "payment    = '$surplus[payment]' ".
           "WHERE id   = '$surplus[rec_id]'"." and sysid ='".$GLOBALS['sysid']."' ";
    $GLOBALS['db']->query($sql);
...
{
    $sql = 'INSERT INTO ' .$GLOBALS['yke']->table(1,'pay_log')." (sysid,order_id, order_amount, order_type, is_paid)".
            " VALUES  ("."'".$GLOBALS['sysid']."',"."'$id', '$amount', '$type', '$is_paid')";
    $GLOBALS['db']->query($sql);
...
{
    $sql = 'SELECT log_id FROM' .$GLOBALS['yke']->table(1,'pay_log').
           " WHERE order_id = '$surplus_id' AND order_type = '$pay_type' AND is_paid = 0"." and sysid ='".$GLOBALS['sysid']."' ";
...
{
    $sql = 'SELECT * FROM ' .$GLOBALS['yke']->table(1,'user_account').
           " WHERE id = '$surplus_id'"." and sysid ='".$GLOBALS['sysid']."' ";
...
function get_online_payment_list($include_balance = true)
{
    $sql = 'SELECT pay_id, pay_code, pay_name, pay_fee, pay_desc ' .
            ...
            " WHERE enabled = 1 AND is_cod <> 1"." and sysid ='".$GLOBALS['sysid']."' ";
    if (!$include_balance)
    {
...
function get_account_log($user_id, $num, $start)
{
    ...
    $sql = 'SELECT * FROM ' .$GLOBALS['yke']->table(1,'user_account').
           ...
           " AND process_type " . db_create_in(array(SURPLUS_SAVE, SURPLUS_RETURN))." and sysid ='".$GLOBALS['sysid']."' " .
    ...
    if ($res)
    {
        while ($rows = $GLOBALS['db']->fetchRow($res))
        {
            ...
            $sql = 'SELECT pay_id FROM ' .$GLOBALS['yke']->table(1,'payment').
                   " WHERE pay_name = '$rows[payment]' AND enabled = 1"." and sysid ='".$GLOBALS['sysid']."' ";
            $pid = $GLOBALS['db']->getOne($sql);

            /* 如果是预付款而且还没有付款, 允许付款 */
            if (($rows['is_paid'] == 0) && ($rows['process_type'] == 0))
            {
                $rows['handle'] = '<a href="user.php?'.'sysessid='.$GLOBALS['sysessid'].'&act=pay&id='.$rows['id'].'&pid='.$pid.'">'.$GLOBALS['_LANG']['pay'].'</a>';
            }
...
{
    $sql = 'DELETE FROM ' .$GLOBALS['yke']->table(1,'user_account').
           " WHERE is_paid = 0 AND id = '$rec_id' AND user_id = '$user_id'"." and sysid ='".$GLOBALS['sysid']."' ";
...
{
    $sql = "SELECT SUM(user_money) FROM " .$GLOBALS['yke']->table(1,'account_log').
...
function get_user_default($user_id)
{
	...
	$sql = "SELECT * FROM " .$GLOBALS['yke']->table(1,'users'). " WHERE user_id = '$user_id'"." and sysid ='".$GLOBALS['sysid']."' ";
    ...
    $sql = "SELECT COUNT(*) FROM " .$GLOBALS['yke']->table(1,'order_info').
            " WHERE user_id = '" .$user_id. "' AND add_time > '" .local_strtotime('-1 months'). "'"." and sysid ='".$GLOBALS['sysid']."' ";
    $info['order_count'] = $GLOBALS['db']->getOne($sql);

    include_once(ROOT_PATH . 'includes/lib_order.php');
    $sql = "SELECT order_id, order_sn ".
            " FROM " .$GLOBALS['yke']->table(1,'order_info').
            " WHERE user_id = '" .$user_id. "' AND shipping_time > '" .$last_time. "'"." and sysid ='".$GLOBALS['sysid']."' ". order_query_sql('shipped');
...
function add_tag($id, $tag)
{
    ...
    {
        /* 检查是否重复 */
        $sql = "SELECT COUNT(*) FROM ".$GLOBALS['yke']->table(1,"tag").
                " WHERE user_id = '".$_SESSION['user_id']."' AND goods_id = '$id' AND tag_words = '$val'"." and sysid ='".$GLOBALS['sysid']."' ";

        if ($GLOBALS['db']->getOne($sql) == 0)
        {
            $sql = "INSERT INTO ".$GLOBALS['yke']->table("tag")." (sysid,user_id, goods_id, tag_words) ".
                    "VALUES ("."'".$GLOBALS['sysid']."',"."'".$_SESSION['user_id']."', '$id', '$val')";
            $GLOBALS['db']->query($sql);
        }
    }
}
...
function color_tag(&$tags)
{
    ...
    foreach ($tags AS $key => $val)
    {
        ...
        {
            $tags[$key]['url'] = 'search.php?sysessid='.$GLOBALS['sysessid'].'&keywords=' . urlencode($val['tag_words']);
        }
    }
    shuffle($tags);
}
...
function get_rank_info()
{
    ...
    {
        $sql = "SELECT rank_name, special_rank FROM " . $yke->table(1,'user_rank') . " WHERE rank_id = '$_SESSION[user_rank]'"." and sysid ='".$GLOBALS['sysid']."' ";
        $row = $db->getRow($sql);
        if (empty($row))
        {
            return array();
        ...
        {
            $user_rank = $db->getOne("SELECT rank_points FROM " . $yke->table(1,'users') . " WHERE user_id = '$_SESSION[user_id]'"." and sysid ='".$GLOBALS['sysid']."' ");
            $sql = "SELECT rank_name,min_points FROM " . $yke->table(1,'user_rank') . " WHERE min_points > '$user_rank'"." and sysid ='".$GLOBALS['sysid']."' "." ORDER BY min_points ASC LIMIT 1";
            $rt  = $db->getRow($sql);
...
function get_user_prompt ($user_id)
{
    ...
    $sql = "SELECT act_id, goods_name, end_time " .
            ...
            " AND (is_finished = 1 OR (is_finished = 0 AND end_time <= '$now'))"." and sysid ='".$GLOBALS['sysid']."' ";
...
function get_comment_list($user_id, $page_size, $start)
{
    $sql = "SELECT c.*, g.goods_id, g.goods_name AS cmt_name, r.content AS reply_content, r.add_time AS reply_time, g.goods_thumb, g.shop_price ".
           " FROM " . $GLOBALS['yke']->table(1,'comment') . " AS c ".
           " LEFT JOIN " . $GLOBALS['yke']->table(1,'comment') . " AS r ".
           " ON r.parent_id = c.comment_id and c.sysid=r.sysid AND r.parent_id > 0 ".
           " LEFT JOIN " . $GLOBALS['yke']->table(1,'goods') . " AS g ".
           " ON c.comment_type=0 AND c.id_value = g.goods_id and c.sysid=g.sysid ".
           " WHERE c.user_id='$user_id'"." and c.sysid ='".$GLOBALS['sysid']."' ";
    ...
    if ($to_article)
    {
        $sql = "SELECT article_id , title FROM " . $GLOBALS['yke']->table(1,'article') . " WHERE " . db_create_in($to_article, 'article_id')." and sysid ='".$GLOBALS['sysid']."' ";
        ...
        foreach ($comments as $key=>$row)
        {
            ...
            /* 获取晒单信息*/
       foreach ($comments as $key=>$row){
           $sql = "SELECT * FROM ".$GLOBALS['yke']->table(1,'shaidan')." where goods_id = ".$row['goods_id']." and sysid ='".$GLOBALS['sysid']."' ";
           $shaidan = $GLOBALS['db']->getAll($sql);
           foreach($shaidan as $k=>$v){
               $shaidan[$k]['shaidan_img'] = $GLOBALS['db']->getAll("SELECT * FROM ".$GLOBALS['yke']->table(1,'shaidan_img')." WHERE shaidan_id = '$v[shaidan_id]]'"." and sysid ='".$GLOBALS['sysid']."' ");
...
{
	$sql = "SELECT c.goods_price,c.goods_id,g.* FROM ".$GLOBALS['yke']->table(1,'goods')."as g,".$GLOBALS['yke']->table(1,'cart')."as c WHERE c.user_id='$user_id' AND g.goods_id=c.goods_id and g.sysid=c.sysid "." and g.sysid ='".$GLOBALS['sysid']."' ";
...
{
	$sql = "SELECT e.*,g.* FROM".$GLOBALS['yke']->table(1,'goods')."as g,".$GLOBALS['yke']->table(1,'exchange_goods')."as e WHERE g.goods_id=e.goods_id and g.sysid=e.sysid AND e.is_exchange='1'"." and g.sysid ='".$GLOBALS['sysid']."' ";
	$res = $GLOBALS['db']->getAll($sql);
	return $res;
}
/*
...
{
	$sql = "SELECT c.user_id,c.goods_id,g.* FROM ".$GLOBALS['yke']->table(1,'goods')."as g,".$GLOBALS['yke']->table(1,'collect_goods')." as c WHERE c.user_id = '$user_id' AND g.goods_id = c.goods_id and g.sysid=c.sysid "." and g.sysid ='".$GLOBALS['sysid']."' ";
	$res = $GLOBALS['db']->getAll($sql);
...
{
	$sql = "SELECT g.*,i.*,o.* FROM ".$GLOBALS['yke']->table(1,'goods')."as g,".$GLOBALS['yke']->table(1,'order_info')."as i,".$GLOBALS['yke']->table(1,'order_goods')."as o WHERE i.user_id ='$user_id' AND i.shipping_status=2 AND i.order_id=o.order_id AND o.goods_id = g.goods_id";
	$res = $GLOBALS['db']->getAll($sql);
	return $res;
}
/*
...
{
	//查找代付款的详情
	$sql = "SELECT * FROM ".$GLOBALS['yke']->table(1,'order_info')."WHERE user_id = '$user_id' AND pay_status=0 "." and sysid ='".$GLOBALS['sysid']."' "." order by order_sn desc limit 4";
	$res = $GLOBALS['db']->getAll($sql);
	foreach($res as $key=>$value)
	{
		$rea[$key] = $value['order_id'];
		$sql = "SELECT g.goods_thumb,o.* FROM ".$GLOBALS['yke']->table(1,'goods')."as g,".$GLOBALS['yke']->table(1,'order_goods')."as o WHERE o.order_id='$rea[$key]' AND o.goods_id = g.goods_id and g.sysid=o.sysid "." and g.sysid ='".$GLOBALS['sysid']."' ";
		$resu[$key] = $GLOBALS['db']->getRow($sql);
		$sqls = "SELECT COUNT(*) FROM ".$GLOBALS['yke']->table(1,'goods')."as g,".$GLOBALS['yke']->table(1,'order_goods')."as o WHERE o.order_id='$rea[$key]' AND o.goods_id = g.goods_id and g.sysid=o.sysid "." and g.sysid ='".$GLOBALS['sysid']."' ";
...
{
	//查询待付款的
	$sql = "SELECT COUNT(*) FROM " . $GLOBALS['yke']->table(1,'order_info') . " WHERE user_id ='$user_id' AND order_status=0"." and sysid ='".$GLOBALS['sysid']."' ";
	$res['daif'] = $GLOBALS['db']->getOne($sql);
	//查询待收货的数量
	$sql = "SELECT COUNT(*) FROM " . $GLOBALS['yke']->table(1,'order_info') . " WHERE user_id='$user_id' AND pay_status=0 AND order_status=1"." and sysid ='".$GLOBALS['sysid']."' ";
	$res['dais'] = $GLOBALS['db']->getOne($sql);
	//查询待评价的数量
	$a_comment = implode(',', $GLOBALS['db']->getRow("select distinct(order_id) from " . $GLOBALS['yke']->table(1,'order_goods') . " where comment_state = 0 or shaidan_state = 0"." and sysid ='".$GLOBALS['sysid']."' "));
	if (!empty($a_comment))
	{
		$sql = "SELECT count(*) FROM " . $GLOBALS['yke']->table(1,'order_info') . " WHERE shipping_status = 2 AND order_id in (" . $a_comment . ") "." and sysid ='".$GLOBALS['sysid']."' ";
