...
function get_shop_license()
{
    ...
    $sql = "SELECT code, value
            FROM " . $GLOBALS['yke']->table(1,'ykesmart_shop_config') . "
            WHERE code IN ('certificate_id', 'token', 'certi')"." and sysid ='".$GLOBALS['sysid']."' "."
            LIMIT 0,3";
...
function exchange_shop_license($certi, $license, $use_lib = 0)
{
    ...
    include_once(PBULIC_PATH . 'includes/cls_json.php');
...
function license_reg($certi_added = '')
{
    ...
    {
        // 注册信息入库
        $sql = "UPDATE " . $GLOBALS['yke']->table(1,'ykesmart_shop_config',1) . "
                SET value = '" . $request_arr['info']['certificate_id'] . "' WHERE code = 'certificate_id'"." and sysid ='".$GLOBALS['sysid']."' ";
        $GLOBALS['db']->query($sql);
        $sql = "UPDATE " . $GLOBALS['yke']->table(1,'ykesmart_shop_config',1) . "
                SET value = '" . $request_arr['info']['token'] . "' WHERE code = 'token'"." and sysid ='".$GLOBALS['sysid']."' "." and sysid ='".$GLOBALS['sysid']."' ";
        $GLOBALS['db']->query($sql);
