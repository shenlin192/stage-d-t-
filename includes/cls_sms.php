...
class sms
{
    ...
    function sms()
    {
        /* 由于要包含init_d.php，所以这两个对象一定是存在的，因此直接赋值 */
        $this->db = $GLOBALS['db'];
        $this->yke = $GLOBALS['yke'];
    ...
    {
        $sql = 'SELECT `value`
                FROM ' . $this->yke->table('ykesmart_shop_config') . "
                WHERE `code` = 'ent_id'"." and sysid ='".$GLOBALS['sysid']."' ";
    ...
    {
        $sql = 'SELECT `email` FROM ' . $this->yke->table('admin_user') . " WHERE `user_id` = '" . $_SESSION['admin_id'] . "'"." and sysid ='".$GLOBALS['sysid']."' ";
