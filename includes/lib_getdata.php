...
function get_article_new( $id = array(0), $getwhat = 'art_id', $num = 0, $isrand = false, $showall = false, $start = 0 ) {
	...
	if( $getwhat == 'art_cat' || $getwhat == 'art_id' ) {
		...
		FROM ' . $GLOBALS['yke']->table(1,'article') . ' AS a, ' .
		 $GLOBALS['yke']->table(1,'article_cat') . ' AS ac' .
		' WHERE (a.cat_id = ac.cat_id and a.sysid = ac.sysid) AND ' . $wherestr." and a.sysid ='".$GLOBALS['sysid']."' ";
	}
	elseif( $getwhat == 'goods_cat' || $getwhat == 'goods_id' ) {
		...
		'LEFT JOIN ' . $GLOBALS['yke']->table(1,'goods_article') . ' AS ga ON g.goods_id=ga.goods_id and  g.sysid=ga.sysid  INNER JOIN ' .
		$GLOBALS['yke']->table(1,'article') . ' AS a ON ga.article_id = a.article_id and  ga.sysid = a.sysid, ' .
		$GLOBALS['yke']->table(1,'article_cat') . 'AS ac ' .
		'WHERE (a.cat_id = ac.cat_id and a.sysid = ac.sysid ) AND ' . $wherestr." and g.sysid ='".$GLOBALS['sysid']."' ";
	}
