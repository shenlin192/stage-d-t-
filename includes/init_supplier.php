...
require(ROOT_PATH . 'includes/cls_ykshop.php');
...
if (!defined('INIT_NO_USERS'))
{
    ...
    $sess = new cls_session($db, $yke->table(1,'sessions'), $yke->table(1,'sessions_data'));
...
if (!defined('INIT_NO_USERS'))
{
    ...
    if (!empty($_COOKIE['YKE']['user_id']) && !empty($_COOKIE['YKE']['password']))
    {
        ...
        $sql = 'SELECT user_id, user_name, password ' .
                ' FROM ' .$yke->table(1,'users') .
                " WHERE user_id = '" . intval($_COOKIE['YKE']['user_id']) . "' AND password = '" .$_COOKIE['YKE']['password']. "'"." and sysid ='".$GLOBALS['sysid']."' ";
