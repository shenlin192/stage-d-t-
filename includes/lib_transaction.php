...
function edit_profile($profile)
{
    ...
    if(!empty($profile['user_name'])){
        $user_id_exist = $GLOBALS['db']->getOne("select user_id from ". $GLOBALS['yke']->table(1,'users') ." where user_id <> '".$profile['user_id']."' AND user_name = '".$profile['user_name']."'"." and sysid ='".$GLOBALS['sysid']."' ");
        if($user_id_exist){
    ...
    $cfg = array();
    $cfg['username'] = $GLOBALS['db']->getOne("SELECT user_name FROM " . $GLOBALS['yke']->table(1,'users') . " WHERE user_id='" . $profile['user_id'] . "'"." and sysid ='".$GLOBALS['sysid']."' ");
    ...
    {
        $GLOBALS['db']->autoExecute($GLOBALS['yke']->table(1,'users'), $profile['other'], 'UPDATE', "user_id = '$profile[user_id]'"." and sysid ='".$GLOBALS['sysid']."' ");
    }
...
function get_profile($user_id)
{
    ...
    $infos = array();
    $sql  = "SELECT * ".
           "FROM " .$GLOBALS['yke']->table(1,'users') . " WHERE user_id = '$user_id'"." and sysid ='".$GLOBALS['sysid']."' ";
    ...
    {
        $sql = "SELECT rank_id, rank_name, discount FROM ".$GLOBALS['yke']->table(1,'user_rank') .
               " WHERE rank_id = '$infos[user_rank]'"." and sysid ='".$GLOBALS['sysid']."' ";
    }
    else
    {
        $sql = "SELECT rank_id, rank_name, discount, min_points".
               " FROM ".$GLOBALS['yke']->table(1,'user_rank') .
               " WHERE min_points<= " . intval($infos['rank_points']) ." and sysid ='".$GLOBALS['sysid']."' ". " ORDER BY min_points DESC";
    }
    ...
    $sql = "SELECT type_name, type_money ".
           "FROM " .$GLOBALS['yke']->table(1,'bonus_type') . " AS t1, " .$GLOBALS['yke']->table(1,'user_bonus') . " AS t2 ".
           "WHERE t1.type_id = t2.bonus_type_id and t1.sysid=t2.sysid  AND t2.user_id = '$user_id' AND t1.use_start_date <= '$cur_date' ".
           "AND t1.use_end_date > '$cur_date' AND t2.order_id = 0"." and t1.sysid ='".$GLOBALS['sysid']."' ";
...
function get_consignee_list($user_id)
{
    $sql = "SELECT * FROM " . $GLOBALS['yke']->table(1,'user_address') .
            " WHERE user_id = '$user_id'"." and sysid ='".$GLOBALS['sysid']."' "." LIMIT 5";
...
function add_bonus($user_id, $bouns_sn)
{
    ...
    /* 查询红包序列号是否已经存在 */
    $sql = "SELECT bonus_id, bonus_sn, user_id, bonus_type_id FROM " .$GLOBALS['yke']->table(1,'user_bonus') .
           " WHERE bonus_sn = '$bouns_sn'"." and sysid ='".$GLOBALS['sysid']."' ";
    ...
    if ($row)
    {
        if ($row['user_id'] == 0)
        {
            ...
            $sql = "SELECT send_end_date, use_end_date ".
                   ...
                   " WHERE type_id = '" . $row['bonus_type_id'] . "'"." and sysid ='".$GLOBALS['sysid']."' ";
            ...
            $sql = "UPDATE " .$GLOBALS['yke']->table(1,'user_bonus') . " SET user_id = '$user_id' ".
                   "WHERE bonus_id = '$row[bonus_id]'"." and sysid ='".$GLOBALS['sysid']."' ";
...
function get_user_orders($user_id, $num = 10, $start = 0)
{
    ...
    $sql = "SELECT order_id, order_sn, order_status, shipping_status, pay_status, add_time, shipping_time, " .
           ...
           " WHERE user_id = '$user_id'"." and sysid ='".$GLOBALS['sysid']."' "." ORDER BY add_time DESC";
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        {
            $href='user.php?sysessid='.$GLOBALS['sysessid'].'&act=cancel_order&order_id=' .$row['order_id'];

            $row['handler'] = "<a href='".$href."' onclick='if (!confirm('".$GLOBALS['_LANG']['confirm_cancel']."')) return false;'>".$GLOBALS['_LANG']['cancel']."</a>";
        }
        else if ($row['order_status'] == OS_SPLITED)
        {
            ...
            {
                $href='user.php?sysessid='.$GLOBALS['sysessid'].'&act=affirm_received&order_id=' .$row['order_id'];
                @$row['handler'] = "<a href='".$href."' onclick='if (!confirm('".$GLOBALS['_LANG']['confirm_received']."')) return false;>".$GLOBALS['_LANG']['received']."</a>";
            }
            ...
            else
            {
                ...
                {
                    $href='user.php?sysessid='.$GLOBALS['sysessid'].'&act=order_detail&order_id='.$row['order_id'];
                    @$row['handler'] = "<a href='".$href."'>" .$GLOBALS['_LANG']['pay_money']. '</a>';
                }
                else
                {
                    $href='user.php?sysessid='.$GLOBALS['sysessid'].'&act=order_detail&order_id=' .$row['order_id'];
                    @$row['handler'] = "<a href='".$href."'>" .$GLOBALS['_LANG']['view_order']. '</a>';
                }
...
{
    /* 查询订单信息，检查状态 */
    $sql = "SELECT user_id, order_id, order_sn , surplus , integral , bonus_id, order_status, shipping_status, pay_status FROM " .$GLOBALS['yke']->table(1,'order_info') ." WHERE order_id = '$order_id'"." and sysid ='".$GLOBALS['sysid']."' ";
    ...
    //将用户订单设置为取消
    $sql = "UPDATE ".$GLOBALS['yke']->table(1,'order_info') ." SET order_status = '".OS_CANCELED."' WHERE order_id = '$order_id'"." and sysid ='".$GLOBALS['sysid']."' ";
...
{
    /* 查询订单信息，检查状态 */
    $sql = "SELECT user_id, order_sn , order_status, shipping_status, pay_status FROM ".$GLOBALS['yke']->table(1,'order_info') ." WHERE order_id = '$order_id'"." and sysid ='".$GLOBALS['sysid']."' ";
    ...
    {
        $sql = "UPDATE " . $GLOBALS['yke']->table(1,'order_info') . " SET shipping_status = '" . SS_RECEIVED . "',shipping_time_end = '" . gmtime() . "' WHERE order_id = '$order_id'"." and sysid ='".$GLOBALS['sysid']."' ";
        if ($GLOBALS['db']->query($sql))
        {
            /* 记录日志 */
...
function save_consignee($consignee, $default=false)
{
    ...
    {
        /* 修改地址 */
        $res = $GLOBALS['db']->autoExecute($GLOBALS['yke']->table(1,'user_address'), $consignee, 'UPDATE', 'address_id = ' . $consignee['address_id']." AND `user_id`= '".$_SESSION['user_id']."'"." and sysid ='".$GLOBALS['sysid']."' ");
    }
    else
    {
        /* 添加地址 */
        $consignee['sysid']= $GLOBALS['sysid'];
        $res = $GLOBALS['db']->autoExecute($GLOBALS['yke']->table(1,'user_address'), $consignee, 'INSERT');
    ...
    if ($default)
    {
        ...
        $sql = "UPDATE " . $GLOBALS['yke']->table(1,'users') .
            " SET address_id = '$consignee[address_id]' WHERE user_id = '$_SESSION[user_id]'"." and sysid ='".$GLOBALS['sysid']."' ";
...
{
    $sql = "SELECT user_id FROM " .$GLOBALS['yke']->table(1,'user_address') . " WHERE address_id = '$id'"." and sysid ='".$GLOBALS['sysid']."' ";
    ...
    {
        $sql = "DELETE FROM " .$GLOBALS['yke']->table(1,'user_address') . " WHERE address_id = '$id'"." and sysid ='".$GLOBALS['sysid']."' ";
        $res = $GLOBALS['db']->query($sql);
...
function update_address($address)
{
    ...
    {
         /* 更新指定记录 */
        $GLOBALS['db']->autoExecute($GLOBALS['yke']->table(1,'user_address'), $address, 'UPDATE', 'address_id = ' .$address_id . ' AND user_id = ' . $address['user_id']." and sysid ='".$GLOBALS['sysid']."' ");
    }
    else
    {
        /* 插入一条新记录 */
        $address['sysid']= $GLOBALS['sysid'];
        $GLOBALS['db']->autoExecute($GLOBALS['yke']->table(1,'user_address'), $address, 'INSERT');
    ...
    if (isset($address['defalut']) && $address['default'] > 0 && isset($address['user_id']))
    {
        $sql = "UPDATE ".$GLOBALS['yke']->table(1,'users') .
                ...
                " WHERE user_id = '" .$address['user_id']. "'"." and sysid ='".$GLOBALS['sysid']."' ";
...
function get_order_detail($order_id, $user_id = 0)
{
    ...
    {
         $shipping_code = $GLOBALS['db']->GetOne("SELECT shipping_code FROM ".$GLOBALS['yke']->table(1,'shipping') ." WHERE shipping_id = '$order[shipping_id]'"." and sysid ='".$GLOBALS['sysid']."' ");
    ...
    if ($order['pay_status'] != PS_UNPAYED)
    {
        ...
        foreach ($virtual_goods AS $code => $goods_list)
        {
            ...
            if ($code == 'package_buy')
            {
                foreach ($goods_list as $goods)
                {
                    $sql = 'SELECT g.goods_id FROM ' . $GLOBALS['yke']->table(1,'package_goods') . ' AS pg, ' . $GLOBALS['yke']->table(1,'goods') . ' AS g ' .
                           "WHERE pg.sysid =g.sysid and pg.goods_id = g.goods_id AND pg.package_id = '" . $goods['goods_id'] . "' AND extension_code = 'virtual_card'"." and sysid ='".$GLOBALS['sysid']."' ";
...
{
    include_once(ROOT_PATH . 'includes/lib_order.php');
    $sql  = "SELECT order_sn FROM ".$GLOBALS['yke']->table(1,'order_info') .
            " WHERE user_id  = '$user_id' " . order_query_sql('unprocessed') .
                "AND extension_code = '' "." and sysid ='".$GLOBALS['sysid']."' ".
...
function merge_user_order($from_order, $to_order, $user_id = 0)
{
    if ($user_id > 0)
    {
        ...
        {
            $sql = "SELECT user_id FROM " .$GLOBALS['yke']->table(1,'order_info').
                   " WHERE order_sn = '$to_order'"." and sysid ='".$GLOBALS['sysid']."' ";
...
function return_to_cart($order_id)
{
    ...
    $sql = "SELECT goods_id, product_id,goods_number, goods_attr, parent_id, goods_attr_id" .
            ...
            " WHERE order_id = '$order_id' AND is_gift = 0 AND extension_code <> 'package_buy'"." and sysid ='".$GLOBALS['sysid']."' " .
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $sql = "SELECT goods_sn, goods_name, goods_number, market_price, " .
                ...
                " WHERE goods_id = '$row[goods_id]' "." and sysid ='".$GLOBALS['sysid']."' " .
        ...
        if($row['product_id'])
        {
            ...
            $sql="SELECT product_number from ".$GLOBALS['yke']->table(1,'products')."where product_id='$order_goods_product_id'"." and sysid ='".$GLOBALS['sysid']."' ";
        ...
        $sql = "SELECT goods_number FROM" . $GLOBALS['yke']->table(1,'cart') . " " .
                ...
                "AND goods_id = '" . $row['goods_id'] . "' "." and sysid ='".$GLOBALS['sysid']."' " .
        ...
        if ($row['parent_id'] > 0)
        {
            ...
            $sql = "SELECT goods_id " .
                    "FROM " . $GLOBALS['yke']->table(1,'goods') .
                    " WHERE goods_id = '$row[parent_id]' "." and sysid ='".$GLOBALS['sysid']."' " .
            ...
            if ($parent)
            {
                ...
                $sql = "SELECT goods_price " .
                        ...
                        " WHERE parent_id = '$row[parent_id]' " ." and sysid ='".$GLOBALS['sysid']."' ".
        ...
        $sql = "SELECT goods_id " .
                ...
                " WHERE session_id = '" . SESS_ID . "' " ." and sysid ='".$GLOBALS['sysid']."' ".
        ...
        if (empty($cart_goods))
        {
            ...
            $return_goods['sysid']= $GLOBALS['sysid'];
            $GLOBALS['db']->autoExecute($GLOBALS['yke']->table(1,'cart'), $return_goods, 'INSERT');
        }
        else
        {
            // 有相同商品，修改数量
            $sql = "UPDATE " . $GLOBALS['yke']->table(1,'cart') . " SET " .
                    "goods_number = '" . $return_goods['goods_number'] . "' " .
                    ",goods_price = '" . $return_goods['goods_price'] . "' " .
                    "WHERE session_id = '" . SESS_ID . "' " .
                    "AND goods_id = '" . $return_goods['goods_id'] . "' " .
                    "AND rec_type = '" . CART_GENERAL_GOODS . "'"." and sysid ='".$GLOBALS['sysid']."' "." LIMIT 1";
            $GLOBALS['db']->query($sql);
        }
    }
    ...
    $sql = "DELETE FROM " . $GLOBALS['yke']->table(1,'cart') .
            " WHERE session_id = '" . SESS_ID . "' AND is_gift = 1"." and sysid ='".$GLOBALS['sysid']."' ";
    $GLOBALS['db']->query($sql);
...
function save_order_address($address, $user_id)
{
    ...
    /* 检查订单状态 */
    $sql = "SELECT user_id, order_status FROM " .$GLOBALS['yke']->table(1,'order_info'). " WHERE order_id = '" .$address['order_id']. "'"." and sysid ='".$GLOBALS['sysid']."' ";
    $row = $GLOBALS['db']->getRow($sql);
    if ($row)
    {
        ...
        $GLOBALS['db']->autoExecute($GLOBALS['yke']->table(1,'order_info'), $address, 'UPDATE', "order_id = '$address[order_id]'"." and sysid ='".$GLOBALS['sysid']."' ");
        return true;
    }
    else
    {
...
function get_user_bouns_list($user_id, $num = 10, $start = 0, $suppid=-1)
{
    $sql = "SELECT u.bonus_sn, u.order_id, u.supplier_id, b.type_name, b.type_money, b.min_goods_amount, b.use_start_date, b.use_end_date ".
           " FROM " .$GLOBALS['yke']->table(1,'user_bonus'). " AS u ,".
           $GLOBALS['yke']->table(1,'bonus_type'). " AS b".
           " WHERE u.bonus_type_id = b.type_id and u.sysid = b.sysid AND u.user_id = '" .$user_id. "'"." and u.sysid ='".$GLOBALS['sysid']."' ";
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        {
            $row['status'] = '<a href="user.php?sysessid='.$GLOBALS['sysessid'].'&act=order_detail&order_id=' .$row['order_id']. '" >' .$GLOBALS['_LANG']['had_use']. '</a>';
        }
        ...
        $row['supplier_name'] = $row['supplier_id']>0? $GLOBALS['db']->getOne("select value from ". $GLOBALS['yke']->table(1,'supplier_shop_config')." where code = 'shop_name' and supplier_id = '".$row['supplier_id']."'"." and sysid ='".$GLOBALS['sysid']."' "):'网站自营';
        $arr[] = $row;
    }
    return $arr;
