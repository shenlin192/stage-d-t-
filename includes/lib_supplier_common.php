...
function get_supplier_goods_count($suppid=0){
	...
	$sql="SELECT count(`goods_id`) FROM ".$GLOBALS['yke']->table(1,'goods')." as g WHERE  g.is_delete = 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_virtual = 0 AND g.supplier_id='$suppid'"." and sysid ='".$GLOBALS['sysid']."' ";
...
{
    $sql = 'SELECT * FROM '. $GLOBALS['yke']->table(1,'supplier_nav') . '
            WHERE ifshow = \'1\' AND supplier_id='.$_GET['suppId']." and sysid ='".$GLOBALS['sysid']."' ".' ORDER BY type, vieworder';
...
function load_config_supplier()
{
    ...
    if ($data === false)
    {
        $sql = 'SELECT code, value FROM ' . $GLOBALS['yke']->table(1,'supplier_shop_config') . ' WHERE parent_id > 0 AND supplier_id='.$suppid." and sysid ='".$GLOBALS['sysid']."' ";
...
function get_categories_tree_supplier($cat_id = 0)
{
    if ($cat_id > 0)
    {
        $sql = 'SELECT parent_id FROM ' . $GLOBALS['yke']->table(1,'supplier_category') . " WHERE cat_id = '$cat_id' AND supplier_id=".$_GET['suppId']." and sysid ='".$GLOBALS['sysid']."' ";
    ...
    /*
     ...
     如果不是取当前分类及其下的子分类
    */
    $sql = 'SELECT count(*) FROM ' . $GLOBALS['yke']->table(1,'supplier_category') . " WHERE parent_id = '$parent_id' AND is_show = 1 AND supplier_id=".$_GET['suppId']." and sysid ='".$GLOBALS['sysid']."' ";
    if ($GLOBALS['db']->getOne($sql) || $parent_id == 0)
    {
        /* 获取当前分类及其子分类 */
        $sql = 'SELECT cat_id,cat_name ,parent_id,is_show ' .
                'FROM ' . $GLOBALS['yke']->table(1,'supplier_category') .
                "WHERE parent_id = '$parent_id' AND is_show = 1 AND supplier_id=".$_GET['suppId']." and sysid ='".$GLOBALS['sysid']."' "." ORDER BY sort_order ASC, cat_id ASC";
...
{
    $three_arr = array();
    $sql = 'SELECT count(*) FROM ' . $GLOBALS['yke']->table(1,'supplier_category') . " WHERE parent_id = '$tree_id' AND is_show = 1 AND supplier_id=".$_GET['suppId']." and sysid ='".$GLOBALS['sysid']."' ";
    if ($GLOBALS['db']->getOne($sql) || $tree_id == 0)
    {
        $child_sql = 'SELECT cat_id, cat_name, parent_id, is_show ' .
                'FROM ' . $GLOBALS['yke']->table(1,'supplier_category') .
                "WHERE parent_id = '$tree_id' AND is_show = 1 AND supplier_id=".$_GET['suppId']." and sysid ='".$GLOBALS['sysid']."' "." ORDER BY sort_order ASC, cat_id ASC";
...
function cat_list_supplier($cat_id = 0, $selected = 0, $re_type = true, $level = 0, $is_show_all = true)
{
    ...
    if ($res === NULL)
    {
            ...
            $sql = "SELECT c.cat_id, c.cat_name, c.measure_unit, c.parent_id, c.is_show, c.show_in_nav, c.grade, c.sort_order, COUNT(s.cat_id) AS has_children ".
                'FROM ' . $GLOBALS['yke']->table(1,'supplier_category') . " AS c ".
                "LEFT JOIN " . $GLOBALS['yke']->table(1,'supplier_category') . " AS s ON s.parent_id=c.cat_id  and c.sysid=s.sysid where c.supplier_id = ".$suppid." and c.sysid ='".$GLOBALS['sysid']."' ".
                " GROUP BY c.cat_id ".
            ...
            $sql = "SELECT gc.cat_id, COUNT(*) AS goods_num " .
                    " FROM " . $GLOBALS['yke']->table(1,'supplier_goods_cat') . " AS gc , " . $GLOBALS['yke']->table(1,'goods') . " AS g " .
                    " WHERE gc.supplier_id = ".$suppid." AND g.goods_id = gc.goods_id and gc.sysid=g.sysid AND g.is_delete = 0 AND g.is_on_sale = 1 " ." and gc.sysid ='".$GLOBALS['sysid']."' ".
                    " GROUP BY gc.cat_id";
...
function get_top10_supplier($cats = '')
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.shop_price, g.market_price, g.original_img, g.goods_thumb, SUM(og.goods_number) as goods_number, g.supplier_id ' .
           ...
           "WHERE g.sysid=o.sysid and og.sysid=g.sysid and g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 $where $top10_time " ." and g.sysid ='".$GLOBALS['sysid']."' ";
    //判断是否启用库存，库存数量是否大于0
...
function get_extension_goods_supplier($cats)
{
    ...
    $suppid = (isset($_GET['suppId']) && intval($_GET['suppId'])>0) ? intval($_GET['suppId']) : $_SESSION['supplier_id'];
    $sql = 'SELECT goods_id FROM ' . $GLOBALS['yke']->table(1,'supplier_goods_cat') . " AS sgc WHERE $cats"." and sysid ='".$GLOBALS['sysid']."' ";
