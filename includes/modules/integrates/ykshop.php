...
if (isset($set_modules) && $set_modules == TRUE)
{
    ...
    $modules[$i]['code']    = 'ykshop';
    /* 被整合的第三方程序的名称 */
    $modules[$i]['name']    = 'ykshop';
    /* 被整合的第三方程序的版本 */
...
class ykshop extends integrate
{
    var $is_ykshop = 1;
    ...
    function check_user($username, $password = null)
    {
        ...
        else
        {
            $sql = "SELECT user_id, password, salt,ec_salt " .
                   " FROM " . $this->table($this->user_table).
                   " WHERE user_name='$post_username'"." and sysid ='".$GLOBALS['sysid']."' ";
            ...
            else
            {
                ...
                $sql = "UPDATE " . $this->table($this->user_table) .
                       ...
                       " WHERE user_id = '$row[user_id]'"." and sysid ='".$GLOBALS['sysid']."' ";
                $this->db->query($sql);
    ...
    function edit_user($cfg)
    {
    	...
    	if ((!empty($cfg['email'])) && $this->field_email != 'NULL')
    	{
    		...
    		" AND " . $this->field_name . " != '$cfg[post_username]'"." and sysid ='".$GLOBALS['sysid']."' ";
    		...
    		else
    		{
    			...
    			$sql = "SELECT count(*)" .
    					" FROM " . $this->table($this->user_table).
    					" WHERE " . $this->field_email . " = '$cfg[email]' "." and sysid ='".$GLOBALS['sysid']."' ";
    			if($this->db->getOne($sql, true) == 0)
    			{
    				// 新的E-mail
    				$cfg['email_validated'] = 0;
    	...
    	if ((!empty($cfg['mobile_phone'])) && $this->field_mobile_phone != 'NULL')
    	{
    		...
    		else
    		{
    			...
    			$sql = "SELECT count(*)" .
    					" FROM " . $this->table($this->user_table).
    					" WHERE " . $this->field_mobile_phone . " = '$cfg[mobile_phone]' "." and sysid ='".$GLOBALS['sysid']."' ";
    			if($this->db->getOne($sql, true) == 0)
    			{
    				// 新的E-mail
    				$cfg['mobile_validated'] = 0;
    	...
    	if ($values)
    	{
    		$sql = "UPDATE " . $this->table($this->user_table).
    		" SET " . implode(', ', $values).
    		" WHERE " . $this->field_name . "='" . $cfg['post_username'] . "'"." and sysid ='".$GLOBALS['sysid']."' "." LIMIT 1";
