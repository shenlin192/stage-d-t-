...
class discuz55 extends integrate
{
    ...
    function discuz55($cfg)
    {
        ...
        $key = $this->db->GetOne('SELECT value FROM ' . $this->table('settings') . " WHERE variable = 'authkey'"." and sysid ='".$GLOBALS['sysid']."' ");
    ...
    function get_points_name ()
    {
        ...
        if ($ava_credits === NULL)
        {
            $sql = "SELECT value FROM " . $this->table('settings') . " WHERE variable='extcredits'"." and sysid ='".$GLOBALS['sysid']."' ";
    ...
    function set_cookie ($username="")
    {
        ...
        else
        {
            ...
            $sql = "SELECT " . $this->field_id . " AS user_id, secques AS salt, " . $this->field_pass . " As password ".
                   " FROM " . $this->table($this->user_table) . " WHERE " . $this->field_name . "='$username'"." and sysid ='".$GLOBALS['sysid']."' ";
    ...
    function check_cookie ()
    {
        if (isset($_COOKIE[$this->cookie_prefix . 'auth']))
        {
            ...
            $sql = "SELECT " . $this->field_name ." AS user_name".
                   " FROM " . $this->table($this->user_table) .
                   " WHERE ".$this->field_id." = '$discuz_uid' AND ".$this->field_pass." = '$discuz_pw'"." and sysid ='".$GLOBALS['sysid']."' ";
    ...
    function add_user($username, $password, $email, $gender = -1, $bday = 0, $reg_date=0, $md5password='')
    {
        ...
        /* 获得默认的用户组 */
        $sql = 'SELECT groupid FROM ' .$this->table('usergroups'). ' WHERE creditshigher <= 0 AND creditslower > 0'." and sysid ='".$GLOBALS['sysid']."' ";
