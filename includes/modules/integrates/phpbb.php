...
class phpbb extends integrate
{
    ...
    function phpbb($cfg)
    {
        ...
        $this->cookie_prefix = $this->db->getOne("SELECT config_value FROM " .$this->table('config'). " WHERE config_name='cookie_name'"." and sysid ='".$GLOBALS['sysid']."' ");
    }
    ...
    function set_cookie ($username="")
    {
        ...
        else
        {
            ...
            {
                $username = yk_iconv('UTF8', $this->charset, $username);
            }

            $sql = "SELECT " .$this->field_id. " AS user_id, " .$this->field_name. " AS user_name, " .$this->field_email." AS email ".
                   " FROM " .$this->table($this->user_table).
                   " WHERE " .$this->field_name. " = '$username'"." and sysid ='".$GLOBALS['sysid']."' ";
            ...
            /* 向整合对象的数据表里写入cookie值 */
            $this->db->query("INSERT INTO " .$this->table('sessions_keys')." (sysid,key_id, user_id, last_login) ".
                             "VALUES ("."'".$GLOBALS['sysid']."',"."'" .$auto_login_key. "', '$row[user_id]', '".time()."')");

            $client_ip = ( !empty($HTTP_SERVER_VARS['REMOTE_ADDR']) ) ? $HTTP_SERVER_VARS['REMOTE_ADDR'] : ( ( !empty($HTTP_ENV_VARS['REMOTE_ADDR']) ) ? $HTTP_ENV_VARS['REMOTE_ADDR'] : getenv('REMOTE_ADDR') );
            $sql = "INSERT INTO ".$this->table('sessions')." (sysid,session_id, session_user_id, session_start, session_time, session_ip, session_logged_in, session_admin) VALUES("."'".$GLOBALS['sysid']."',"."'$auto_login_key', '".$row[$this->field_id]."','".time()."','".time()."','".$this->encode_ip($client_ip)."',1, 0)";
            $this->db->query($sql);
    ...
    function check_cookie ()
    {
        ...
        $sql = "SELECT " . $this->field_name .
               ...
               " LEFT JOIN " . $this->table($this->user_table) . " AS u ON s.session_user_id = u.user_id and s.sysid=u.sysid ".
               " WHERE session_id = '$cookie_session_id' AND session_user_id = '$cookie_data[userid]'"." and s.sysid ='".$GLOBALS['sysid']."' ";
        ...
        else
        {
            if ($this->charset != 'UTF8')
            {
                $username = yk_iconv($this->charset, 'UTF8', $username);
            }
    ...
    {
        $dss_seeded = false;
        $rand_seed = $this->db->getOne("SELECT config_value FROM " .$this->table('config'). " WHERE config_name = 'rand_seed'"." and sysid ='".$GLOBALS['sysid']."' ");
        ...
        if ($dss_seeded !== true)
        {
            $sql = "UPDATE ".$this->table('config')." SET config_value = '".$rand_seed."' WHERE config_name = 'rand_seed'"." and sysid ='".$GLOBALS['sysid']."' ";
