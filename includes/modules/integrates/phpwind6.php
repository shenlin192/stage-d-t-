...
class phpwind6 extends integrate
{
    ...
    function phpwind6 ($cfg)
    {
        ...
        /* 设置论坛的加密密钥 */
        $this->db_hash = $this->db->GetOne("SELECT `db_value` FROM ".$this->table('config')." WHERE `db_name` = 'db_hash'"." and sysid ='".$GLOBALS['sysid']."' ");
        $this->db_sitehash = $this->db->GetOne("SELECT `db_value` FROM ".$this->table('config')." WHERE `db_name` = 'db_sitehash'"." and sysid ='".$GLOBALS['sysid']."' ");
    }
    ...
    function check_cookie ()
    {
        ...
        $sql = "SELECT " .$this->field_id. " AS user_id, " . $this->field_name . " As user_name, ".
               ...
               " FROM ".$this->table($this->user_table).
               " WHERE " . $this->field_id . " = '$user_id'"." and sysid ='".$GLOBALS['sysid']."' ";
        $row = $this->db->getRow($sql);
    ...
    function get_points_name ()
    {
        ...
        if ($ava_credits === NULL)
        {
            $sql = "SELECT db_value FROM " . $this->table('config') . " WHERE db_name='db_credits'"." and sysid ='".$GLOBALS['sysid']."' ";
            ...
            if (empty($str))
            {
                ...
                {
                    $sql = "SELECT db_value FROM " . $this->table('config') . " WHERE db_name='".$name."unit'"." and sysid ='".$GLOBALS['sysid']."' ";
                    $ava_credits[$key]['unit'] = $this->db->getOne($sql);

                    $sql = "SELECT db_value FROM " . $this->table('config') . " WHERE db_name='".$name."name'"." and sysid ='".$GLOBALS['sysid']."' ";
    ...
    function get_points($username)
    {
        ...
        if ($fileds)
        {
            $sql = "SELECT ud." . $this->field_id . ', ' . implode(', ',$fileds).
                   ...
                   " WHERE ud.sysid=u.sysid and u." . $this->field_id . "= ud." .$this->field_id . " AND u." . $this->field_name . "='$username'"." and ud.sysid ='".$GLOBALS['sysid']."' ";
            $row = $this->db->getRow($sql);
            if (isset($row['rvrc']))
            {
    ...
    function set_points ($username, $credits)
    {
        ...
        if ($set)
        {
            ...
            $sql = "UPDATE " . $this->table('memberdata').
                   " SET " . implode(', ', $tmp).
                   " WHERE " . $this->field_id . " = '$uid'"." and sysid ='".$GLOBALS['sysid']."' ";
            $this->db->query($sql);
        }
