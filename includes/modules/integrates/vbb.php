...
class vbb extends integrate
{
    ...
    function add_user($username, $password, $email, $gender = -1, $bday = 0, $reg_date=0, $md5password='')
    {
        ...
        $user_title = $this->db->GetOne("SELECT title FROM " .$this->table('usertitle')." where sysid ='".$GLOBALS['sysid']."' ". " ORDER BY minposts LIMIT 1");
    ...
    function check_cookie ()
    {
        ...
        $row = $this->db->getRow("SELECT " . $this->field_name . " AS user_name, " . $this->field_pass . " As password ".
                                 " FROM " . $this->table($this->user_table) . " WHERE " . $this->field_id . "='$user_id'"." and sysid ='".$GLOBALS['sysid']."' ");
        if (empty($row))
        {
            return '';
        }
        ...
        if ($this->charset != 'UTF8')
        {
            $row['user_name'] = yk_iconv($this->charset, 'UTF8', $row['user_name']);
        }
    ...
    function check_user($username, $password = null)
    {
        if ($this->charset != 'UTF8')
        {
            $post_username = yk_iconv('UTF8', $this->charset, $username);
        }
        else
        {
        ...
        if ($password === null)
        {
            $sql = "SELECT " . $this->field_id .
                   " FROM " . $this->table($this->user_table).
                   " WHERE " . $this->field_name . "='" . $post_username . "'"." and sysid ='".$GLOBALS['sysid']."' ";
