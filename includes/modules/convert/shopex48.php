...
class shopex48
{
    ...
    function process_cat()
    {
        ...
        /* 查询分类并循环处理 */
        $sql = "SELECT * FROM ".$this->sprefix."goods_cat"." where sysid ='".$GLOBALS['sysid']."' ";
        ...
        /* 查询商品类型并循环处理 */
        $sql = "SELECT * FROM ".$this->sprefix."goods_type"." where sysid ='".$GLOBALS['sysid']."' ";
        $res = $this->sdb->query($sql);
        while ($row = $this->sdb->fetchRow($res))
        {
            $type = array();
    ...
    function process_brand()
    {
        ...
        /* 查询品牌并插入 */
        $sql = "SELECT * FROM ".$this->sprefix."brand"." where sysid ='".$GLOBALS['sysid']."' ";
        $res = $this->sdb->query($sql);
        while ($row = $this->sdb->fetchRow($res))
        {
            ...
            $brand = array(
                ...
                'brand_desc' => '',
                'site_url' => yk_iconv($this->scharset, $this->tcharset, addslashes($row['brand_url'])),
    ...
    function process_goods()
    {
        ...
        /* 取得商品分类对应的商品类型 */
        $cat_type_list = array();
        $sql = "SELECT cat_id, supplier_cat_id FROM ".$this->sprefix."goods_cat"." where sysid ='".$GLOBALS['sysid']."' ";
        $res = $this->sdb->query($sql);
        while ($row = $this->sdb->fetchRow($res))
        {
        ...
        /* 查询商品并处理 */
        $sql = "SELECT * FROM ".$this->sprefix."goods"." where sysid ='".$GLOBALS['sysid']."' ";
        $res = $this->sdb->query($sql);
        while ($row = $this->sdb->fetchRow($res))
        {
            $goods = array();
            ...
            /* 商品相册 */
            $sql2 = "SELECT * FROM ".$this->sprefix."gimages"." where sysid ='".$GLOBALS['sysid']."' ";
    ...
    function process_users()
    {
        ...
        /* 查询并插入会员等级 */
        $sql = "SELECT * FROM ".$this->sprefix."member_lv order by point desc"." where sysid ='".$GLOBALS['sysid']."' ";
        $res = $this->sdb->query($sql);
        $max_points = 50000;
        while ($row = $this->sdb->fetchRow($res))
        {
        ...
        /* 查询并插入会员 */
        $sql = "SELECT * FROM ".$this->sprefix."members"." where sysid ='".$GLOBALS['sysid']."' ";
        $res = $this->sdb->query($sql);
        while ($row = $this->sdb->fetchRow($res))
        {
            $user = array();
            ...
            $user['user_name']      = yk_iconv($this->scharset, $this->tcharset, addslashes($row['uname']));
        ...
        /* 收货人地址 */
        $sql = "SELECT * FROM ".$this->sprefix."member_addrs"." where sysid ='".$GLOBALS['sysid']."' ";
        $res = $this->sdb->query($sql);
        while ($row = $this->sdb->fetchRow($res))
        {
            $address = array();
            ...
            $address['address']         = yk_iconv($this->scharset, $this->tcharset, addslashes($row['addr']));
        ...
        /* 会员价格 */
        $temp_arr = array();
        $sql = "SELECT * FROM ".$this->sprefix."goods_lv_price"." where sysid ='".$GLOBALS['sysid']."' ";
        $res = $this->sdb->query($sql);
        while ($row = $this->sdb->fetchRow($res))
        {
        ...
        /* 帐户明细 */
        $sql = "SELECT * FROM ".$this->sprefix."advance_logs"." where sysid ='".$GLOBALS['sysid']."' ";
        $res = $this->sdb->query($sql);
        while ($row = $this->sdb->fetchRow($res))
        {
            $user_account = array();
    ...
    function process_article()
    {
        ...
        /* 文章 */
        $sql = "SELECT * FROM ".$this->sprefix."articles"." where sysid ='".$GLOBALS['sysid']."' ";
        $res = $this->sdb->query($sql);
        while ($row = $this->sdb->fetchRow($res))
        {
            $article = array();
            ...
            $article['content']     = yk_iconv($this->scharset, $this->tcharset, addslashes($row['content']));
        ...
        /* 友情链接 */
        $sql = "SELECT * FROM ".$this->sprefix."link"." where sysid ='".$GLOBALS['sysid']."' ";
        $res = $this->sdb->query($sql);
        while ($row = $this->sdb->fetchRow($res))
        {
            $link = array();
            ...
            $link['link_name']   = yk_iconv($this->scharset, $this->tcharset, addslashes($row['link_name']));
    ...
    function process_order()
    {
        ...
        /* 订单 */
        $sql = "SELECT o.* FROM ".$this->sprefix."orders AS o "." where sysid ='".$GLOBALS['sysid']."' " ;
        $res = $this->sdb->query($sql);
        while ($row = $this->sdb->fetchRow($res))
        {
            $order = array();
            ...
            $order['consignee']         = yk_iconv($this->scharset, $this->tcharset, addslashes($row['consignee']));
            $order['address']           = yk_iconv($this->scharset, $this->tcharset, addslashes($row['ship_addr']));
            ...
            $order['shipping_name']     = yk_iconv($this->scharset, $this->tcharset, addslashes($row['ship_name']));
            $order['pay_name']          = yk_iconv($this->scharset, $this->tcharset, addslashes($row['shipping']));
            $order['inv_payee']         = yk_iconv($this->scharset, $this->tcharset, addslashes($row['tax_company']));
    ...
    function process_config()
    {
        ...
        /* 查询设置 */
        $sql = "SELECT * FROM ".$this->sprefix."settings"." where sysid ='".$GLOBALS['sysid']."' ";
        $row = $this->sdb->getRow($sql);
        $store = $row['store'];
        $store_arr = unserialize($store);
        $config = array();
        //$config['shop_name']        = yk_iconv($this->scharset, $this->tcharset, addslashes($store_arr[0]);
        //$config['shop_title']       = yk_iconv($this->scharset, $this->tcharset, addslashes($store_arr[0]));
        //$config['shop_desc']        = yk_iconv($this->scharset, $this->tcharset, addslashes($store_arr[1]));
        //$config['shop_address']     = yk_iconv($this->scharset, $this->tcharset, addslashes($row['store']));
