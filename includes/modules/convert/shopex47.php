...
if (isset($set_modules) && $set_modules == TRUE)
{
    ...
    /* 作者 */
    $modules[$i]['author'] = 'ykshop R&D TEAM';
...
class shopex47
{
    ...
    function process_cat()
    {
        ...
        /* 查询分类并循环处理 */
        $sql = "SELECT * FROM ".$this->sprefix."mall_offer_pcat"." where sysid ='".$GLOBALS['sysid']."' ";
        $res = $this->sdb->query($sql);
        while ($row = $this->sdb->fetchRow($res))
        {
            $cat = array();
            $cat['sysid']= $GLOBALS['sysid'];
            ...
            /* 插入分类 */
            if (!$db->autoExecute($yke->table(1,'category'), $cat, 'INSERT', '', 'SILENT'))
            {
                //return $db->error();
            }
        }
        ...
        /* 查询商品类型并循环处理 */
        $sql = "SELECT * FROM ".$this->sprefix."mall_prop_category"." where sysid ='".$GLOBALS['sysid']."' ";
        $res = $this->sdb->query($sql);
        while ($row = $this->sdb->fetchRow($res))
        {
            $type = array();
            $type['sysid']= $GLOBALS['sysid'];
            $type['cat_id']     = $row['prop_cat_id'];
            $type['cat_name']   = $row['cat_name'];
            $type['enabled']    = '1';
            if (!$db->autoExecute($yke->table(1,'goods_type'), $type, 'INSERT', '', 'SILENT'))
            {
                //return $db->error();
            }
        }
        ...
        /* 查询属性值并循环处理 */
        $sql = "SELECT * FROM ".$this->sprefix."mall_prop WHERE prop_type = 'propvalue'"." and sysid ='".$GLOBALS['sysid']."' ";
        $res = $this->sdb->query($sql);
        while ($row = $this->sdb->fetchRow($res))
        {
            $attr = array();
            $attr['sysid']= $GLOBALS['sysid'];
            ...
            $sql = "SELECT DISTINCT prop_value FROM ".$this->sprefix."mall_prop_value WHERE prop_id = '$row[prop_id]'"." and sysid ='".$GLOBALS['sysid']."' ";
            $attr['attr_values']= join("\n", $this->sdb->getCol($sql));
            if (!$db->autoExecute($yke->table(1,'attribute'), $attr, 'INSERT', '', 'SILENT'))
            {
                 //return $db->error();
            }
        }
    ...
    function process_brand()
    {
        ...
        /* 查询品牌并插入 */
        $sql = "SELECT * FROM ".$this->sprefix."mall_brand"." where sysid ='".$GLOBALS['sysid']."' ";
        $res = $this->sdb->query($sql);
        while ($row = $this->sdb->fetchRow($res))
        {
            $brand = array(
                ...
                'brand_desc' => '',
                'site_url' => yk_iconv($this->scharset, $this->tcharset, addslashes($row['brand_site_url'])),
                'brand_logo' => yk_iconv($this->scharset, $this->tcharset, addslashes($row['brand_logo']))
            );
            if (!$db->autoExecute($yke->table(1,'brand'), $brand, 'INSERT', '', 'SILENT'))
            {
                //return $db->error();
            }
        }
    ...
    function process_goods()
    {
        ...
        /* 查询品牌列表 name => id */
        $brand_list = array();
        $sql = "SELECT brand_id, brand_name FROM " . $yke->table(1,'brand')." where sysid ='".$GLOBALS['sysid']."' ";
        $res = $db->query($sql);
        ...
        /* 取得商店设置 */
        $sql = "SELECT offer_pointtype, offer_pointnum FROM ".$this->sprefix."mall_offer WHERE offerid = '1'"." and sysid ='".$GLOBALS['sysid']."' ";
        $config = $this->sdb->getRow($sql);

        /* 取得商品分类对应的商品类型 */
        $cat_type_list = array();
        $sql = "SELECT catid, prop_cat_id FROM ".$this->sprefix."mall_offer_pcat"." where sysid ='".$GLOBALS['sysid']."' ";
        $res = $this->sdb->query($sql);
        while ($row = $this->sdb->fetchRow($res))
        {
        ...
        /* 查询商品并处理 */
        $sql = "SELECT * FROM ".$this->sprefix."mall_goods"." where sysid ='".$GLOBALS['sysid']."' ";
        $res = $this->sdb->query($sql);
        while ($row = $this->sdb->fetchRow($res))
        {
            $goods = array();
            ...
            $goods['goods_sn']      = yk_iconv($this->scharset, $this->tcharset, addslashes($row['bn']));
            $goods['goods_name']    = $row['goods'];
            $goods['brand_id']      = trim($row['brand']) == '' ? '0' : $brand_list[yk_iconv($this->scharset, $this->tcharset, addslashes($row['brand']))];
            ...
            $goods['goods_brief']   = yk_iconv($this->scharset, $this->tcharset, addslashes($row['intro']));
            $goods['goods_desc']    = str_replace('pictures/newsimg/', $this->tdocroot . '/images/upload/Image/', yk_iconv($this->scharset, $this->tcharset, addslashes($row['memo'])));
            ...
            $goods['sysid']= $GLOBALS['sysid'];
            /* 插入 */
            if (!$db->autoExecute($yke->table(1,'goods'), $goods, 'INSERT', '', 'SILENT'))
            {
                //return $db->error();
            }
            ...
            {
                $goods_cat = array();
                $goods_cat['sysid']= $GLOBALS['sysid'];
                $goods_cat['goods_id'] = $row['gid'];
                $cat_id_list = explode(',', trim($row['linkclass'], ','));
                foreach ($cat_id_list as $cat_id)
                {
                    $goods_cat['cat_id'] = $cat_id;
                    if (!$db->autoExecute($yke->table(1,'goods_cat'), $goods_cat, 'INSERT', '', 'SILENT'))
                    {
                        //return $db->error();
                    }
                }
            ...
            $sql = "SELECT DISTINCT pv.prop_id, pv.prop_value " .
                    ...
                    "WHERE gp.prop_value_id = pv.prop_value_id and sp.sysid=pv.sysid " .
                    "AND gp.gid = '$row[gid]'"." and sp.sysid ='".$GLOBALS['sysid']."' ";
            $res1 = $this->sdb->query($sql);
            while ($attr = $this->sdb->fetchRow($res1))
            {
                $goods_attr = array();
                $goods_attr['sysid']= $GLOBALS['sysid'];
                $goods_attr['goods_id']     = $row['gid'];
                $goods_attr['attr_id']      = $attr['prop_id'];
                $goods_attr['attr_value']   = yk_iconv($this->scharset, $this->tcharset, addslashes($attr['prop_value']));
                $goods_attr['attr_price']   = '0';
                if (!$db->autoExecute($yke->table(1,'goods_attr'), $goods_attr, 'INSERT', '', 'SILENT'))
                {
                    //return $db->error();
                }
            }
            ...
            {
                $goods_gallery = array();
                $goods_gallery['sysid']= $GLOBALS['sysid'];
                ...
                foreach ($img_list as $img)
                {
                    ...
                    if (!$db->autoExecute($yke->table(1,'goods_gallery'), $goods_gallery, 'INSERT', '', 'SILENT'))
                    {
                        //return $db->error();
                    }
                }
        ...
        /* 关联商品 */
        $sql = "SELECT * FROM ".$this->sprefix."mall_offer_linkgoods"." where sysid ='".$GLOBALS['sysid']."' ";
        $res = $this->sdb->query($sql);
        while ($row = $this->sdb->fetchRow($res))
        {
            $link_goods = array();
            $link_goods['sysid']= $GLOBALS['sysid'];
            $link_goods['goods_id']         = $row['pgid'];
            $link_goods['link_goods_id']    = $row['sgid'];
            $link_goods['is_double']        = $row['type'];

            if (!$db->autoExecute($yke->table(1,'link_goods'), $link_goods, 'INSERT', '', 'SILENT'))
            {
                //return $db->error();
            }
        ...
        /* 组合商品 */
        $sql = "SELECT DISTINCT gid, prop_goods_id, price FROM ".$this->sprefix."mall_pcat_prop_has_goods"." where sysid ='".$GLOBALS['sysid']."' ";
        $res = $this->sdb->query($sql);
        while ($row = $this->sdb->fetchRow($res))
        {
            $group_goods = array();
            $group_goods['sysid']= $GLOBALS['sysid'];
            $group_goods['parent_id']   = $row['gid'];
            $group_goods['goods_id']    = $row['prop_goods_id'];
            $group_goods['goods_price'] = $row['price'];

            if (!$db->autoExecute($yke->table(1,'group_goods'), $group_goods, 'INSERT', '', 'SILENT'))
            {
                //return $db->error();
            }
        }
    ...
    function process_users()
    {
        ...
        /* 查询并插入会员等级 */
        $sql = "SELECT * FROM ".$this->sprefix."mall_member_level "." where sysid ='".$GLOBALS['sysid']."' "." order by point desc";
        $res = $this->sdb->query($sql);
        $max_points = 50000;
        while ($row = $this->sdb->fetchRow($res))
        {
            $user_rank = array();
            $user_rank['sysid']= $GLOBALS['sysid'];
            $user_rank['rank_id']       = $row['levelid'];
            $user_rank['rank_name']     = yk_iconv($this->scharset, $this->tcharset, addslashes($row['name']));
            ...
            if (!$db->autoExecute($yke->table(1,'user_rank'), $user_rank, 'INSERT', '', 'SILENT'))
            {
                //return $db->error();
            }
        ...
        /* 查询并插入会员 */
        $sql = "SELECT * FROM ".$this->sprefix."mall_member"." where sysid ='".$GLOBALS['sysid']."' ";
        $res = $this->sdb->query($sql);
        while ($row = $this->sdb->fetchRow($res))
        {
            $user = array();
            $user['sysid']= $GLOBALS['sysid'];
            $user['user_id']        = $row['userid'];
            $user['email']          = $row['email'];
            $user['user_name']      = yk_iconv($this->scharset, $this->tcharset, addslashes($row['user']));
            $user['password']       = $row['password'];
            $user['question']       = yk_iconv($this->scharset, $this->tcharset, addslashes($row['pw_question']));
            $user['answer']         = yk_iconv($this->scharset, $this->tcharset, addslashes($row['pw_answer']));
            $user['sex']            = $row['sex'];
            ...
            if (!$db->autoExecute($yke->table(1,'users'), $user, 'INSERT', '', 'SILENT'))
            {
                //return $db->error();
            }
        ...
        /* 收货人地址 */
        $sql = "SELECT * FROM ".$this->sprefix."mall_member_receiver"." where sysid ='".$GLOBALS['sysid']."' ";
        $res = $this->sdb->query($sql);
        while ($row = $this->sdb->fetchRow($res))
        {
            $address = array();
            $address['sysid']= $GLOBALS['sysid'];
            $address['address_id']      = $row['receiveid'];
            $address['address_name']    = yk_iconv($this->scharset, $this->tcharset, addslashes($row['name']));
            $address['user_id']         = $row['memberid'];
            $address['consignee']       = yk_iconv($this->scharset, $this->tcharset, addslashes($row['name']));
            $address['email']           = $row['email'];
            $address['address']         = yk_iconv($this->scharset, $this->tcharset, addslashes($row['address']));
            $address['zipcode']         = $row['zipcode'];
            $address['tel']             = $row['telphone'];
            $address['mobile']          = $row['mobile'];

            if (!$db->autoExecute($yke->table(1,'user_address'), $address, 'INSERT', '', 'SILENT'))
            {
                //return $db->error();
            }
        }
        ...
        /* 会员价格 */
        $temp_arr = array();
        $sql = "SELECT * FROM ".$this->sprefix."mall_member_price"." where sysid ='".$GLOBALS['sysid']."' ";
        $res = $this->sdb->query($sql);
        while ($row = $this->sdb->fetchRow($res))
        {
            if ($row['gid'] > 0 && $row['levelid'] > 0 && !isset($temp_arr[$row['gid']][$row['levelid']]))
            {
                ...
                $member_price = array();
                $member_price['sysid']= $GLOBALS['sysid'];
                $member_price['goods_id']   = $row['gid'];
                $member_price['user_rank']  = $row['levelid'];
                $member_price['user_price'] = $row['price'];

                if (!$db->autoExecute($yke->table(1,'member_price'), $member_price, 'INSERT', '', 'SILENT'))
                {
                    //return $db->error();
                }
            }
        ...
        /* 帐户明细 */
        $sql = "SELECT * FROM ".$this->sprefix."mall_member_advance"." where sysid ='".$GLOBALS['sysid']."' ";
        $res = $this->sdb->query($sql);
        while ($row = $this->sdb->fetchRow($res))
        {
            $user_account = array();
            $user_account['sysid']= $GLOBALS['sysid'];
            ...
            if (!$db->autoExecute($yke->table(1,'user_account'), $user_account, 'INSERT', '', 'SILENT'))
            {
                //return $db->error();
            }
        }
    ...
    function process_article()
    {
        ...
        /* 文章类型 */
        $sql = "SELECT * FROM ".$this->sprefix."mall_offer_ncat"." where sysid ='".$GLOBALS['sysid']."' ";
        $res = $this->sdb->query($sql);
        while ($row = $this->sdb->fetchRow($res))
        {
            $cat = array();
            $cat['sysid']= $GLOBALS['sysid'];
            $cat['cat_id']      = $row['catid'];
            $cat['cat_name']    = yk_iconv($this->scharset, $this->tcharset, addslashes($row['cat']));
            $cat['cat_type']    = '1';
            $cat['sort_order']  = $row['pid'];
            $cat['is_open']     = '1';

            if (!$db->autoExecute($yke->table(1,'article_cat'), $cat, 'INSERT', '', 'SILENT'))
            {
                //return $db->error();
            }
        }
        ...
        /* 文章 */
        $sql = "SELECT * FROM ".$this->sprefix."mall_offer_ncon"." where sysid ='".$GLOBALS['sysid']."' ";
        $res = $this->sdb->query($sql);
        while ($row = $this->sdb->fetchRow($res))
        {
            $article = array();
            $article['sysid']= $GLOBALS['sysid'];
            $article['article_id']  = $row['newsid'];
            $article['cat_id']      = $row['catid'];
            $article['title']       = yk_iconv($this->scharset, $this->tcharset, addslashes($row['title']));
            $article['content']     = yk_iconv($this->scharset, $this->tcharset, addslashes($row['con']));
            ...
            if (!$db->autoExecute($yke->table(1,'article'), $article, 'INSERT', '', 'SILENT'))
            {
                //return $db->error();
            }
        }
        ...
        /* 友情链接 */
        $sql = "SELECT * FROM ".$this->sprefix."mall_offer_link"." where sysid ='".$GLOBALS['sysid']."' ";
        $res = $this->sdb->query($sql);
        while ($row = $this->sdb->fetchRow($res))
        {
            $link = array();
            $link['sysid']= $GLOBALS['sysid'];
            $link['link_id']     = $row['linkid'];
            $link['link_name']   = yk_iconv($this->scharset, $this->tcharset, addslashes($row['linktitle']));
            ...
            if (!$db->autoExecute($yke->table(1,'friend_link'), $link, 'INSERT', '', 'SILENT'))
            {
                //return $db->error();
            }
        }
    ...
    function process_order()
    {
        ...
        $sql = "SELECT o.*, t.tmethod, p.payment FROM ".$this->sprefix."mall_orders AS o " .
                "LEFT JOIN ".$this->sprefix."mall_offer_t AS t ON o.ttype = t.id and o.sysid=t.sysid " .
                "LEFT JOIN ".$this->sprefix."mall_offer_p AS p ON o.ptype = p.id and o.sysid=p.sysid "." where o.sysid ='".$GLOBALS['sysid']."' ";
        $res = $this->sdb->query($sql);
        while ($row = $this->sdb->fetchRow($res))
        {
            $order = array();
            ...
            $order['consignee']         = yk_iconv($this->scharset, $this->tcharset, addslashes($row['name']));
            $order['address']           = yk_iconv($this->scharset, $this->tcharset, addslashes($row['addr']));
            $order['zipcode']           = $row['zip'];
            $order['tel']               = $row['tel'];
            $order['mobile']            = $row['mobile'];
            $order['email']             = $row['email'];
            $order['postscript']        = yk_iconv($this->scharset, $this->tcharset, addslashes($row['memo']));
            $order['shipping_name']     = is_null($row['tmethod']) ? ' ' : yk_iconv($this->scharset, $this->tcharset, addslashes($row['tmethod']));
            $order['pay_name']          = is_null($row['payment']) ? ' ' : yk_iconv($this->scharset, $this->tcharset, addslashes($row['payment']));
            $order['inv_payee']         = yk_iconv($this->scharset, $this->tcharset, addslashes($row['invoiceform']));
            ...
            if (!$db->autoExecute($yke->table(1,'order_info'), $order, 'INSERT', '', 'SILENT'))
            {
                //return $db->error();
            }
            ...
            $sql = "SELECT i.*, g.priceintro FROM ".$this->sprefix."mall_items AS i " .
                    "LEFT JOIN ".$this->sprefix."mall_goods AS g ON i.gid = g.gid and i.sysid=g.sysid " .
                    "WHERE orderid = '$row[orderid]'"." and i.sysid ='".$GLOBALS['sysid']."' ";
            $res1 = $this->sdb->query($sql);
            while ($row = $this->sdb->fetchRow($res1))
            {
                ...
                $goods['goods_id']      = $row['gid'];
                $goods['goods_name']    = yk_iconv($this->scharset, $this->tcharset, addslashes($row['goods']));
                $goods['goods_sn']      = yk_iconv($this->scharset, $this->tcharset, addslashes($row['bn']));
                ...
                if (!$db->autoExecute($yke->table(1,'order_goods'), $goods, 'INSERT', '', 'SILENT'))
                {
                    //return $db->error();
                }
            }
    ...
    function process_config()
    {
        ...
        $sql = "SELECT * FROM ".$this->sprefix."mall_offer " .
                "WHERE offerid = '1'"." and sysid ='".$GLOBALS['sysid']."' ";
        $row = $this->sdb->getRow($sql);

        $config = array();
        $config['shop_name']        = yk_iconv($this->scharset, $this->tcharset, addslashes($row['offer_name']));
        $config['shop_title']       = yk_iconv($this->scharset, $this->tcharset, addslashes($row['offer_shoptitle']));
        $config['shop_desc']        = yk_iconv($this->scharset, $this->tcharset, addslashes($row['offer_metadesc']));
        $config['shop_address']     = yk_iconv($this->scharset, $this->tcharset, addslashes($row['offer_addr']));
        $config['service_email']    = $row['offer_email'];
        $config['service_phone']    = $row['offer_tel'];
        $config['icp_number']       = yk_iconv($this->scharset, $this->tcharset, addslashes($row['offer_certtext']));
        ...
        {
            $sql = "UPDATE " . $yke->table(1,'ykesmart_shop_config',1) . " SET " .
                    "value = '$value' " .
                    "WHERE code = '$code'"." and sysid ='".$GLOBALS['sysid']."' "." LIMIT 1";
