...
if (isset($set_modules) && $set_modules == TRUE)
{
	...
	/* 作者 */
	$modules[$i]['author']  = 'QQ:458445463';
	
	/* 网址 */
	$modules[$i]['website'] = 'http://www.yikaiqiche.cn';
...
class weixin
{
	...
	function get_code($order, $payment)
	{
		...
		$root_url = str_replace('mobile/', '', $GLOBALS['yke']->url());
		$return_url=$root_url.'wx_native_callback.php';
		...
		$jsApi = new JsApi_pub ();
		if($_SESSION['user_id']){
			$openid = $GLOBALS['db']->getOne("select fake_id from " . $GLOBALS['yke']->table(1,'weixin_user') . " where ecuid='{$_SESSION['user_id']}'"." and sysid ='".$GLOBALS['sysid']."' ");
		}
		...
		if(!$openid){
			if (! isset ( $_GET ['code'] )) {
				...
				Header ( 'Location:'.$uri.'?sysessid='.$GLOBALS['sysessid']);//.'\n'exit;
			} else {
				// 获取code码，以获取openid
				$code = $_GET ['code'];
				$jsApi->setCode ( $code );
		...
		$jsApiParameters = $jsApi->getParameters();
            ...
            if($allow_use_wxPay)
            {
                ...
                $html .= "function(res){";
                $html .= "if(res.err_msg == 'get_brand_wcpay_request:ok'){window.location.href='".$GLOBALS['yke']->url()."respond.php'}";
            ...
            $html .= '</script>';
            $html .= '<button type="button" onclick="callpay()" class="sub-btn">微信支付</button>';
	...
	function respond() {
		...
		if ($notify->checkSign () == TRUE) {
			if ($notify->data ["return_code"] == "FAIL") {
				...
				$log_id = $GLOBALS ['db']->getOne ( "SELECT log_id FROM " . $GLOBALS ['YKE']->table ( 'pay_log' ) . "where order_id='{$order_sn}' and is_paid=0"." and sysid ='".$GLOBALS['sysid']."' "." order by log_id desc" );
				/* 检查支付的金额是否相符 */
	...
	function addLog($other = array(), $type = 1) {
		...
		$log ['other'] = $other;
		$log = serialize ( $log );
		return $GLOBALS ['db']->query ( "INSERT INTO " . $GLOBALS['yke']->table(1,'weixin_paylog') . " (`sysid`,`log`,`type`) VALUES ("."'".$GLOBALS['sysid']."',"."'$log','$type')" );
	}
	// 生成原生支付二维码
	function natpayHtml($order) {
		if (! strpos ( $_SERVER ['HTTP_USER_AGENT'], 'MicroMessenger' )) {
			...
			if ($unifiedOrderResult["return_code"] == "FAIL") {
				...
				$log_id = $GLOBALS ['db']->getOne ( "SELECT log_id FROM " . $GLOBALS ['YKE']->table ( 'pay_log' ) . "where order_id='{$order ['order_id']}' and is_paid=0 "." and sysid ='".$GLOBALS['sysid']."' "." order by log_id desc" );
			...
			$href1='http://qr.liantu.com/api.php?sysessid='.$GLOBALS['sysessid'].'&text=' . $product_url . '';
			$href2='weixin_order_check.php?sysessid='.$GLOBALS['sysessid']."&oid={$order['order_id']}";
			return "<img src='".$href1."' alt='扫描进行支付'><iframe src='".$href2."' style='display:none'></iframe>";
		}
	}
}
?>