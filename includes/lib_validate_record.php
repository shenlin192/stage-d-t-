...
function save_validate_record ($key, $code, $type, $last_send_time, $expired_time, $ext_info = array())
{
	...
	//$_SESSION[VT_EMAIL_REGISTER] = $key;
	if(! $exist)
	{
		$record['record_key'] = $key;
		// 记录创建时间
		$record["create_time"] = time();
		$record['sysid']= $GLOBALS['sysid'];
		/* insert */
		$GLOBALS['db']->autoExecute($GLOBALS['yke']->table(1,'validate_record'), $record, 'INSERT');
	}
	else
	{
		/* update */
		$GLOBALS['db']->autoExecute($GLOBALS['yke']->table(1,'validate_record'), $record, 'UPDATE', "record_key = '$key'"." and sysid ='".$GLOBALS['sysid']."' ");
	}
}
...
{
	$sql = "select count(*) from " . $GLOBALS['yke']->table(1,'validate_record') . " where record_key = '" . $key . "'"." and sysid ='".$GLOBALS['sysid']."' ";
...
{
	$current_time = time();
	$sql = "delete from " . $GLOBALS['yke']->table(1,'validate_record') . " where expired_time < '$current_time'"." and sysid ='".$GLOBALS['sysid']."' ";
...
{
	$sql = "delete from " . $GLOBALS['yke']->table(1,'validate_record') . " where record_key = '$key'"." and sysid ='".$GLOBALS['sysid']."' ";
	return $GLOBALS['db']->query($sql);
}
...
{
	$sql = "select * from " . $GLOBALS['yke']->table(1,'validate_record') . " where record_key = '$key'"." and sysid ='".$GLOBALS['sysid']."' ";
	...
	//$_SESSION[VT_EMAIL_REGISTER] = $row['record_key'];
	$record = array(
		// 验证代码
