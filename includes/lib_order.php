...
function shipping_list()
{
    $sql = 'SELECT shipping_id, shipping_name ' .
            'FROM ' . $GLOBALS['yke']->table(1,'shipping') .
            ' WHERE enabled = 1'." and sysid ='".$GLOBALS['sysid']."' ";
...
{
    $sql = 'SELECT * FROM ' . $GLOBALS['yke']->table(1,'shipping') .
            " WHERE shipping_id = '$shipping_id' " .
            'AND enabled = 1'." and sysid ='".$GLOBALS['sysid']."' ";
...
function available_shipping_list($region_id_list,$suppid=0,$is_default_show=1,$shipping_code=null)
{
    ...
    $sql = 'SELECT DISTINCT s.shipping_id, s.shipping_code, s.shipping_name, ' .
            ...
            'FROM ' . $GLOBALS['yke']->table(1,'shipping') . ' AS s, ' .
                $GLOBALS['yke']->table(1,'shipping_area') . ' AS a, ' .
                $GLOBALS['yke']->table(1,'area_region') . ' AS r ' .
            'WHERE s.sysid=a.sysid and s.sysid=r.sysid and r.shipping_area_id = a.shipping_area_id AND a.shipping_id = s.shipping_id AND s.enabled = 1 ' . $where ." and s.sysid ='".$GLOBALS['sysid']."' ". ' ORDER BY s.shipping_order desc';
...
function shipping_area_info($shipping_id, $region_id_list)
{
    $sql = 'SELECT s.shipping_code, s.shipping_name, ' .
            ...
            'FROM ' . $GLOBALS['yke']->table(1,'shipping') . ' AS s, ' .
                $GLOBALS['yke']->table(1,'shipping_area') . ' AS a, ' .
                $GLOBALS['yke']->table(1,'area_region') . ' AS r ' .
            "WHERE s.sysid=a.sysid and s.sysid=r.sysid and s.shipping_id = '$shipping_id' " .
            'AND r.region_id ' . db_create_in($region_id_list) .
            ' AND r.shipping_area_id = a.shipping_area_id AND a.shipping_id = s.shipping_id AND s.enabled = 1'." and s.sysid ='".$GLOBALS['sysid']."' ";
...
function payment_list()
{
    $sql = 'SELECT pay_id, pay_name ' .
            'FROM ' . $GLOBALS['yke']->table(1,'ykesmart_payment') .
            ' WHERE enabled = 1'." and sysid ='".$GLOBALS['sysid']."' ";
...
{
    $sql = 'SELECT * FROM ' . $GLOBALS['yke']->table(1,'ykesmart_payment') .
            " WHERE pay_id = '$pay_id' AND enabled = 1"." and sysid ='".$GLOBALS['sysid']."' ";
...
function pack_list()
{
    $sql = 'SELECT * FROM ' . $GLOBALS['yke']->table(1,'pack')." where sysid ='".$GLOBALS['sysid']."' ";
    $res = $GLOBALS['db']->query($sql);
...
{
    $sql = "SELECT * FROM " . $GLOBALS['yke']->table(1,'pack') .
            " WHERE pack_id = '$pack_id'"." and sysid ='".$GLOBALS['sysid']."' ";
...
function card_list()
{
    $sql = "SELECT * FROM " . $GLOBALS['yke']->table(1,'card')." where sysid ='".$GLOBALS['sysid']."' ";
    $res = $GLOBALS['db']->query($sql);
...
{
    $sql = "SELECT * FROM " . $GLOBALS['yke']->table(1,'card') .
            " WHERE card_id = '$card_id'"." and sysid ='".$GLOBALS['sysid']."' ";
...
function order_info($order_id, $order_sn = '')
{
    ...
    if ($order_id > 0)
    {
        $sql = "SELECT *, " . $total_fee . " FROM " . $GLOBALS['yke']->table(1,'order_info') .
                " WHERE order_id = '$order_id'"." and sysid ='".$GLOBALS['sysid']."' ";
    }
    else
    {
        $sql = "SELECT *, " . $total_fee . "  FROM " . $GLOBALS['yke']->table(1,'order_info') .
                " WHERE order_sn = '$order_sn'"." and sysid ='".$GLOBALS['sysid']."' ";
    }
...
function order_goods($order_id)
{
    $sql = "SELECT rec_id, goods_id, goods_name, goods_sn, market_price, goods_number, " .
            ...
            "FROM " . $GLOBALS['yke']->table(1,'order_goods') .
            " WHERE order_id = '$order_id'"." and sysid ='".$GLOBALS['sysid']."' ";
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $row['goods_thumb'] = $GLOBALS['db']->getOne("select goods_thumb from ". $GLOBALS['yke']->table(1,'goods') ." where goods_id = ".$row['goods_id']." and sysid ='".$GLOBALS['sysid']."' ");
        $goods_list[] = $row;
    }
...
function order_weight_price($order_id)
{
    $sql = "SELECT SUM(g.goods_weight * o.goods_number) AS weight, " .
            ...
            "FROM " . $GLOBALS['yke']->table(1,'order_goods') . " AS o, " .
                $GLOBALS['yke']->table(1,'goods') . " AS g " .
            "WHERE o.order_id = '$order_id' and o.sysid=g.sysid  " ." and o.sysid ='".$GLOBALS['sysid']."' ".
...
function order_fee($order, $goods, $consignee)
{
    ...
    if ($order['shipping_id'] > 0 && $total['real_goods_count'] > 0)
    {
        ...
        if (!empty($shipping_info))
        {
            ...
            $sql = 'SELECT count(*) FROM ' . $GLOBALS['yke']->table(1,'cart') . " WHERE  $sql_where AND `extension_code` != 'package_buy' AND `is_shipping` = 0 AND rec_id in (".$_SESSION['sel_cartgoods'].")"." and sysid ='".$GLOBALS['sysid']."' ";  //jx
	...
	if (count($order['shipping_pay']) > 0 && $total['real_goods_count'] > 0){
		...
		foreach ($goods AS $val)
		{
			$sql_supp = "select g.supplier_id, IF(g.supplier_id='0', '本网站', s.supplier_name) AS supplier_name2 from ".$GLOBALS['yke']->table(1,'goods').
								  " AS g left join ".$GLOBALS['yke']->table(1,'supplier')." AS s on g.supplier_id=s.supplier_id and g.sysid=s.sysid where g.goods_id='". $val['goods_id'] ."' "." and g.sysid ='".$GLOBALS['sysid']."' ";
		   ...
		   $sql = 'SELECT count(*) FROM ' . $GLOBALS['yke']->table(1,'cart') . " AS c left join ". $GLOBALS['yke']->table(1,'goods') ." AS g on c.goods_id=g.goods_id and c.sysid=g.sysid WHERE g.supplier_id = '". $row_supp['supplier_id'] ."' AND $sql_where AND c.extension_code != 'package_buy' AND c.is_shipping = 0 AND c.rec_id in (".$_SESSION['sel_cartgoods'].")"." and c.sysid ='".$GLOBALS['sysid']."' ";  //jx
    ...
    if ($order['extension_code'] == 'exchange_goods')
    {
        ...
        $sql = 'SELECT SUM(eg.exchange_integral) '.
               'FROM ' . $GLOBALS['yke']->table(1,'cart') . ' AS c,' . $GLOBALS['yke']->table(1,'exchange_goods') . 'AS eg '.
               "WHERE c.goods_id = eg.goods_id and c.sysid=eg.sysid AND " . $sql_exchange .
               "  AND c.rec_type = '" . CART_EXCHANGE_GOODS . "' " .
               '  AND c.is_gift = 0 AND c.goods_id > 0 '." and c.sysid ='".$GLOBALS['sysid']."' " .
...
{
    return $GLOBALS['db']->autoExecute($GLOBALS['yke']->table(1,'order_info'),
        $order, 'UPDATE', "order_id = '$order_id'"." and sysid ='".$GLOBALS['sysid']."' ");
}
...
$sql_where = $_SESSION['user_id']>0 ? "c.user_id='". $_SESSION['user_id'] ."' " : "c.session_id = '" . SESS_ID . "' AND c.user_id=0 ";
    $sql = "SELECT c.rec_id, c.user_id, c.goods_id, c.goods_name, c.goods_sn, c.goods_number, c.market_price, " .
            ...
            " FROM " . $GLOBALS['yke']->table(1,'cart') .
            " as c LEFT JOIN " . $GLOBALS['yke']->table(1,'goods') . " as g ON c.goods_id = g.goods_id and c.sysid=g.sysid
            LEFT JOIN ". $GLOBALS['yke']->table(1,'supplier') .
            " as s ON s.supplier_id = g.supplier_id and s.sysid=g.sysid " .
			" left join " . $GLOBALS['yke']->table(1,'goods_activity') . " as ga " .
			" on ga.act_id = c.goods_id and c.sysid=ga.sysid and c.extension_code = 'package_buy'" .
			" left join " . $GLOBALS['yke']->table(1,'supplier') . " as ss on ss.supplier_id = ga.supplier_id and ss.sysid=ga.sysid " .
			" WHERE $sql_where " ." and c.sysid ='".$GLOBALS['sysid']."' ".
            " AND c.rec_type = '$type' $id_ext ";  //代码修改 By  www.hzyikai.cn  增加一个 $id_ext , package_attr_id
	...
	$arr[$key]['goods_thumb']  = $GLOBALS['db']->getOne("SELECT `goods_thumb` FROM " . $GLOBALS['yke']->table(1,'goods') . " WHERE `goods_id`='{$value['goods_id']}'"." and sysid ='".$GLOBALS['sysid']."' ");
...
function cart_amount($include_gift = true, $type = CART_GENERAL_GOODS)
{
    ...
    $sql = "SELECT SUM(goods_price * goods_number) " .
            " FROM " . $GLOBALS['yke']->table(1,'cart') .
            " WHERE $sql_where " .
            "AND rec_type = '$type' "." and sysid ='".$GLOBALS['sysid']."' ";
...
{
    /* 检查该商品是否已经存在在购物车中 */
    $sql = "SELECT COUNT(*) FROM " .$GLOBALS['yke']->table(1,'cart').
...
function cart_weight_price($type = CART_GENERAL_GOODS)
{
    ...
    /* 计算超值礼包内商品的相关配送参数 */
    $sql = 'SELECT goods_id, goods_number, goods_price FROM ' . $GLOBALS['yke']->table(1,'cart') . " WHERE extension_code = 'package_buy' AND session_id = '" . SESS_ID . "'"." and sysid ='".$GLOBALS['sysid']."' ";
    ...
    if ($row)
    {
        ...
        foreach ($row as $val)
        {
            ...
            $sql = 'SELECT count(*) FROM ' .
                    $GLOBALS['yke']->table(1,'package_goods') . ' AS pg, ' .
                    $GLOBALS['yke']->table(1,'goods') . ' AS g ' .
                    "WHERE pg.sysid = g.sysid and g.goods_id = pg.goods_id AND g.is_shipping = 0 AND pg.package_id = '"  . $val['goods_id'] . "'"." and pg.sysid ='".$GLOBALS['sysid']."' ";
            $shipping_count = $GLOBALS['db']->getOne($sql);
    ...
    $sql    = 'SELECT SUM(g.goods_weight * c.goods_number) AS weight, ' .
                ...
                'LEFT JOIN ' . $GLOBALS['yke']->table(1,'goods') . ' AS g ON g.goods_id = c.goods_id and g.sysid=c.sysid '.
                "WHERE c.session_id = '" . SESS_ID . "' " .
                "AND rec_type = '$type' AND g.is_shipping = 0 AND c.extension_code != 'package_buy'"." and c.sysid ='".$GLOBALS['sysid']."' ";
    $row = $GLOBALS['db']->getRow($sql);
...
function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0)
{
    ...
    $sql = "SELECT g.goods_name, g.goods_sn, g.is_on_sale, g.is_real, ".
            ...
            " FROM " .$GLOBALS['yke']->table(1,'goods'). " AS g ".
            " LEFT JOIN " . $GLOBALS['yke']->table(1,'member_price') . " AS mp ".
                    "ON mp.goods_id = g.goods_id and g.sysid=mp.sysid AND mp.user_rank = '$_SESSION[user_rank]' ".
            " WHERE g.goods_id = '$goods_id'"." and g.sysid ='".$GLOBALS['sysid']."' " .
            " AND g.is_delete = 0";
    ...
    {
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['yke']->table(1,'cart') .
                " WHERE goods_id='$parent' AND session_id='" . SESS_ID . "' AND extension_code <> 'package_buy'"." and sysid ='".$GLOBALS['sysid']."' ";
    ...
    $sql = "SELECT * FROM " .$GLOBALS['yke']->table(1,'products'). " WHERE goods_id = '$goods_id'"." and sysid ='".$GLOBALS['sysid']."' "." LIMIT 0, 1";
    ...
    $sql = "SELECT parent_id, goods_price " .
            "FROM " . $GLOBALS['yke']->table(1,'group_goods') .
            " WHERE goods_id = '$goods_id'" .
            " AND goods_price < '$goods_price'" .
            " AND parent_id = '$_parent_id'"." and sysid ='".$GLOBALS['sysid']."' " .
    ...
    if ($basic_list)
    {
        $sql = "SELECT goods_id, SUM(goods_number) AS count " .
                ...
                " AND goods_id " . db_create_in(array_keys($basic_list)) ." and sysid ='".$GLOBALS['sysid']."' ".
                " GROUP BY goods_id";
        $res = $GLOBALS['db']->query($sql);
        while ($row = $GLOBALS['db']->fetchRow($res))
        {
    ...
    if ($basic_count_list)
    {
        $sql = "SELECT parent_id, SUM(goods_number) AS count " .
                ...
                " AND extension_code <> 'package_buy' " .
                " AND parent_id " . db_create_in(array_keys($basic_count_list))." and sysid ='".$GLOBALS['sysid']."' " .
                " GROUP BY parent_id";
        $res = $GLOBALS['db']->query($sql);
        while ($row = $GLOBALS['db']->fetchRow($res))
        {
    ...
    foreach ($basic_list as $parent_id => $fitting_price)
    {
        ...
        $parent['sysid']= $GLOBALS['sysid'];
        /* 添加 */
        $GLOBALS['db']->autoExecute($GLOBALS['yke']->table(1,'cart'), $parent, 'INSERT');
    ...
    {
        /* 检查该商品是否已经存在在购物车中 */
        $sql = "SELECT goods_number FROM " .$GLOBALS['yke']->table(1,'cart').
                " WHERE session_id = '" .SESS_ID. "' AND goods_id = '$goods_id' ".
                " AND parent_id = 0 AND goods_attr = '" .get_goods_attr_info($spec). "' " .
                " AND extension_code <> 'package_buy' " .
		" AND user_id= '".$_SESSION['user_id']."'".
                " AND rec_type = 'CART_GENERAL_GOODS'"." and sysid ='".$GLOBALS['sysid']."' ";
        ...
        if($row) //如果购物车已经有此物品，则更新
        {
            ...
            {
                $goods_price = get_final_price($goods_id, $num, true, $spec);
                $sql = "UPDATE " . $GLOBALS['yke']->table(1,'cart') . " SET goods_number = '$num'" .
    ...
    /* 把赠品删除 */
    $sql = "DELETE FROM " . $GLOBALS['yke']->table(1,'cart') . " WHERE session_id = '" . SESS_ID . "' AND is_gift <> 0"." and sysid ='".$GLOBALS['sysid']."' ";
    $GLOBALS['db']->query($sql);
...
{
	$sql = "SELECT * FROM " . $GLOBALS['yke']->table(1,'ykesmart_distrib_goods') . " WHERE goods_id = '$goods_id' AND (distrib_time = 0 OR (start_time <= " . gmtime() . " AND end_time >= " .gmtime()."))"." and sysid ='".$GLOBALS['sysid']."' ";
	...
	if($rows)
	{
		...
		{
			 $sql = "SELECT shop_price FROM " . $GLOBALS['yke']->table(1,'goods') . " WHERE goods_id = '" . $rows['goods_id'] . "'"." and sysid ='".$GLOBALS['sysid']."' ";
...
function clear_cart($type = CART_GENERAL_GOODS,$other='')
{
	...
	$sql = "DELETE FROM " . $GLOBALS['yke']->table(1,'cart') .
            " WHERE $sql_where  AND rec_type = '$type' $other"." and sysid ='".$GLOBALS['sysid']."' ";
    $GLOBALS['db']->query($sql);
}
...
function get_goods_attr_info($arr, $type = 'pice')
{
    ...
    if (!empty($arr))
    {
        ...
        $sql = "SELECT a.attr_name, ga.attr_value, ga.attr_price ".
                "FROM ".$GLOBALS['yke']->table(1,'goods_attr')." AS ga, ".
                    $GLOBALS['yke']->table(1,'attribute')." AS a ".
                "WHERE " .db_create_in($arr, 'ga.goods_attr_id')." AND a.attr_id = ga.attr_id and ga.sysid=a.sysid "." and ga.sysid ='".$GLOBALS['sysid']."' ";
        $res = $GLOBALS['db']->query($sql);
...
{
    $sql = "SELECT * FROM " . $GLOBALS['yke']->table(1,'users') .
            " WHERE user_id = '$user_id'"." and sysid ='".$GLOBALS['sysid']."' ";
...
{
    return $GLOBALS['db']->autoExecute($GLOBALS['yke']->table(1,'users'),
        $user, 'UPDATE', "user_id = '$user_id'"." and sysid ='".$GLOBALS['sysid']."' ");
}
...
{
    $sql = "SELECT * FROM " . $GLOBALS['yke']->table(1,'user_address') .
            " WHERE user_id = '$user_id'"." and sysid ='".$GLOBALS['sysid']."' ";
...
{
    $sql = "SELECT * FROM " . $GLOBALS['yke']->table(1,'user_address') .
            " WHERE address_id = '$address_id'"." and sysid ='".$GLOBALS['sysid']."' ";
...
function user_bonus($user_id, $goods_amount = 0)
{
    ...
    $sql = "SELECT t.type_id, t.type_name, t.type_money, b.bonus_id, b.supplier_id " .
            "FROM " . $GLOBALS['yke']->table(1,'bonus_type') . " AS t," .
                $GLOBALS['yke']->table(1,'user_bonus') . " AS b " .
            "WHERE t.type_id = b.bonus_type_id and t.sysid=b.sysid " .
            "AND t.use_start_date <= '$today' " .
            "AND t.use_end_date >= '$today' " .
            "AND t.min_goods_amount <= '$goods_amount' " .
            "AND b.user_id<>0 " .
            "AND b.user_id = '$user_id' " .
            "AND b.order_id = 0"." and t.sysid ='".$GLOBALS['sysid']."' ";
    return $GLOBALS['db']->getAll($sql);
}
...
function bonus_info($bonus_id, $bonus_sn = '')
{
    $sql = "SELECT t.*, sum(t.type_money) as type_money, b.* " .
            "FROM " . $GLOBALS['yke']->table(1,'bonus_type') . " AS t," .
                $GLOBALS['yke']->table(1,'user_bonus') . " AS b " .
            "WHERE t.type_id = b.bonus_type_id and t.sysid=b.sysid "." and t.sysid ='".$GLOBALS['sysid']."' ";
...
{
    $sql = "SELECT order_id FROM " . $GLOBALS['yke']->table(1,'user_bonus') .
            " WHERE bonus_id = '$bonus_id'"." and sysid ='".$GLOBALS['sysid']."' ";
...
{
    $sql = "UPDATE " . $GLOBALS['yke']->table(1,'user_bonus') .
            " SET order_id = '$order_id', used_time = '" . gmtime() . "' " .
            "WHERE bonus_id = '$bonus_id'"." and sysid ='".$GLOBALS['sysid']."' "." LIMIT 1";
...
function order_refund($order, $refund_type, $refund_note, $refund_amount = 0)
{
    ...
    elseif (2 == $refund_type)
    {
        ...
        $GLOBALS['db']->autoExecute($GLOBALS['yke']->table(1,'user_account'), $account, 'INSERT');
...
function get_cart_goods($other='')
{
	...
	$sql = "SELECT c.*, g.supplier_id, IF(c.parent_id, c.parent_id, c.goods_id) AS pid  " .
            " FROM " . $GLOBALS['yke']->table(1,'cart') . " AS c left join " .$GLOBALS['yke']->table(1,'goods')." AS g ".
			" on c.goods_id=g.goods_id and c.sysid=g.sysid ".
			" WHERE $sql_where AND c.rec_type = '" . CART_GENERAL_GOODS . "' $other " ." and c.sysid ='".$GLOBALS['sysid']."' ".
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        if (trim($row['goods_attr']) != '')
        {
            ...
            $sql = "SELECT attr_value FROM " . $GLOBALS['yke']->table(1,'goods_attr') . " WHERE goods_attr_id "." and sysid ='".$GLOBALS['sysid']."' " .
        ...
        {
            $goods_thumb = $GLOBALS['db']->getOne("SELECT `goods_thumb` FROM " . $GLOBALS['yke']->table(1,'goods') . " WHERE `goods_id`='{$row['goods_id']}'"." and sysid ='".$GLOBALS['sysid']."' ");
            $row['goods_thumb'] = '../'. get_image_path($row['goods_id'], $goods_thumb, true);
        }
        if ($row['extension_code'] == 'package_buy')
        {
            $row['package_goods_list'] = get_package_goods($row['goods_id'], $row['package_attr_id'] ); //修改 by www.yikaiqiche.cn 增加一个变量
        }
	$row['is_cansel'] = is_cansel($row['goods_id'], $row['product_id'], $row['package_buy']);

	if($row['supplier_id'])
	{
		$supplier_name = $GLOBALS['db']->getOne("select supplier_name from ". $GLOBALS['yke']->table(1,'supplier') ." where supplier_id='". $row['supplier_id']."' "." and sysid ='".$GLOBALS['sysid']."' ");
...
function get_consignee($user_id)
{
    ...
    else
    {
        ...
        if ($user_id > 0)
        {
            /* 取默认地址 */
            $sql = "SELECT ua.*".
                    " FROM " . $GLOBALS['yke']->table(1,'user_address') . "AS ua, ".$GLOBALS['yke']->table(1,'users').' AS u '.
                    " WHERE u.user_id='$user_id' AND ua.address_id = u.address_id and ua.sysid=u.sysid "." and ua.sysid ='".$GLOBALS['sysid']."' ";
...
function exist_real_goods($order_id = 0, $flow_type = CART_GENERAL_GOODS)
{
    if ($order_id <= 0)
    {
        ...
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['yke']->table(1,'cart') .
                ...
                "AND rec_type = '$flow_type'"." and sysid ='".$GLOBALS['sysid']."' ";
    }
    else
    {
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['yke']->table(1,'order_goods') .
                " WHERE order_id = '$order_id' AND is_real = 1"." and sysid ='".$GLOBALS['sysid']."' ";
    }
...
function last_shipping_and_payment()
{
    $sql = "SELECT shipping_id, pay_id " .
            " FROM " . $GLOBALS['yke']->table(1,'order_info') .
...
function get_total_bonus()
{
    ...
    $sql = "SELECT SUM(c.goods_number * t.type_money)" .
            "FROM " . $GLOBALS['yke']->table(1,'cart') . " AS c, "
                    . $GLOBALS['yke']->table(1,'bonus_type') . " AS t, "
                    . $GLOBALS['yke']->table(1,'goods') . " AS g " .
            "WHERE c.session_id = '" . SESS_ID . "' " .
            "AND c.is_gift = 0 " .
            "AND c.goods_id = g.goods_id and c.sysid=t.sysid " .
            "AND g.bonus_type_id = t.type_id and g.sysid=t.sysid " .
            "AND t.send_type = '" . SEND_BY_GOODS . "' " .
            "AND t.send_start_date <= '$today' " .
            "AND t.send_end_date >= '$today' " .
            "AND c.rec_type = '" . CART_GENERAL_GOODS . "'"." and c.sysid ='".$GLOBALS['sysid']."' ";
    ...
    $sql = "SELECT FLOOR('$amount' / min_amount) * type_money " .
            "FROM " . $GLOBALS['yke']->table(1,'bonus_type') .
            " WHERE send_type = '" . SEND_BY_ORDER . "' " .
            " AND send_start_date <= '$today' " .
            "AND send_end_date >= '$today' " .
            "AND min_amount > 0 "." and sysid ='".$GLOBALS['sysid']."' ";
...
function change_user_bonus($bonus_id, $order_id, $is_used = true)
{
    if ($is_used)
    {
        $sql = 'UPDATE ' . $GLOBALS['yke']->table(1,'user_bonus') . ' SET ' .
                'used_time = ' . gmtime() . ', ' .
                "order_id = '$order_id' " .
                "WHERE bonus_id = '$bonus_id'"." and sysid ='".$GLOBALS['sysid']."' ";
    }
...
function merge_order($from_order_sn, $to_order_sn)
{
    ...
    do
    {
        ...
        $order['sysid']= $GLOBALS['sysid'];
        if ($GLOBALS['db']->autoExecute($GLOBALS['yke']->table(1,'order_info'), addslashes_deep($order), 'INSERT'))
        {
            break;
        }
        else
    ...
    /* 更新订单商品 */
    $sql = 'UPDATE ' . $GLOBALS['yke']->table(1,'order_goods') .
            " SET order_id = '$order_id' " .
            "WHERE order_id " . db_create_in(array($from_order['order_id'], $to_order['order_id']))." and sysid ='".$GLOBALS['sysid']."' ";
    $GLOBALS['db']->query($sql);
    ...
    /* 删除原订单 */
    $sql = 'DELETE FROM ' . $GLOBALS['yke']->table(1,'order_info') .
            " WHERE order_id " . db_create_in(array($from_order['order_id'], $to_order['order_id']))." and sysid ='".$GLOBALS['sysid']."' ";
    $GLOBALS['db']->query($sql);

    /* 删除原订单支付日志 */
    $sql = 'DELETE FROM ' . $GLOBALS['yke']->table(1,'pay_log') .
            " WHERE order_id " . db_create_in(array($from_order['order_id'], $to_order['order_id']))." and sysid ='".$GLOBALS['sysid']."' "." and sysid ='".$GLOBALS['sysid']."' ";
    $GLOBALS['db']->query($sql);
...
function get_agency_by_regions($regions)
{
    ...
    $sql = "SELECT region_id, agency_id " .
            "FROM " . $GLOBALS['yke']->table(1,'region') .
            " WHERE region_id " . db_create_in($regions) .
            " AND region_id > 0 AND agency_id > 0"." and sysid ='".$GLOBALS['sysid']."' ";
    $res = $GLOBALS['db']->query($sql);
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
...
function change_order_goods_storage($order_id, $is_dec = true, $storage = 0)
{
    ...
    {
        case 0 :
            $sql = "SELECT goods_id, SUM(send_number) AS num, MAX(extension_code) AS extension_code, product_id FROM " . $GLOBALS['yke']->table(1,'order_goods') .
                    " WHERE order_id = '$order_id' AND is_real = 1 GROUP BY goods_id, product_id"." and sysid ='".$GLOBALS['sysid']."' ";
        break;

        case 1 :
            $sql = "SELECT goods_id, SUM(goods_number) AS num, MAX(extension_code) AS extension_code, product_id FROM " . $GLOBALS['yke']->table(1,'order_goods') .
                    " WHERE order_id = '$order_id' AND is_real = 1 GROUP BY goods_id, product_id"." and sysid ='".$GLOBALS['sysid']."' ";
        break;
    }
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        else
        {
            $sql = "SELECT goods_id, goods_number" .
                   " FROM " . $GLOBALS['yke']->table(1,'package_goods') .
                   " WHERE package_id = '" . $row['goods_id'] . "'"." and sysid ='".$GLOBALS['sysid']."' ";
            $res_goods = $GLOBALS['db']->query($sql);
            while ($row_goods = $GLOBALS['db']->fetchRow($res_goods))
            {
                $sql = "SELECT is_real" .
                   " FROM " . $GLOBALS['yke']->table(1,'goods') .
                   " WHERE goods_id = '" . $row_goods['goods_id'] . "'"." and sysid ='".$GLOBALS['sysid']."' ";
...
function change_goods_storage($good_id, $product_id, $number = 0)
{
    ...
    if (!empty($product_id))
    {
        $sql = "UPDATE " . $GLOBALS['yke']->table(1,'products') ."
                SET product_number = product_number $number
                WHERE goods_id = '$good_id'
                AND product_id = '$product_id'"." and sysid ='".$GLOBALS['sysid']."' "."
                LIMIT 1";
        $products_query = $GLOBALS['db']->query($sql);
    }

    /* 处理商品库存 */
    $sql = "UPDATE " . $GLOBALS['yke']->table(1,'goods') ."
            SET goods_number = goods_number $number
            WHERE goods_id = '$good_id'"." and sysid ='".$GLOBALS['sysid']."' "."
            LIMIT 1";
...
{
    $sql = "SELECT pay_id FROM " . $GLOBALS['yke']->table(1,'ykesmart_payment');
    if ($is_cod)
    {
        $sql .= " WHERE is_cod = 1"." and sysid ='".$GLOBALS['sysid']."' ";
    }
    else
    {
        $sql .= " WHERE is_cod = 0"." and sysid ='".$GLOBALS['sysid']."' ";
    }
...
function compute_discount($supplierid=-1)
{
    ...
    $sql = "SELECT *" .
            "FROM " . $GLOBALS['yke']->table(1,'favourable_activity') .
            " WHERE start_time <= '$now'" .
            " AND end_time >= '$now'" .
            " AND CONCAT(',', user_rank, ',') LIKE '%" . $user_rank . "%'" .
            " AND act_type " . db_create_in(array(FAT_DISCOUNT, FAT_PRICE))." and sysid ='".$GLOBALS['sysid']."' ";
    ...
    $sql = "SELECT c.goods_id, c.goods_price * c.goods_number AS subtotal, g.cat_id, g.brand_id, g.supplier_id " .
            "FROM " . $GLOBALS['yke']->table(1,'cart') . " AS c, " . $GLOBALS['yke']->table(1,'goods') . " AS g " .
            "WHERE c.goods_id = g.goods_id and c.sysid=g.sysid " .
            "AND " .$sql_where.
            "AND c.parent_id = 0 " .
            "AND c.is_gift = 0 " .
            "AND rec_type = '" . CART_GENERAL_GOODS . "'"." and c.sysid ='".$GLOBALS['sysid']."' ";
...
function get_give_integral()
{
        $sql = "SELECT SUM(c.goods_number * IF(g.give_integral > -1, g.give_integral, c.goods_price))" .
                ...
                "AND c.parent_id = 0 " .
                "AND c.rec_type = 0 " .
                "AND c.is_gift = 0"." and c.sysid ='".$GLOBALS['sysid']."' ";
...
function integral_to_give($order)
{
    ...
    else
    {
        $sql = "SELECT SUM(og.goods_number * IF(g.give_integral > -1, g.give_integral, og.goods_price)) AS custom_points, SUM(og.goods_number * IF(g.rank_integral > -1, g.rank_integral, og.goods_price)) AS rank_points " .
                "FROM " . $GLOBALS['yke']->table(1,'order_goods') . " AS og, " .
                          $GLOBALS['yke']->table(1,'goods') . " AS g " .
                "WHERE og.goods_id = g.goods_id and og.sysid=g.sysid " .
                "AND og.order_id = '$order[order_id]' " .
                "AND og.goods_id > 0 " .
                "AND og.parent_id = 0 " .
                "AND og.is_gift = 0 AND og.extension_code != 'package_buy'"." and og.sysid ='".$GLOBALS['sysid']."' ";
...
function send_order_bonus($order_id)
{
    ...
    if ($bonus_list)
    {
        ...
        $sql = "SELECT u.user_id, u.user_name, u.email " .
                "FROM " . $GLOBALS['yke']->table(1,'order_info') . " AS o, " .
                          $GLOBALS['yke']->table(1,'users') . " AS u " .
                "WHERE o.order_id = '$order_id' " .
                "AND o.user_id = u.user_id and o.sysid=u.sysid "." and o.sysid ='".$GLOBALS['sysid']."' ";
        $user = $GLOBALS['db']->getRow($sql);
        ...
        foreach ($bonus_list AS $bonus)
        {
            ...
            /* 修改用户红包 */
            $sql = "INSERT INTO " . $GLOBALS['yke']->table(1,'user_bonus') . " (sysid,bonus_type_id, user_id) " .
                    "VALUES("."'".$GLOBALS['sysid']."',"."'$bonus[type_id]', '$user[user_id]')";
...
function return_order_bonus($order_id)
{
    ...
    if ($bonus_list)
    {
        ...
        foreach ($bonus_list AS $bonus)
        {
            $sql = "DELETE FROM " . $GLOBALS['yke']->table(1,'user_bonus') .
                    " WHERE bonus_type_id = '$bonus[type_id]' " .
                    "AND user_id = '$user_id' " .
                    "AND order_id = '0' LIMIT " . $bonus['number']." and sysid ='".$GLOBALS['sysid']."' ";
            $GLOBALS['db']->query($sql);
        }
    }
}
...
function order_bonus($order_id)
{
    ...
    $sql = "SELECT b.type_id, b.type_money, SUM(o.goods_number) AS number " .
            "FROM " . $GLOBALS['yke']->table(1,'order_goods') . " AS o, " .
                      $GLOBALS['yke']->table(1,'goods') . " AS g, " .
                      $GLOBALS['yke']->table(1,'bonus_type') . " AS b " .
            " WHERE o.order_id = '$order_id' " .
            " AND o.is_gift = 0 " .
            " AND o.goods_id = g.goods_id and o.sysid=g.sysid " .
            " AND g.bonus_type_id = b.type_id and g.sysid=b.sysid " .
            " AND b.send_type = '" . SEND_BY_GOODS . "' " .
            " AND b.send_start_date <= '$today' " .
            " AND b.send_end_date >= '$today' " ." and o.sysid ='".$GLOBALS['sysid']."' ".
            " GROUP BY b.type_id ";
    ...
    $sql = "SELECT add_time " .
            " FROM " . $GLOBALS['yke']->table(1,'order_info') .
            " WHERE order_id = '$order_id' "." and sysid ='".$GLOBALS['sysid']."' "." LIMIT 1";
    ...
    $sql = "SELECT type_id, type_money, IFNULL(FLOOR('$amount' / min_amount), 1) AS number " .
            ...
            "AND send_end_date >= '$order_time' "." and sysid ='".$GLOBALS['sysid']."' ";
...
function add_package_to_cart($package_id, $num = 1, $package_attr_id='', $package_prices='')
{
    ...
    {
         /* 检查该商品是否已经存在在购物车中 */
        $sql = "SELECT sysid,goods_number FROM " .$GLOBALS['yke']->table(1,'cart').
                " WHERE session_id = '" .SESS_ID. "' AND goods_id = '" . $package_id . "' ".
                " AND parent_id = 0 AND extension_code = 'package_buy' " .
                " AND package_attr_id = '$package_attr_id'  AND rec_type = '" . CART_GENERAL_GOODS . "'"." and sysid ='".$GLOBALS['sysid']."' ";
...
function judge_package_stock($package_id, $package_num = 1)
{
    $sql = "SELECT goods_id, product_id, goods_number
            FROM " . $GLOBALS['yke']->table(1,'package_goods') . "
            WHERE package_id = '" . $package_id . "'"." and sysid ='".$GLOBALS['sysid']."' ";
    $row = $GLOBALS['db']->getAll($sql);
    if (empty($row))
    {
        return true;
    ...
    if ($goods['product_ids'] != '')
    {
        $sql = "SELECT p.product_id
                FROM " . $GLOBALS['yke']->table(1,'products') . " AS p, " . $GLOBALS['yke']->table(1,'package_goods') . " AS pg
                WHERE pg.product_id = p.product_id and p.sysid=pg.sysid
                AND pg.package_id = '$package_id'
                AND pg.goods_number * $package_num > p.product_number
                AND p.product_id IN (" . trim($goods['product_ids'], ',') . ")"." and p.sysid ='".$GLOBALS['sysid']."' ";
        $row = $GLOBALS['db']->getAll($sql);
    ...
    if ($goods['goods_ids'] != '')
    {
        $sql = "SELECT g.goods_id
                FROM " . $GLOBALS['yke']->table(1,'goods') . "AS g, " . $GLOBALS['yke']->table(1,'package_goods') . " AS pg
                WHERE pg.goods_id = g.goods_id and g.sysid=pg.sysid
                AND pg.goods_number * $package_num > g.goods_number
                AND pg.package_id = '" . $package_id . "'
                AND pg.goods_id IN (" . trim($goods['goods_ids'], ',') . ")"." and g.sysid ='".$GLOBALS['sysid']."' ";
        $row = $GLOBALS['db']->getAll($sql);
...
function cart_weight_price2($type = CART_GENERAL_GOODS, $supplier_id)
{
    ...
    if ($row)
    {
        ...
        foreach ($row as $val)
        {
            ...
            $sql = 'SELECT count(*) FROM ' .
                    $GLOBALS['yke']->table(1,'package_goods') . ' AS pg, ' .
                    $GLOBALS['yke']->table(1,'goods') . ' AS g ' .
                    "WHERE pg.sysid=g.sysid and g.supplier_id='". $supplier_id ."' and g.goods_id = pg.goods_id AND g.is_shipping = 0 AND pg.package_id = '"  . $val['goods_id'] . "'"." and pg.sysid ='".$GLOBALS['sysid']."' ";
            $shipping_count = $GLOBALS['db']->getOne($sql);
            ...
            if ($shipping_count > 0)
            {
                ...
                $sql = 'SELECT SUM(g.goods_weight * pg.goods_number) AS weight, ' .
                    ...
                    $GLOBALS['yke']->table(1,'package_goods') . ' AS pg, ' .
                    $GLOBALS['yke']->table(1,'goods') . ' AS g ' .
                    "WHERE g.supplier_id='". $supplier_id ."' and g.goods_id = pg.goods_id and g.sysid = pg.sysid AND g.is_shipping = 0 AND pg.package_id = '"  . $val['goods_id'] . "'"." and pg.sysid ='".$GLOBALS['sysid']."' ";
    ...
    $sql    = 'SELECT SUM(g.goods_weight * c.goods_number) AS weight, ' .
                    ...
                    'SUM(c.goods_number) AS number '.
                'FROM ' . $GLOBALS['yke']->table(1,'cart') . ' AS c '.
                'LEFT JOIN ' . $GLOBALS['yke']->table(1,'goods') . ' AS g ON g.goods_id = c.goods_id and g.sysid and c.sysid '.
...
function get_order_rebate($suppid){
	...
	$sql = "select rebate_id, rebate_paytime_start, rebate_paytime_end from ". $GLOBALS['yke']->table(1,'supplier_rebate') ." where supplier_id='$spkey' and is_pay_ok=0 "." and sysid ='".$GLOBALS['sysid']."' "." order by rebate_id desc limit 0,1";
	$row = $GLOBALS['db']->getRow($sql);
	$nowtime = gmtime();
	...
	else
	{
		...
		while($kkk=='yes')
		{
			...
			$sql2 = "select rebate_id, rebate_paytime_start, rebate_paytime_end from ". $GLOBALS['yke']->table(1,'supplier_rebate') ." where supplier_id='$spkey' and is_pay_ok=0 "." and sysid ='".$GLOBALS['sysid']."' "." order by rebate_id desc limit 0,1";
...
{
	$sql = "select IF(g.supplier_id, g.supplier_id,0) AS supplier_id, og.rec_id, og.goods_number, og.goods_price from ".$GLOBALS['yke']->table(1,"order_goods") .
				" AS og left join ".$GLOBALS['yke']->table(1,"goods") ." AS g on og.goods_id=g.goods_id and og.sysid=g.sysid ".
				" where og.order_id = '$new_order_id' "." andog. sysid ='".$GLOBALS['sysid']."' ";
	$res = $GLOBALS['db']->query($sql);
	$split_orders = array();
	$all_amount = 0;
	while ($row=$GLOBALS['db']->fetchRow($res))
	...
	if ($count_split_orders>1)
	{
		$sql="delete from ".$GLOBALS['yke']->table(1,'order_info')." where order_id='$new_order_id' "." and sysid ='".$GLOBALS['sysid']."' ";
		$GLOBALS['db']->query($sql);
	}
...
{
		$sql="select supplier_rebate_paytime from ". $GLOBALS['yke']->table(1,'supplier') ." where supplier_id='$supplier_id'"." and sysid ='".$GLOBALS['sysid']."' ";
		$supplier_rebate_paytime = $GLOBALS['db']->getOne($sql);

		$sql = "select rebate_paytime_start, rebate_paytime_end from ". $GLOBALS['yke']->table(1,'supplier_rebate') ." where supplier_id= '$supplier_id' and is_pay_ok=0 "." and sysid ='".$GLOBALS['sysid']."' "." order by rebate_id DESC LIMIT 0,1";
		$row = $GLOBALS['db']->getRow($sql);
		...
		$sql="insert into ". $GLOBALS['yke']->table(1,'supplier_rebate') ."(sysid,rebate_paytime_start, rebate_paytime_end, supplier_id) value("."'".$GLOBALS['sysid']."',"."'$rebate_paytime_start', '$rebate_paytime_end', '$supplier_id') ";
		$GLOBALS['db']->query($sql);
}
...
function is_cansel($goods_id, $product_id, $package_buy)
{
	...
	$sql = "select is_on_sale, goods_number from ". $GLOBALS['yke']->table(1,'goods') ." where goods_id='$goods_id' "." and sysid ='".$GLOBALS['sysid']."' ";
	$row = $GLOBALS['db']->getRow($sql);
	if (!$row['is_on_sale'])
	{
		return '0';
	...
	else
	{
		if ($product_id>0)
		{
			$sql2 = "select product_number from ". $GLOBALS['yke']->table(1,'products') ." where product_id='$product_id' "." and sysid ='".$GLOBALS['sysid']."' ";
			$row2 = $GLOBALS['db']->getRow($sql2);
...
{
    $sql = 'SELECT region_name FROM ' . $GLOBALS['yke']->table(1,'region') .
            " WHERE region_id = '$region_id' "." and sysid ='".$GLOBALS['sysid']."' ";
...
function is_pups($id){
	global $db,$yke;
	$sql = "select shipping_code,support_pickup from ".$yke->table(1,'shipping')." where shipping_id=".$id." and sysid ='".$GLOBALS['sysid']."' ";
	$info = $db->getRow($sql);
	if($info){
...
function get_pickup_info($cityid,$suppid=0){
	global $db,$yke;
	$sql = "select pp.*,r.region_name from ".$yke->table(1,'pickup_point')." as pp left join ".$yke->table(1,'region')." as r on pp.district_id=r.region_id where pp.city_id=".$cityid." and pp.supplier_id=".$suppid.""." and sysid ='".$GLOBALS['sysid']."' "." order by pp.id";
	$info = $db->getAll($sql);
	return $info;
}
/**
...
function get_pickup_one_info($id){
	global $db,$yke;
	$sql = "select * from ".$yke->table(1,'pickup_point')." where id=".$id." and sysid ='".$GLOBALS['sysid']."' ";
	$info = $db->getRow($sql);
	return $info;
}

function update_order_shipping($order_id){
    $sql = "update ".$GLOBALS['yke']->table(1,'order_info')." set shipping_status = ".SS_SHIPPED." where order_id = $order_id"." and sysid ='".$GLOBALS['sysid']."' ";
    $GLOBALS['db']->query($sql);
}
?>