...
<head>
    ...
    <link rel="stylesheet" href="/<?php echo $this->_var['project_path']; ?>/styles/global/index2.css">
...
<template id="add-box">
    <div class="add-box" >
        <span class="cancel-add-box">&times</span>
        <div data-spy="scroll" data-target="#sidebar{{count}}" style="height:480px;overflow:auto">
            <!--Nav Bar -->
            <nav class="col-xs-4 bs-docs-sidebar">
                <div style="position:fixed">
                    <!--第一级navigation-->
                    <ul id="sidebar{{count}}" class="nav nav-stacked fixed">
                        <li v-for="firstTierCategory in templates" track-by="$index">
                            <a href="#addBox{{count}}_Group{{firstTierCategory[0]}}">{{firstTierCategory[1]}}</a>
                            <!--第二级navigation-->
                            <ul class="nav nav-stacked">
                                <li  v-for="secondTierCategory in firstTierCategory[2]" track-by="$index">
                                    <a href="#addBox{{count}}_Group{{firstTierCategory[0]}}_Sub{{secondTierCategory[4]}}">{{secondTierCategory[2]}}</a>
                                </li>
                            </ul>
                        </li>
                    </ul>
            ...
            <div class="col-xs-7">
                <!--第一级分类内容-->
                <div v-for="firstTierCategory in templates" track-by="$index" class="group"  style="padding-top: 250px; margin-top: -250px;">
                    <div id="addBox{{count}}_Group{{firstTierCategory[0]}}"></div>
                    <!--第二级分类内容-->
                    <div v-for="secondTierCategory in firstTierCategory[2]" track-by="$index"  style="padding-top: 20px; margin-top: -20px;">
                        <div class="img-item" id="addBox{{count}}_Group{{firstTierCategory[0]}}_Sub{{secondTierCategory[4]}}">
...
<script>
    ...
    templateInfo.sort((x, y) => x['lib_id'] - y['lib_id']);
                        ...
                        if (result) {
                            ...
                            for (var i = 0; i < (ref).soloColImgs.length; i++) {
                                if ((ref).soloColImgs[i].id == index) {
                                    console.log( (ref).soloColImgs[i].imgUrl);
                            ...
                            if (result) {
                                ...
                                for(var i=0;i<(ref).imgs.length;i++){
                                    if((ref).imgs[i].id==index){
                                        (ref).imgs[i].imgUrl=projectPath+result;
                                    }
                                }
                            } else {
                                alert('图片不符要求');
    ...
    /*******************************************drug and drop************************************************/
    //由于section 这个关键字已经被当作items的参数，因此在#hook 内不可随意使用section
    $(function() {
    ...
    function transformer(templateInfo) {
        ...
        for(j= 0;j<a.length;j++){
            for (  i ; i < templateInfo.length&&a[j]==templateInfo[i]['lib_type_id']; i++ ) {
                secondCategory[j].push([templateInfo[i]['lib_id'],templateInfo[i]['lib_name'],templateInfo[i]['lib_title'],`${projectPath}${templateInfo[i]['lib_preview_img']}`,templateInfo[i]['sub_lib_id']]);
            }
        }
